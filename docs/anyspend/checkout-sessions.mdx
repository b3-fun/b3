...
description: "Decoupled checkout sessions for flexible payment flows with AnySpend"
---
...
<Steps>
  <Step title="Merchant creates session" icon="server">
    `POST /checkout-sessions` returns `{ id, status: "open" }`
  </Step>
  <Step title="User picks payment method" icon="wallet">
    `POST /orders { checkoutSessionId }` returns `{ id, globalAddress, oneClickBuyUrl }`
  </Step>
  <Step title="User pays" icon="money-bill">
    **Crypto:** send to `globalAddress` | **Onramp:** redirect to `oneClickBuyUrl`
  </Step>
  <Step title="Merchant polls for completion" icon="check">
