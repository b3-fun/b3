...
3. **Conversion \+ Execution**\
   AnySpend converts fiat to crypto (typically USDC on Base) and executes the transaction.
4. **Wallet Delivery**\
   The result is sent to the user's connected wallet. No crypto acquisition required.
...
- Users can pay directly with their Coinbase account balances
