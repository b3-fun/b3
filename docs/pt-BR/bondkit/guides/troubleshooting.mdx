...
<AccordionGroup>
  ...
  <Accordion title="Erro: 'chainId não suportado: X. Este SDK está configurado apenas para Base'">
    **Causa:** Tentativa de usar um chain ID que não seja Base.
...
<AccordionGroup>
  <Accordion title="Erro: 'Compra: execução revertida' ou 'Venda: execução revertida'">
    ...
    **Soluções:**
    ...
    if (status !== 1) {
      throw new Error("Token não está na fase de bonding - usar métodos DEX em vez disso");
    }
  ...
  <Accordion title="Erro: 'Fase de Bonding Encerrada' ou problemas de negociação DEX">
    **Causa:** Token migrado para fase DEX, mas ainda utilizando métodos de bonding.
    ...
    **Soluções:**
    ...
    async function getOptimalTradingMethod(tokenAddress) {
      ...
      switch (status) {
        ...
        default:
          throw new Error(`Status desconhecido do token: ${status}`);
      }
    }
    ```
  </Accordion>
...
<AccordionGroup>
  <Accordion title="Erro: 'Conta para transação não pôde ser determinada'">
    **Causa:** Carteira não conectada ou configurada adequadamente.
...
<AccordionGroup>
  ...
  <Accordion title="Problemas de Estimativa de Gás">
    **Problema:** Transações falham com "fora de gás" ou erros de estimativa de gás.
