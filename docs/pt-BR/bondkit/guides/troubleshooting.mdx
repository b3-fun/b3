...
Comece aqui para identificação rápida de problemas:

```typescript
import { BondkitToken, BondkitTokenFactory } from "@b3dotfun/sdk/bondkit";

// Função rápida de verificação de saúde
async function bondkitHealthCheck() {
  try {
    // Testar conexão com a fábrica
    ...
    if (!implementationAddress) {
      return { status: "error", issue: "Falha na conexão com a fábrica" };
    }
...
<AccordionGroup>
  <Accordion title="Erro: 'Endereço de contrato inválido é necessário para BondkitToken'">
    **Causa:** Endereço do token está faltando, inválido ou não está checksummed.
  ...
  <Accordion title="Erro: 'DeployBondkitToken: execução revertida'">
    ...
    **Soluções:**
    ...
    const config = {
      ...
      // Configuração V4 - todos obrigatórios
    ...
    function validateConfig(config) {
      ...
      if (config.aggressivenessFactor < 0 || config.aggressivenessFactor > 100) {
        errors.push("Aggressividade deve ser 0-100");
      }
...
<AccordionGroup>
  <Accordion title="Erro: 'Buy: execução revertida' ou 'Sell: execução revertida'">
    ...
    **Soluções:**
    ...
    async function diagnoseBuyError(token, amount, minTokensOut) {
      ...
      if (progress.progress >= 1.0) {
        return "Meta alcançada - migração disponível, não é possível comprar mais";
      }
      ...
      return "Erro desconhecido de compra - verificar detalhes da transação";
    }
    ...
    async function diagnoseSellError(token, tokenAmount) {
      ...
      if (progress.raised === 0n) {
        return "Sem liquidez disponível - não é possível vender quando nada foi arrecadado";
      }
      
      return "Erro desconhecido de venda - verificar detalhes da transação";
    }
    ```
  </Accordion>
  ...
  <Accordion title="Erro: 'BondingPhaseEnded' ou problemas de negociação DEX">
          ...
          return {
            ...
            quoteMethod: async (amount) => {
              const params = {
                ...
                tokenOutDecimals: 18,
                slippageTolerance: 0.005,
                recipient: "0x0000000000000000000000000000000000000000" // Dummy para cotação
              };
...
### Problemas de Integração com Carteiras
...
<AccordionGroup>
  <Accordion title="Erro: 'Conta para transação não pôde ser determinada'">
    ...
    **Soluções:**
    ```typescript
    // Opção 1: Usar chave privada (apenas servidor)
    ...
    async function ensureWalletConnected(token) {
      ...
      const connected = await token.connect(window.ethereum);
      if (!connected) {
        throw new Error("Falha ao conectar token à carteira");
      }
  ...
  <Accordion title="Erro: 'Tempo de confirmação da transação esgotado'">
    ...
    **Soluções:**
    ...
    const options = {
      ...
      maxFeePerGas: parseGwei("20"), // Taxa mais alta para inclusão mais rápida
      maxPriorityFeePerGas: parseGwei("2")
    };
    
    await token.buy(parseEther("10"), 0n, options);
    
    // Espera manual da transação com timeout
  ...
  <Accordion title="Problemas específicos da MetaMask/Carteira">
    **Problemas Comuns da Carteira:**
    
    ```typescript
    // Mudança de rede para MetaMask
    ...
    // Auxiliar para importação de token
...
<AccordionGroup>
  <Accordion title="Problemas de Conexão RPC">
    ...
    **Soluções:**
    ...
    async function testRPCHealth() {
      ...
      for (const rpcUrl of rpcUrls) {
        try {
          ...
          return rpcUrl; // Retorna o primeiro RPC funcional
        } catch (error) {
    ...
    // Sobrescrever RPC do SDK se necessário
