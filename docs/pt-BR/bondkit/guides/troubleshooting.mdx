    ...
    **Soluções:**
    ...
    async function diagnoseBuyError(token, amount, minTokensOut) {
      ...
      if (tradingTokenAddress !== "0x0000000000000000000000000000000000000000") {
        ...
        if (balance < required) {
          return `Saldo insuficiente de ${await token.getTradingTokenSymbol()}. Necessário: ${formatEther(required)}, Possuído: ${formatEther(balance)}`;
        }
      }
    ...
    async function diagnoseSellError(token, tokenAmount) {
      ...
      if (balance < tokenAmount) {
        return `Saldo de token insuficiente. Necessário: ${formatEther(tokenAmount)}, Possuído: ${formatEther(balance)}`;
      }
...
<AccordionGroup>
  <Accordion title="Erro: 'Conta para transação não pôde ser determinada'">
    ...
    **Soluções:**
    ...
    // Opção 3: Verificar conexão antes de transações
...
<AccordionGroup>
  ...
  <Accordion title="Problemas de Estimação de Gás">
    **Problema:** Transações falham com "out of gas" ou erros de estimação de gás.
    
    **Soluções:**
    ```typescript
    // Auxiliar de estimação de gás
