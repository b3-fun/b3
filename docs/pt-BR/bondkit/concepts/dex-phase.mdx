...
A fase DEX é o estágio final do ciclo de vida de um token Bondkit, onde a negociação transita da curva de bonding automatizada para um pool totalmente descentralizado na Uniswap v4. Esta fase começa imediatamente após a migração e representa a descentralização completa.

```mermaid
stateDiagram-v2
    [*] --> BondingPhase: Token Criado
    BondingPhase --> MigrationReady: Meta Alcançada
...
Quando `migrateToDex()` é chamado após alcançar a meta:

1. **Status do Contrato**: Status do token muda para `TokenStatus.Dex`
2. **Pool da Uniswap v4**: Novo pool criado com liquidez acumulada
3. **Inicialização de Preço**: Preço inicial do pool calculado a partir do preço de saída da curva de bonding
4. **Renúncia de Propriedade**: Propriedade do contrato transferida para o endereço zero
5. **Negociação Habilitada**: Negociação completa na DEX começa imediatamente
...
| **Tipos de Ordem** | Compra/venda apenas | Recursos completos da DEX |
...
if (status === TokenStatus.Dex) {
  // Usar métodos de negociação da DEX
...
Tokens Bondkit na Uniswap v4 incluem hooks personalizados para funcionalidades aprimoradas:
...
// Ganhar taxas de negociação + potenciais incentivos
```

**Benefícios LP:**
- **Taxas de Negociação**: Ganhar 0.3% de todo o volume de swap
- **Programas de Incentivo**: Potenciais recompensas adicionais
- **Valorização de Preço**: Beneficiar-se do crescimento do token
- **Perda Impermanente**: Riscos padrão de AMM aplicam
...
- **Incentivos de Liquidez**: Incentivar participação LP  
...
<AccordionGroup>
  <Accordion title="Os tokens podem voltar para a fase de bonding?">
    **Não** - A migração é permanente e irreversível. Uma vez na fase DEX, o token sempre negociará na Uniswap v4.
  </Accordion>
  
  <Accordion title="O que acontece com a liquidez não utilizada da curva de bonding?">
    Todos os tokens de negociação acumulados da fase de bonding tornam-se a liquidez inicial no pool da Uniswap v4, emparelhados com tokens bondkit calculados no preço de saída.
  </Accordion>
  
  <Accordion title="Existem restrições na fase DEX?">
    Sem restrições - tokens tornam-se ERC20s padrão negociando na Uniswap v4. Composabilidade DeFi completa é habilitada.
  </Accordion>
  
  <Accordion title="Como as taxas de negociação são diferentes?">
    A fase DEX usa taxas padrão da Uniswap v4 (0.3%) que vão para os provedores de liquidez em vez da taxa de 5% da fase de bonding para o destinatário da taxa.
  </Accordion>
</AccordionGroup>
