...
AnySpend x402 usa assinaturas criptográficas em vez de aprovações de tokens tradicionais, possibilitando **pagamentos sem gás** para os usuários. O facilitador paga todas as taxas de gás enquanto os usuários simplesmente assinam mensagens de autorização.
...
| **Tipo de Nonce** | Sequencial (autoincremento) | Bytes32 aleatórios |
...
const signature = await signTransferWithAuthorization({
  ...
  nonce: randomBytes32()         // Gera nonce aleatório
});
```
...
- ✅ **Sem gás para o pagador** - Facilitador paga o gás
- ✅ **Suporte do ecossistema** - Suportado por carteiras e dapps principais
...
permit(
    ...
    uint256 value,         // Quantidade de aprovação
...
const nonce = await token.nonces(ownerAddress);  // Obtém o nonce atual
...
Facilitador chama `permit()` para definir a permissão on-chain
</Step>
...
### Verificando Suporte ao Token

```typescript
// Verifica se o token suporta EIP-3009 (estilo USDC)
const hasTransferWithAuth = await token.read.transferWithAuthorization !== undefined;

// Verifica se o token suporta EIP-2612 (Permit padrão)
...
- Eficiente em gás (único slot de armazenamento)
- Padrão em todas as implementações de permit

**Implementação:**
```typescript
// Obtém o nonce atual do contrato do token
const currentNonce = await token.read.nonces([ownerAddress]);

// Assina com o nonce atual
const signature = await signPermit({
  // ...
  nonce: currentNonce
});
...
- Estado do nonce armazenado on-chain em mapping
...
```typescript
// Recupera endereço do assinante da assinatura
...
// Verifica se o assinante corresponde ao endereço esperado
...
// 5. Inclui assinatura no cabeçalho X-PAYMENT
...
<Note>
Todos os custos de gás são pagos pelo facilitador e incluídos na taxa de 0,25% da AnySpend. Os usuários nunca pagam gás diretamente.
</Note>
