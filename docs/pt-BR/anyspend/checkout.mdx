...
<Info>
  Para a **API REST de sessões de checkout** (fluxos baseados em sessão e conduzidos pelo backend), veja [Sessões de Checkout](/anyspend/checkout-sessions). Esta página cobre os **componentes de checkout React**.
</Info>
...
```mermaid
flowchart LR
    A[Renderizar Checkout] --> B{Possui Formulário?}
...
<Steps>
  ...
  <Step title="Renderizar o checkout">
    ...
    <AnySpendCheckout
      ...
      themeColor="#4f46e5"
      onSuccess={(result) => {
        console.log("Pagamento concluído:", result.orderId);
      }}
    />;
    ```
  </Step>
...
<ParamField path="summaryLines" type="CheckoutSummaryLine[]">
  Itens de linha adicionais do resumo do pedido, como taxas da plataforma, gorjetas ou cobranças de serviço
</ParamField>
...
<ParamField path="senderAddress" type="string">
  Preencher previamente o endereço do remetente para mostrar saldos de tokens antes da conexão da carteira
</ParamField>

<ParamField path="checkoutSessionId" type="string">
  Vincule este checkout a uma sessão de checkout do backend para rastreamento
</ParamField>

#### Callbacks

<ParamField path="onSuccess" type="(result: { txHash?: string; orderId?: string }) => void">
  Chamado em caso de pagamento bem-sucedido
</ParamField>
...
<ParamField path="mode" type="'page' | 'embedded'" default="'page'">
  `page` para autônomo, `embedded` para integrado ao seu layout
</ParamField>
...
Estende `AnySpendCheckout` com integração de fluxo de trabalho B3. Quando um usuário completa o pagamento, um fluxo de trabalho B3 é automaticamente acionado com os dados do pagamento e quaisquer metadados personalizados.
...
<ParamField path="workflowId" type="string">
  ID do workflow B3 a ser acionado em caso de pagamento bem-sucedido
</ParamField>

<ParamField path="orgId" type="string">
  ID da organização que possui o workflow
</ParamField>

<ParamField path="callbackMetadata" type="object">
  Metadados mesclados ao pedido. O campo `inputs` é acessível nos workflows via `{{root.result.inputs.*}}`.
</ParamField>
...
interface CheckoutItem {
  ...
  /** Quantidade */
  quantity: number;
  /** Metadados personalizados exibidos como pares rótulo: valor (ex.: { "Tamanho": "Grande" }) */
...
interface CheckoutSummaryLine {
  /** Rótulo de exibição (ex.: "Taxa da Plataforma", "Gorjeta") */
  label: string;
...
<Tip>
  O tipo de campo `"address"` renderiza um formulário de endereço completo (rua, cidade, estado, CEP, país) automaticamente — não é necessário definir cada subcampo.
</Tip>
...
interface DiscountResult {
  ...
  /** Montante final após o desconto em wei */
  final_amount?: string;
...
interface CheckoutFormComponentProps {
  /** Chame quando os valores do formulário mudarem */
  onSubmit: (data: Record<string, unknown>) => void;
  /** Sinalize se o formulário é válido */
...
function CheckoutPage({ cart, shippingAddress }) {
  ...
  return (
    <AnySpendCheckout
      ...
      tax={{ amount: "850000", label: "Imposto
