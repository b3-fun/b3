...
---
Sessões de checkout permitem que você crie um objeto de rastreamento no seu servidor antes do usuário escolher um método de pagamento. A sessão é um registro de banco de dados leve -- sem chamadas de API externas, sem criação de pedido. O pedido vem depois, quando o usuário se compromete a pagar.

<Info>
  Procurando pelo componente de checkout do React? Veja [Checkout](/anyspend/checkout). Esta página cobre a **REST API** para fluxos controlados pelo servidor.
</Info>
...
<Steps>
  ...
  <Step title="Usuário paga">
    **Crypto:** enviar para `globalAddress` | **Fiat onramp:** redirecionar para `oneClickBuyUrl`
  </Step>
...
stateDiagram-v2
    ...
    open --> expired : TTL expirado / manualmente expirado
```
...
Todos os campos são opcionais. Configuração de pagamento (quantia, tokens, cadeias) fica no pedido, não na sessão.
...
- A sessão deve estar `open` (`400` se expirada/processando/completa)
...
Se nenhuma variável de template estiver presente, `?sessionId=<uuid>` é anexado automaticamente.
...
Hook de consulta com auto-polling. Para de fazer polling quando o status alcança `complete` ou `expired`.
...
// 3. Usuário envia crypto para order.data.globalAddress

// 4. Verificar sessão até completar
...
// Passos 1-2 são os mesmos que acima, mas incluem configuração de onramp na criação do pedido:
const order = await fetch(`${BASE_URL}/orders`, {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({
