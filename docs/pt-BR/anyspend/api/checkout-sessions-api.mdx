...
---
Sessões de checkout representam uma tentativa de pagamento individual. Você pode criar sessões no lado do servidor para gerar URLs de checkout únicas para cada cliente, preencher previamente detalhes de pagamento e rastrear conversões com seus próprios IDs de referência.

<Info>
  Sessões de checkout expiram após 30 minutos por padrão. Use o parâmetro `expires_in` para personalizar o período de expiração (mínimo de 5 minutos, máximo de 24 horas).
</Info>

## O objeto Sessão de Checkout

<ResponseField name="id" type="string">
  Identificador único para a sessão de checkout (ex.: `cs_abc123`).
</ResponseField>
...
<ResponseField name="status" type="string">
  Status atual da sessão. Um dos: `open`, `processing`, `completed`, `expired`, `failed`.
</ResponseField>
...
<ResponseField name="client_reference_id" type="string | null">
  Seu ID de referência interno para esta sessão.
</ResponseField>
...
<ResponseField name="customer_email" type="string | null">
  Endereço de email do cliente. Criptografado em repouso, incluído em cargas de webhook.
</ResponseField>

<ResponseField name="customer_name" type="string | null">
  Nome do cliente. Criptografado em repouso, incluído em cargas de webhook.
</ResponseField>
...
<Tip>
  ...
  `https://exemplo.com/success?session_id={SESSION_ID}`
</Tip>
      ...
      "data": [
        {
          ...
          "amount": "50000000",
          "amount_usd": "50.00",
          "success_url": "https://exemplo.com/success?session_id=cs_abc123",
...
<ParamField body="cancel_url" type="string">
  URL para redirecionar o cliente se ele cancelar.
</ParamField>

<ParamField body="client_reference_id" type="string">
  Seu ID de referência interno (ex.: ID do pedido). Máximo de 200 caracteres.
</ParamField>

<ParamField body="metadata" type="object">
  Pares chave-valor arbitrários passados para webhooks e a transação resultante. Até 50 chaves, valores de caracteres de 500.
</ParamField>

<ParamField body="customer_email" type="string">
  Endereço de email do cliente. Criptografado em repouso e incluído em cargas de webhook para cumprimento de pedido.
</ParamField>

<ParamField body="customer_name" type="string">
  Nome do cliente. Criptografado em repouso e incluído em cargas de webhook para cumprimento de pedido.
</ParamField>
...
curl -X POST "https://platform-api.anyspend.com/api/v1/checkout-sessions" \
  ...
  -d '{
    "payment_link_id": "pl_abc123",
    "success_url": "https://exemplo.com/success?session_id={SESSION_ID}",
    "cancel_url": "https://exemplo.com/cancel",
    "client_reference_id": "order_12345",
    "metadata": {
      "order_id": "12345"
...
const session = await anyspend.checkoutSessions.create({
  ...
  successUrl: "https://exemplo.com/success?session_id={SESSION_ID}",
...
<ParamField path="id" type="string" required>
  O ID da sessão de checkout (ex.: `cs_abc123`).
</ParamField>
