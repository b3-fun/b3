...
---
O cliente SDK da Plataforma AnySpend é um cliente TypeScript **headless** (sem dependência do React) para a API completa da Plataforma AnySpend. Funciona em todos os locais onde o JavaScript moderno é executado -- Node.js, navegadores, Cloudflare Workers, Deno e Bun.
...
const platform = new AnySpendPlatformClient(process.env.ANYSPEND_API_KEY!, {
  ...
  maxRetries: 3,             // contagem de tentativas automáticas para erros 5xx / de rede
...
O cliente é organizado em namespaces de recursos, cada um com métodos que mapeiam diretamente para endpoints REST.
...
Para conveniência, cada método `list` tem um auxiliar `listAutoPaginate` correspondente que retorna um iterador assíncrono. Ele busca páginas nos bastidores à medida que você consome itens.
...
<Tip>
  A auto-paginação respeita automaticamente os limites de taxa. Se a API retornar um `429`, o iterador espera pela duração do `Retry-After` antes de buscar a próxima página.
</Tip>
...
try {
  ...
  if (err instanceof NotFoundError) {
    ...
    console.log(`Limitado por taxa. Tente novamente após ${err.retryAfter} segundos`);
    ...
    // Captura geral para qualquer erro de API
    console.log(err.status, err.code, err.message);
  }
}
```

### Referência de Classe de Erro
...
O cliente tenta automaticamente falhas de solicitações nos seguintes casos:
...
As tentativas usam **backoff exponencial** com jitter. O padrão é 3 tentativas, configurável via a opção `maxRetries`.
...
<Warning>
  Tentativas **não** são realizadas para erros `4xx` além de `429`, pois esses indicam um problema do lado do cliente que não será resolvido tentando novamente.
</Warning>

## Idempotência

Todas as solicitações `POST` e `PATCH` incluem automaticamente um cabeçalho `Idempotency-Key`. Isso garante que, se uma solicitação for repetida (por exemplo, devido a um erro de rede), o servidor não a processará duas vezes.
