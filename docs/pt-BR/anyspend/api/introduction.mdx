...
description: >-
  ...
  acompanhe transações e muito mais
lang: pt-BR
originalPath: anyspend/api/introduction.mdx
---
A API da Plataforma AnySpend oferece controle programático total sobre sua infraestrutura de pagamentos. Construa integrações, automatize fluxos de trabalho e deixe agentes de IA gerenciar sua pilha de comércio -- tudo através de uma API REST limpa, ao estilo Stripe.

## Para quem é isso?

- **Desenvolvedores** construindo integrações personalizadas com a AnySpend
- **Agentes de IA** que criam links de pagamento, gerenciam produtos ou puxam análises
...
Acrescente `client_reference_id` e `metadata` ao seu URL de link de pagamento:

```
https://anyspend.com/pay/xK9mQ2?client_reference_id=order_123&metadata[user_id]=clerk_abc
```

Esses valores fluem automaticamente para o seu webhook `payment.completed`.

### Padrão 2: Sessões do lado do servidor (enterprise)
...
A resposta inclui um campo `url` -- redirecione seu cliente para lá para pagar. Todos os dados estão incluídos no webhook quando o pagamento é concluído.
...
|------|--------------|------------|----------|
| **Aberto** | Nenhum (limitação de taxa baseada em IP) | 5 solicitações/min por IP | Pagamento Rápido -- pagamentos únicos sem necessidade de conta |
| **Chave de API** | chave `asp_xxx` via cabeçalho | 120 solicitações/min por chave | Todas as operações padrão da API -- CRUD em links de pagamento, produtos, transações, etc. |
| **JWT** | token de sessão do painel | Limites de sessão padrão | Rotas exclusivas do painel (gerenciadas automaticamente pelo aplicativo web) |

A maioria dos desenvolvedores usará autenticação por Chave de API. Veja a página de [Autenticação](/anyspend/api/authentication) para detalhes sobre a criação e gerenciamento de chaves.
...
Veja a página de [Paginação & Filtragem](/anyspend/api/pagination) para detalhes sobre navegação nas respostas de lista.
...
Veja a página de [Erros](/anyspend/api/errors) para a taxonomia completa de erros.
...
| Nível | Limite | Janela |
|------|-------|--------|
| Aberto (Pagamento Rápido) | 5 solicitações | 1 minuto |
| Chave de API | 120 solicitações | 1 minuto |
...
Para reenvios seguros de solicitações `POST` e `PATCH`, inclua um cabeçalho `Idempotency-Key`. Solicitações duplicadas com a mesma chave e corpo retornam a resposta armazenada em cache. Veja a página de [Idempotência](/anyspend/api/idempotency) para detalhes.
...
<Card title="Transações" icon="arrow-right-arrow-left" href="/anyspend/api/transactions">
  Consulte o histórico de transações, filtre por status e puxe detalhes de liquidação.
</Card>
...
<Step title="Obtenha sua chave de API">
  Vá ao seu [Painel AnySpend](https://anyspend.com/dashboard) em **Configurações > Chaves de API** e crie uma chave com as permissões de que precisa.
</Step>
