...
---
Clientes representam as pessoas que pagam por meio de seus links de pagamento. Registros de clientes são criados automaticamente quando uma carteira completa uma sessão de checkout, mas você também pode criá-los e gerenciá-los manualmente via API.
...
<ResponseField name="id" type="string">
  Identificador único para o cliente (por exemplo, `cust_abc123`).
</ResponseField>
...
<ResponseField name="total_spent" type="string">
  Total gasto em todas as transações concluídas (em equivalente em USD).
</ResponseField>
...
<ParamField body="metadata" type="object">
  Pares chave-valor arbitrários (por exemplo, `{"plan": "pro"}`). Até 50 chaves, valores de 500 caracteres.
</ParamField>
...
<ParamField path="id" type="string" required>
  O ID do cliente (por exemplo, `cust_abc123`).
</ParamField>
...
<Tabs>
  <Tab title="Response">
    Retorna o [objeto Cliente](#the-customer-object) completo.
  </Tab>
</Tabs>
...
<Tip>
  O `wallet_address` não pode ser alterado após a criação. Para associar um cliente com uma carteira diferente, crie um novo registro de cliente.
</Tip>
...
<Tabs>
  <Tab title="Response">
    Retorna o [objeto Cliente](#the-customer-object) atualizado.
  </Tab>
</Tabs>
...
Exclua permanentemente um registro de cliente. O histórico de transações associado a este cliente é preservado, mas a referência ao cliente mostrará como excluída.
...
<Info>
  Exportações são limitadas a 10.000 registros. Para conjuntos de dados maiores, use paginação com o endpoint de listagem.
</Info>
