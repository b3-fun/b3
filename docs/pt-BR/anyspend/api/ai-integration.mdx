...
---
A API da Plataforma AnySpend é projetada com a **integração AI-first** em mente. Cada endpoint possui um `operationId` único, esquemas JSON detalhados e convenções consistentes que facilitam o uso com chamadas de função LLM, agentes autônomos e frameworks de orquestração de IA.

## Especificação OpenAPI

A especificação completa do OpenAPI 3.1 está disponível em:
...
- Um `operationId` único para cada endpoint (por exemplo, `createPaymentLink`, `listTransactions`)
...
Converte operações OpenAPI em definições de ferramentas Claude. Aqui está um exemplo de definição de ferramenta para criar um link de pagamento:
...
```json
{
  ...
  "input_schema": {
    ...
    "properties": {
      "name": {
        "type": "string",
        "description": "Nome legível por humanos para o link de pagamento (por exemplo, 'Plano Pro Mensal')"
      },
      "amount": {
        "type": "string",
        "description": "Quantidade de pagamento na menor unidade do token. Para USDC (6 decimais), '1000000' = 1 USDC."
      },
      "recipient_address": {
        "type": "string",
  ...
  "function": {
    ...
    "parameters": {
      ...
      "properties": {
        ...
        "amount": {
          "type": "string",
          "description": "Quantidade de pagamento na menor unidade (por exemplo, '1000000' para 1 USDC)"
        },
        "recipient_address": {
          "type": "string",
...
## Exemplo: Fluxo de Conversação

Aqui está como um assistente de IA pode usar as ferramentas AnySpend em uma conversa:

<Steps>
  <Step title="Usuário pede para criar um link de pagamento">
    **Usuário:** "Crie um link de pagamento para 50 USDC para a assinatura do plano Pro"
  ...
  <Step title="IA responde ao usuário">
    "Eu criei um link de pagamento para 50 USDC para a Assinatura do Plano Pro. Aqui está o link: https://anyspend.com/pay/pl_abc123"
  </Step>
</Steps>

## Construindo um Agente de Pagamento com IA

Abaixo está um exemplo completo de um agente de IA que pode gerenciar pagamentos usando Claude e o SDK AnySpend:
...
const tools: Anthropic.Tool[] = [
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        amount: { type: "string", description: "Quantidade na menor unidade" },
  ...
  },
  {
    ...
    description: "Liste transações recentes com filtro de status opcional",
    input_schema: {
      type: "object" as const,
      properties: {
        status: { type: "string", enum: ["completed", "pending", "failed"] },
        limit: { type: "number", description: "Resultados máximos (padrão 10)" },
      },
    },
  },
  {
    name: "get_transaction_stats",
    description: "Obtenha estatísticas agregadas de transações",
    input_schema: {
      type: "object" as const,
      properties: {},
    },
...
function requireString(input: Record<string, unknown>, key: string): string {
  ...
  if (typeof value !== "string" || !value) {
    throw new Error(`Campo ausente ou inválido: ${key}`);
  }
  return value;
}
...
const SYSTEM_PROMPT =
  "Você é um assistente de gerenciamento de pagamentos. Use as ferramentas fornecidas para ajudar os usuários a criar links de pagamento, visualizar transações e gerenciar sua conta AnySpend. As quantidades estão na menor unidade do token -- para USDC com 6 decimais, 1 USDC = 1000000.";
...
async function chat(userMessage: string) {
  ...
  // Trate o uso da ferramenta em um loop
...
<Info>
  Estamos construindo um servidor Protocolo de Contexto de Modelo (MCP) que exporá a API completa da Plataforma AnySpend como ferramentas MCP. Isso permitirá que qualquer cliente de IA compatível com MCP (Claude Desktop, Cursor, Windsurf, etc.) interaja diretamente com sua conta AnySpend.
...
<AccordionGroup>
  <Accordion title="list_payment_links">
    ```json
    {
      ...
      "description": "Liste todos os links de pagamento, opcionalmente filtrados por status",
      "input_schema": {
        "type": "object",
        "properties": {
          "status": {
  ...
  <Accordion title="get_analytics_overview">
    ```json
    {
      ...
      "description": "Obtenha uma visão geral de análises de alto nível incluindo volume total, contagem de transações e principais links de pagamento",
      "input_schema": {
        "type": "object",
        "properties": {
          "from": {
  ...
  <Accordion title="create_discount_code">
    ```json
    {
      ...
      "description": "Crie um código de desconto que os clientes podem aplicar no checkout",
      "input_schema": {
        "type": "object",
        "properties": {
          "code": {
            "type": "string",
            "description": "A string do código de desconto (por exemplo, 'VERAO20')"
          },
          "type": {
            "type": "string",
          ...
          "value": {
            "type": "string",
            "description": "Valor do desconto: porcentagem (por exemplo, '20' para 20%) ou valor fixo na menor unidade"
          },
          "max_uses": {
            "type": "number",
  ...
  <Accordion title="export_transactions">
    ```json
    {
      ...
      "description": "Exporte transações como CSV ou JSON para relatórios e análises",
      "input_schema": {
        "type": "object",
        "properties": {
          "format": {
...
<CardGroup cols={2}>
  <Card title="Use chaves somente leitura para agentes de consulta" icon="eye">
    Agentes de IA que apenas leem dados (listam transações, visualizam análises) devem usar chaves de API com permissão **somente leitura**. Isso impede mutações acidentais de chamadas de ferramentas alucinadas.
  </Card>
  ...
  <Card title="Sempre valide quantidades" icon="calculator">
    LLMs podem alucinar números. Antes de executar uma chamada de ferramenta que envolva uma quantidade, valide se a quantidade é razoável. Considere adicionar uma etapa de confirmação para quantidades acima de um limiar.
  </Card>
...
<Warning>
  **Nunca exponha sua chave de API em código do lado do cliente ou prompts de IA visíveis para os usuários finais.** Use um proxy do lado do servidor para fazer chamadas de API em nome do seu agente de IA. Se usar chamadas de função Claude ou OpenAI, a chave de API deve existir apenas na sua camada de execução de ferramentas de backend -- nunca no prompt do sistema LLM.
</Warning>
