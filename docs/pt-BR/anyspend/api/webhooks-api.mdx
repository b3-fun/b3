...
---
Webhooks permitem que você receba notificações HTTP POST em tempo real quando eventos ocorrem na sua conta AnySpend. Configure endpoints, escolha quais eventos assinar e monitore o histórico de entrega.

<Info>
  Todos os endpoints de gerenciamento de webhook requerem permissão de **admin** a menos que notado. Endpoints somente leitura (listar, obter, entregas) requerem permissão de **leitura**.
</Info>
...
<ParamField path="POST /webhooks" type="admin">
  Crie um novo endpoint de webhook. A resposta inclui um campo `secret` usado para verificar assinaturas de webhook. Armazene isso com segurança -- ele só é retornado uma vez no momento da criação.
</ParamField>
...
<Warning>
  O campo `secret` só está incluído na resposta de criação. Copie imediatamente e armazene-o em suas variáveis de ambiente ou gerenciador de segredos.
</Warning>
...
### Tentar Novamente uma Entrega Falhada

<ParamField path="POST /webhooks/:id/deliveries/:did/retry" type="admin">
  Tente manualmente uma entrega de webhook falhada novamente. A entrega deve ter um status de `falhada`.
</ParamField>
...
| `secret` | `string` | Segredo de assinatura (só é retornado na criação) |
| `is_active` | `boolean` | Se o webhook está atualmente ativo |
| `success_count` | `number` | Total de entregas bem-sucedidas |
| `failure_count` | `number` | Total de entregas falhadas |
| `last_triggered_at` | `string \| null` | Carimbo de data/hora ISO 8601 da última tentativa de entrega |
| `created_at` | `string` | Carimbo de data/hora ISO 8601 de criação |
| `updated_at` | `string` | Carimbo de data/hora ISO 8601 da última atualização |
...
| `checkout.completed` | Uma sessão de checkout foi concluída (inclui dados de formulário, envio, informações de desconto) |
...
<Note>
  Sempre retorne um código de status `2xx` dentro de 30 segundos. Entregas falhadas são reenviadas até 3 vezes com backoff exponencial.
</Note>
