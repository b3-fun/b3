...
<Info>
  A gestão de códigos de desconto requer permissão de **admin**. O endpoint `/validate` requer permissão de **leitura**, tornando-o seguro para ser chamado de fluxos de checkout do lado do cliente.
</Info>
...
<ParamField path="active" type="boolean" query>
  Filtrar por status ativo. Omitir para retornar todos os códigos.
</ParamField>
...
<ParamField path="code" type="string" required>
  A string do código de desconto (por exemplo, `SUMMER25`). Deve ser único. Convertido automaticamente para maiúsculas.
</ParamField>
...
<Note>
  O endpoint de validação **não** consome uma utilização. As utilizações são contadas apenas quando um pagamento é concluído com o desconto aplicado.
</Note>

---

### Criar Códigos de Desconto em Lote
...
| `value` | `number` | Valor do desconto (porcentagem 1-100, ou valor fixo na menor unidade) |
| `payment_link_id` | `string \| null` | Link de pagamento restrito, ou `null` para todos os links |
| `max_uses` | `number \| null` | Redenções máximas, ou `null` para ilimitado |
| `current_uses` | `number` | Número de vezes resgatado até agora |
| `min_order_amount` | `string \| null` | Valor mínimo do pedido necessário |
| `expires_at` | `string \| null` | Expiração ISO 8601, ou `null` para sem expiração |
...
| Código é restrito a um link de pagamento diferente | `Código não é válido para este link de pagamento` |
