...
---
Todos os endpoints de listagem na API da Plataforma AnySpend retornam respostas paginadas. Esta página aborda como navegar por grandes conjuntos de resultados, filtrar dados e ordenar resultados.

## Formato da resposta de listagem

Cada endpoint de listagem retorna um envelope consistente:
...
|--------------|-----------|---------------------------------------------------------------------------------------------|
| `object`     | `string`  | Sempre `"list"` para respostas de listagem.                                                 |
| `data`       | `array`   | O array de objetos de recurso para a página atual. Cada item inclui seu próprio campo `object` type. |
...
| `url`        | `string`  | O caminho do endpoint da API para esta listagem.                                             |
...
A maioria dos endpoints de listagem suporta parâmetros de consulta para filtrar resultados. Os filtros disponíveis dependem do tipo de recurso.

### Links de Pagamento

| Parâmetro | Tipo                   | Descrição                                  |
|-----------|------------------------|--------------------------------------------|
| `search`  | `string`               | Pesquisar por nome ou código curto (correspondência parcial). |
| `active`  | `"true"` ou `"false"`  | Filtrar por status ativo/inativo.          |

```bash
# Pesquisar por links de pagamento contendo "premium"
curl "https://platform-api.anyspend.com/api/v1/payment-links?search=premium" \
  -H "Authorization: Bearer asp_live_abc123..."

# Apenas links de pagamento ativos
...
```bash
# Sessões completadas para um link de pagamento
...
Endpoints de listagem que suportam ordenação aceitam os parâmetros `sort` e `order`.

| Parâmetro | Tipo      | Padrão      | Descrição                        |
|-----------|-----------|-------------|----------------------------------|
| `sort`    | `string`  | `created_at`| O campo pelo qual ordenar. Os valores permitidos variam conforme o recurso. |
| `order`   | `"asc"` ou `"desc"` | `desc` | Direção da ordenação.            |

### Campos de ordenação para Links de Pagamento

| Campo         | Descrição                                      |
|---------------|------------------------------------------------|
| `created_at`  | Quando o link de pagamento foi criado (padrão).|
| `updated_at`  | Quando o link de pagamento foi modificado pela última vez. |
| `name`        | Alfabético por nome.                           |
| `current_uses`| Número de checkouts completados.               |

```bash
# Links de pagamento mais populares primeiro
...
Quando precisar buscar todos os registros (por exemplo, para exportação de dados ou sincronização), use o `limit=100` máximo para minimizar o número de chamadas à API e permanecer dentro dos limites de taxa.
</Accordion>

<Accordion title="Use has_more em vez de total_count para loops de paginação">
`has_more` é o sinal autoritário para se buscar a próxima página. Evite calcular contagens de página a partir de `total_count`, pois o total pode mudar entre as solicitações.
</Accordion>

<Accordion title="Cache total_count para controles de paginação da UI">
Ao construir uma UI de paginação (por exemplo, "Página 3 de 8"), busque `total_count` uma vez e armazene em cache para a sessão. Atualize apenas quando o usuário atualizar ou aplicar novos filtros.
</Accordion>
...
Se estiver paginando por um grande conjunto de dados, esteja atento ao limite de 120 solicitações/min. Para conjuntos de dados muito grandes, adicione um curto atraso entre as solicitações de página:
