...
---
A API da Plataforma AnySpend fornece endpoints de exportação para baixar seus dados de transações e clientes em massa. As exportações suportam formatos CSV e JSON com filtragem opcional.
...
| `status` | `string` | Não | todos | Filtrar por status: `completed`, `pending`, `failed` |
| `from` | `string` | Não | -- | Data de início (ISO 8601, ex. `2025-01-01T00:00:00Z`) |
| `to` | `string` | Não | -- | Data de término (ISO 8601) |

### Saída CSV

As exportações em CSV incluem uma linha de cabeçalho seguida por uma linha por transação:
...
As exportações em JSON retornam um array de objetos de transação:
...
console.log("Exportadas transações para transactions.csv");
```
...
console.log(`Exportadas ${transactions.length} transações`);
...
console.log("Exportados clientes para customers.csv");
```
...
<CardGroup cols={2}>
  ...
  <Card title="CSV para planilhas" icon="table">
    Exportações em CSV são abertas diretamente no Excel, Google Sheets e outras ferramentas de planilha para análise rápida.
  </Card>
  <Card title="JSON para uso programático" icon="code">
    Exportações em JSON são ideais para alimentar dados em pipelines de análise, bancos de dados ou dashboards personalizados.
  </Card>
</CardGroup>

<Warning>
  As exportações estão sujeitas a limitação de taxa. Grandes exportações (mais de 10.000 registros) podem levar vários segundos para serem geradas. A solicitação será bloqueada até que a exportação esteja completa -- planeje de acordo para conjuntos de dados muito grandes.
</Warning>
