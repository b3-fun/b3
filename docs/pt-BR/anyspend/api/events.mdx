...
---
O endpoint de eventos fornece um registro de auditoria de todas as ações da API realizadas em sua conta. Cada operação de criação, atualização e exclusão é registrada com metadados, incluindo o método de solicitação, caminho, endereço IP e código de status.
...
<ParamField path="GET /events" type="read">
  Lista eventos da API com filtragem opcional. Os resultados são retornados em ordem cronológica inversa (mais recentes primeiro).
</ParamField>

<ParamField path="type" type="string" query>
  Filtrar por tipo de evento (ex.: `payment_link.created`, `webhook.updated`, `discount_code.deleted`).
</ParamField>

<ParamField path="resource_type" type="string" query>
  Filtrar por tipo de recurso (ex.: `payment_link`, `webhook`, `discount_code`, `widget`, `notification`).
</ParamField>

<ParamField path="from" type="string" query>
  Timestamp ISO 8601. Retorna apenas eventos criados em ou após este momento.
</ParamField>

<ParamField path="to" type="string" query>
  Timestamp ISO 8601. Retorna apenas eventos criados em ou antes deste momento.
</ParamField>
...
|-------|------|-------------|
| `id` | `string` | Identificador único do evento (ex.: `evt_abc123`) |
| `event_type` | `string` | O tipo de ação que ocorreu (veja [Tipos de Eventos](#tipos-de-eventos)) |
...
| `api_key.created` | Uma chave de API foi criada |
...
| `discount_code` | Entradas de código de desconto |
| `widget` | Configurações de widgets |
| `notification` | Configurações de notificação |
| `api_key` | Gerenciamento de chaves de API |
...
<Note>
  Os eventos são retidos por 90 dias. Para uma retenção mais longa, exporte os eventos periodicamente usando os filtros de intervalo de datas.
</Note>
