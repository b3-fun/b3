...
Este guia mostra como usar o **AnySpend X402** na **Solana** tanto para aplicações de servidor (aceitando pagamentos) quanto para clientes (fazendo pagamentos). O AnySpend X402 na Solana permite pagamentos de tokens SPL sem gas usando assinaturas de transações, tornando-o perfeito para APIs com paywall, agentes de IA e serviços premium.

**Principais Características:**
- **Transações sem gas** - Facilitador patrocina todas as taxas de transação
...
const SOLANA_PAYTO_ADDRESS = 'SeuEnderecoDeCarteiraSolana...';

// Configure os endpoints de pagamento
app.use(
  paymentMiddleware(
...
4. Facilitador verifica e submete a transação (sem gas)
...
### Opção 1: Usando Fetch (Agentes de IA & Backend)
...
async function main() {
  ...
  // Envolver fetch com suporte a pagamento
  const fetchWithPayment = wrapFetchWithPayment(fetch, signer);

  // Fazer a solicitação de pagamento
...
**Pontos Chave:**
- Use chave privada codificada em base58 (de um par de chaves Solana)
- Manipula automaticamente respostas 402 e fluxo de pagamento
- Retorna hash da transação no cabeçalho `x-payment-response`
...
#### Configuração do Wrapper do App

Envolver seu app com os provedores de carteira:
...
- Totalmente sem gas - usuários apenas assinam, facilitador paga as taxas
- Integra Solana Wallet Adapter v1 com assinaturas X402 v2
...
Para outros tokens SPL, especifique o endereço do mint e os decimais.
...
Para testes na Solana devnet:

1. Obtenha SOL devnet de [Solana Faucet](https://faucet.solana.com/)
2. Use mint de USDC devnet: `4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU`
3. Defina a rede para `solana-devnet` na sua configuração
...
<Card title="Métodos de Assinatura" icon="signature" href="/anyspend/x402-signature-methods">
  Mergulho profundo nas assinaturas de transações Solana
</Card>
...
<AccordionGroup>
<Accordion title="Falha na verificação de assinatura da transação">
Verifique se:
...
<Accordion title="Adaptador de carteira não funciona">
Certifique-se de que:
- Você envolveu seu app com `WalletProvider`
...
- A quantia do pagamento corresponde às expectativas do servidor
