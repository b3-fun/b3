...
Este guia mostra como adicionar suporte de pagamento **AnySpend x402** à sua API Express, permitindo que você aceite pagamentos em qualquer token enquanto recebe o token de sua escolha. Com apenas algumas linhas de código, sua API pode monetizar endpoints e lidar com verificação e liquidação de pagamentos automáticos.

**Principais Benefícios:**
- **Compradores** podem pagar com qualquer token que possuam (ETH, DAI, USDC, etc.)
- **Vendedores** podem receber qualquer token que preferirem (USDC, B3, DAI, tokens personalizados, etc.)
- AnySpend lida automaticamente com todas as conversões de token
- Não é necessário infraestrutura de blockchain ou gerenciamento de carteira
...
2. Middleware retorna `402 Payment Required` com detalhes do pagamento
...
### 2. Receber tokens personalizados (por exemplo, B3)
...
const endpointConfig = {
  'POST /api/premium': {
    ...
    network: NETWORK,
    config: {
      description: 'Acesso a dados premium de preço de ETH',
...
app.post('/api/premium', (req, res) => {
  ...
  res.json({
    data: 'Histórico de preço de ETH premium',
...
- AnySpend converte o pagamento deles para **tokens B3**
- Você recebe exatamente **100 tokens B3** em sua carteira
- Tanto compradores quanto vendedores mantêm suas preferências de token
...
#### Método 1: quantias simples em dólar (recomendado)
...
Exija tokens específicos em redes específicas. Você pode receber **qualquer token**, não apenas USDC!
...
const endpointConfig = {
  'POST /api/premium': {
    ...
    network: NETWORK,
    config: {
      description: 'Acesso a dados premium de preço de ETH da CoinGecko',
      mimeType: 'application/json',
    },
  }
};
...
- A flexibilidade de ambos, comprador e vendedor, é mantida
</Note>
...
const name = await token.read.name();      // por exemplo, "B3"
const version = await token.read.version(); // por exemplo, "1"
const decimals = await token.read.decimals(); // por exemplo, 18
```

**Método 2: tokens comuns**

| Token | Nome | Versão | Decimais | Endereço na Base |
|-------|------|---------|----------|------------------|
...
Os clientes podem então especificar sua preferência de token:
...
### Configuração de Facilitador Personalizado
...
const customFacilitator = new Facilitator({
  ...
  retries: 3                                  // Tenta novamente requisições falhas
});
...
Roteie pagamentos para diferentes carteiras com base no endpoint:
