...
- **Uma API Express.js** (ou vontade de criar uma)
...
app.get('/api/ai-inference', (req, res) => {
  res.json({
    result: 'Resultado de inferência de IA',
    model: 'gpt-4'
  });
});
...
app.post('/api/premium', (req, res) => {
  ...
  res.json({
    data: 'Histórico de preço do ETH premium',
...
O pacote `@b3dotfun/anyspend-x402` inclui uma instância do facilitador pré-configurada:
...
Deixe a AnySpend lidar com a conversão de token automaticamente:
...
- Você recebe exatamente o token que especificou na sua configuração
- Flexibilidade mantida para comprador e vendedor
</Note>
...
const endpointConfig = {
  '/api/data': {
    amount: '1.00',
    asset: 'USD',
    flexible: true       // Aceitar cabeçalho X-PREFERRED-TOKEN
  }
};
```
...
const customFacilitator = new Facilitator({
  ...
  retries: 3                                  // Tentar novamente requisições falhas
});
