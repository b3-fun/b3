...
- Não é necessário infraestrutura de blockchain ou gestão de carteiras
...
const endpointConfig = {
  'POST /api/premium': {
    ...
    network: NETWORK,
    config: {
      description: 'Acesso a dados premium de preço do ETH',
...
app.post('/api/premium', (req, res) => {
  ...
  res.json({
    data: 'Histórico de preços do ETH premium',
...
const endpointConfig = {
  'POST /api/premium': {
    ...
    network: NETWORK,
    config: {
      description: 'Acesso a dados premium de preço do ETH da CoinGecko',
      mimeType: 'application/json',
    },
  }
};
...
- Flexibilidade mantida tanto para comprador quanto para vendedor
</Note>
...
#### Método 3: Flexível (Deixar Cliente Escolher)
...
const customFacilitator = new Facilitator({
  ...
  retries: 3                                  // Reenviar requisições falhas
});
...
app.get('/api/premium', (req, res) => {
  res.json({ message: 'Isso custou dinheiro!' });
});
```
