...
// Aplicar middleware de pagamento
...
1. Clientes solicitam seu endpoint da API
...
app.post('/api/premium', (req, res) => {
  ...
  res.json({
    data: 'Histórico de preços de ETH premium',
...
- Você recebe exatamente **100 tokens B3** na sua carteira
- Flexibilidade completa para compradores e vendedores!
...
Você tem três métodos para configurar requisitos de pagamento:

#### Método 1: Valores em Dólar Simples (Recomendado)

Deixe AnySpend lidar com a conversão de token automaticamente:

```typescript
const endpointConfig = {
  '/api/data': {
    amount: '1.00',      // $1.00 equivalente em qualquer token
    asset: 'USD'
  },
  '/api/compute': {
    amount: '2.50',
...
const endpointConfig = {
  '/api/data': {
    price: {
      amount: '1000000000000000000', // 1 token (ajuste decimais)
      asset: {
    ...
    network: 'base-mainnet',
    config: {
      description: 'Descrição do seu endpoint da API',
      mimeType: 'application/json',
    },
  }
};
...
- A flexibilidade de compradores e vendedores é mantida
</Note>
...
Calcule preços com base em parâmetros de solicitação:
...
app.use('/api/compute', (req, res, next) => {
  ...
  // Anexe configuração dinâmica à solicitação
  req.x402Config = {
    amount: totalPrice,
    asset: 'USD'
  };
...
const customFacilitator = new Facilitator({
  ...
  retries: 3                                  // Tenta novamente requisições falhadas
});
