...
O Facilitador AnySpend x402 gerencia pagamentos multi-token e cross-chain para o protocolo x402. Ele cuida das trocas de tokens, pontes entre cadeias e liquidação para que os desenvolvedores possam aceitar pagamentos em qualquer token enquanto recebem na moeda de sua preferência.
...
Gerencia automaticamente pagamentos entre diferentes redes blockchain:

- **Pagamentos na mesma cadeia**: Transferências diretas de tokens com sobrecarga mínima
- **Pagamentos cross-chain**: Ponte automática entre redes (por exemplo, ETH → Base)
...
- **Preferência do vendedor**: Comerciantes recebem na moeda de liquidação escolhida
- **Melhores preços**: Rotas através de fontes de liquidez DEX ótimas
...
**Propósito:**
- Validação rápida (menos de 100ms)
...
```json
{
  ...
  "paymentToken": "0xfde4...",    // Endereço USDT
  "network": "base-mainnet"
}
```
...
- Contrato do token mantém contador de nonce
...
- Padrão: 5 minutos a partir do momento da assinatura
...
- **Verificação**: Menos de 100ms para validação de assinatura
- **Liquidação na mesma cadeia**: Média de 2-10 segundos
- **Liquidação cross-chain**: 2-5 minutos (depende da ponte)
- **Geração de cotação**: Menos de 200ms

### Escalabilidade

- Lida com 1000+ pagamentos simultâneos
...
Abstract, Arbitrum, Avalanche, B3, Base, Cadeia BNB, Ethereum, IoTeX, Optimism, Peaq, Polygon, Sei
