...
import { AuthPreview, GoogleAuthPreview, AllStrategiesPreview } from "/snippets/auth-preview";

## Interactive Preview

Experience B3 authentication in action with the actual SDK component:

<AuthPreview />

<Note>
  This is a **live, interactive component** using the real B3 SDK - not an iframe or mockup. Click the button above to authenticate with your preferred provider and see the component in action.
</Note>
...
You can allow users to choose from multiple authentication providers:

<AllStrategiesPreview />
...
function MultipleAuthOptions() {
  return (
    <SignInWithB3
      strategies={["google", "discord", "x", "apple"]}
      chain={b3Chain}
      partnerId="your-partner-id"
      onLoginSuccess={(globalAccount) => {
...
<Note>
  When you don't specify `strategies`, the SignInWithB3 component will display all available authentication options as shown in the preview above.
</Note>
...
For advanced use cases, you can configure session keys with specific permissions using the `RequestPermissionsButton` component:
...
<ParamField path="sessionKeyAddress" type="Address">
  Optional session key address for enhanced permissions.
</ParamField>
...
<ParamField path="onSessionKeySuccess" type="() => void">
