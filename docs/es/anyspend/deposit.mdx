...
Una página de depósito alojada donde los usuarios pueden enviar tokens a una dirección de billetera específica. Conectan su billetera, eligen un token y AnySpend maneja el enrutamiento entre cadenas. Generas una URL, ellos hacen clic.

## Cómo funciona

1. **Tu aplicación genera una URL de depósito** con la dirección del destinatario, cadena y token.
2. **El usuario conecta su billetera** (MetaMask, Coinbase, Rainbow, Rabby, Trust Wallet).
...
La conexión automática está habilitada para usuarios que regresan y que previamente han conectado su billetera.
...
interface PartnerConfig {
  ...
  returnHomeLabel?: string;      // Etiqueta personalizada para el botón de regreso
}
```
...
- **Bots de Telegram** -- financiar billeteras de bots a través de un enlace compartible
