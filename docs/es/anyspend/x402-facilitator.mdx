...
El Facilitador AnySpend x402 es una infraestructura de pagos de grado de producción que potencia pagos multi-token y cross-chain para el protocolo x402. Maneja toda la complejidad de los intercambios de tokens, puentes entre cadenas y liquidación, permitiendo a los desarrolladores aceptar pagos en cualquier token mientras reciben su moneda preferida.
...
- **Pagos en la misma cadena**: Transferencias de tokens directas con mínima sobrecarga
- **Pagos cross-chain**: Puentes automáticos entre redes (por ejemplo, ETH → Base)
...
- No es necesario tener tokens nativos para el gas
...
- **Compatibilidad con el servidor**: Se puede usar con cualquier middleware de x402
...
- Integración de puentes cross-chain
...
Obtener tasa de conversión de tokens para pagos multi-token.

**Propósito:**
- Calcular cantidad exacta de token para el pago
...
```json
{
  ...
  "paymentToken": "0xfde4...",    // Dirección USDT
  "network": "base-mainnet"
}
```
...
```json
{
  "tokenAmount": "1000000",  // 1.0 USDT necesario
  "exchangeRate": "1.0",
  "deadline": 1730000000,
  "route": {
    "srcToken": "0xfde4...",
...
app.use(paymentMiddleware(
  '0xYourAddress...',
  endpointConfig,
  facilitator  // Pre-configurado a mainnet.anyspend.com/x402
));
```
...
- Predeterminado: 5 minutos desde el tiempo de la firma
...
- Los usuarios firman mensajes en el lado del cliente
...
- **Liquidación cross-chain**: 2-5 minutos (depende del puente)
