...
- **EIP-3009 (transferWithAuthorization)** - Usado por USDC, transferencias directas de un paso
...
| **Protección contra Repeticiones** | Nonce secuencial | Seguimiento de nonce aleatorio |
...
Autorización de transferencia directa - la firma autoriza una transferencia inmediata del emisor al receptor sin un paso de aprobación separado.

**Usado por:** USDC en todas las redes (Base, Ethereum, Arbitrum, Optimism, Polygon, etc.)

### Principales Ventajas

- ✅ **Ejecución de un paso** - Transferencia directa, no se necesita aprobación
...
transferWithAuthorization(
    ...
    uint256 value,         // Cantidad a transferir
    uint256 validAfter,    // 0 (válido inmediatamente)
    uint256 validBefore,   // Marca de tiempo de caducidad
...
<Steps>
<Step title="Usuario Firma Autorización">
...
**Usado por:** La mayoría de los tokens ERC-20 modernos con soporte permit
...
permit(
    ...
    uint256 value,         // Cantidad de aprobación
    uint256 deadline,      // Marca de tiempo de caducidad
...
<Steps>
<Step title="Usuario Firma Permit">
El usuario firma un mensaje permit EIP-712 con el nonce secuencial actual
</Step>
...
- Muchos tokens DeFi
- Verificar compatibilidad: [anyspend.com/x402-tokens](https://anyspend.com/x402-tokens)
...
**EIP-3009:**
- Nonce aleatorio previene repeticiones a través de cadenas y contratos
- Cada nonce solo puede usarse una vez por dirección
- Estado de nonce almacenado en cadena en mapping

**EIP-2612:**
- Nonce secuencial previene repeticiones
...
|--------|----------|-----------------|-------------------|
| EIP-3009 | 0 | ~45,000 | 1 |
| EIP-2612 | 0 | ~70,000 (permit) + ~45,000 (transferencia) | 2 |
