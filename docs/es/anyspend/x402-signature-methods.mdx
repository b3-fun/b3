...
- **EIP-3009 (transferWithAuthorization)** - Utilizado por USDC, transferencias directas de un solo paso
- **EIP-2612 (permit)** - Utilizado por la mayoría de los tokens ERC-20 modernos, aprobación en dos pasos + transferencia

## Comparación Rápida

| Característica | EIP-2612 (Permit) | EIP-3009 (transferWithAuthorization) |
|---------|-------------------|--------------------------------------|
| **Utilizado por** | La mayoría de los ERC-20 modernos | USDC (todas las redes) |
...
| **Dependencia** | Debe esperar al nonce anterior | No se requiere orden |
| **Ideal para** | Tokens ERC-20 generales | Pagos con stablecoin |
| **Protección contra repetición** | Nonce secuencial | Seguimiento de nonce aleatorio |
...
**Utilizado por:** USDC en todas las redes (Base, Ethereum, Arbitrum, Optimism, Polygon, etc.)

### Principales Ventajas

- ✅ **Ejecución de un solo paso** - Transferencia directa, no se necesita aprobación
...
transferWithAuthorization(
    ...
    uint256 validBefore,   // Marca de tiempo límite
    bytes32 nonce,         // Nonce aleatorio (previene repetición)
...
El nonce aleatorio se marca como usado, previniendo ataques de repetición
</Step>
</Steps>
...
**Utilizado por:** La mayoría de los tokens ERC-20 modernos con soporte de permit
...
### Verificando el Soporte del Token
...
<Note>
El cliente AnySpend x402 detecta automáticamente qué método de firma utilizar basado en el contrato del token. No necesitas especificar esto manualmente.
</Note>
...
```solidity
// Verificar si el nonce ha sido utilizado
...
**EIP-3009:**
- Nonce aleatorio previene la repetición a través de cadenas y contratos
- Cada nonce solo puede usarse una vez por dirección
- Estado del nonce almacenado en cadena en mapping

**EIP-2612:**
- Nonce secuencial previene la repetición
- Debe usar el nonce actual (autoincrementa)
- Transacciones fallidas bloquean firmas subsiguientes hasta ser firmadas de nuevo
...
<Card title="Soporte de Red" icon="globe" href="/anyspend/x402-network-support">
  Ver qué tokens soportan qué métodos en cada red
</Card>
</CardGroup>
