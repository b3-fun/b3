---
title: Inicio rápido para vendedores
description: >-
...
Esta guía te muestra cómo agregar soporte de pagos **AnySpend x402** a tu API de Express, permitiéndote aceptar pagos en cualquier token mientras recibes el token de tu elección. Con solo unas pocas líneas de código, tu API puede monetizar endpoints y manejar la verificación y liquidación de pagos automáticamente.

**Beneficios clave:**
- Los **compradores** pueden pagar con cualquier token que posean (ETH, DAI, USDC, etc.)
- Los **vendedores** pueden recibir cualquier token que prefieran (USDC, B3, DAI, tokens personalizados, etc.)
- AnySpend maneja automáticamente todas las conversiones de tokens
- No se requiere infraestructura de blockchain ni gestión de billeteras
...
- **Una dirección de billetera** para recibir pagos en USDC
- **Conocimiento básico** del middleware de Express
...
const endpointConfig = {
  '/api/premium-data': {
    ...
    asset: 'USD'           // Se convertirá a USDC
  },
  '/api/ai-inference': {
...
app.use(paymentMiddleware(
  recipientAddress,
  endpointConfig,
  facilitator                    // Facilitador de AnySpend preconfigurado
));
...
2. El middleware devuelve `402 Payment Required` con detalles del pago
3. El cliente paga con su token preferido (ETH, DAI, etc.)
4. El facilitador de AnySpend convierte a USDC y liquida a tu billetera
5. Tu manejador de ruta se ejecuta y devuelve datos

---

### 2. Recibir Tokens Personalizados (p. ej., B3)
...
// Configura para recibir tokens B3
const endpointConfig = {
  'POST /api/premium': {
    price: {
    ...
    network: NETWORK,
    config: {
      description: 'Acceso a datos premium de historial de precios de ETH',
...
app.post('/api/premium', (req, res) => {
  // Pago verificado - ¡recibiste 100 tokens B3!
  res.json({
    data: 'Historial de precios premium de ETH',
...
- Recibes exactamente **100 tokens B3** en tu billetera
- Completa flexibilidad para compradores y vendedores!

## Configuración

### URL del Facilitador

El facilitador de AnySpend se hospeda en:

```
https://mainnet.anyspend.com/x402
```

El paquete `@b3dotfun/anyspend-x402` incluye una instancia del facilitador preconfigurada:

```typescript
import { facilitator } from '@b3dotfun/anyspend-x402';

// Preconfigurado para usar mainnet.anyspend.com/x402
...
#### Método 1: Cantidades Simples en Dólares (Recomendado)
...
const endpointConfig = {
  'POST /api/premium': {
    ...
    network: NETWORK,
    config: {
      description: 'Acceso a datos premium de historial de precios de ETH de CoinGecko',
      mimeType: 'application/json',
    },
  }
};
...
- Se mantiene la flexibilidad tanto para el comprador como para el vendedor
</Note>

### Encontrar Detalles del Token

Para configurar un token personalizado, necesitas conocer sus parámetros EIP-712. Así es como puedes encontrarlos:

**Método 1: Verificar el Contrato del Token**

Usa un explorador de blockchain como [Basescan](https://basescan.org):

```typescript
// Leer del contrato del token
const name = await token.read.name();      // p. ej., "B3"
const version = await token.read.version(); // p. ej., "1"
const decimals = await token.read.decimals(); // p. ej., 18
```

**Método 2: Tokens Comunes**

| Token | Nombre | Versión | Decimales | Dirección Base |
|-------|--------|---------|-----------|----------------|
...
const endpointConfig = {
  '/api/data': {
    amount: '1.00',
    asset: 'USD',
    flexible: true       // Aceptar el encabezado X-PREFERRED-TOKEN
  }
};
```
...
app.use('/api/compute', (req, res, next) => {
  // Calcula el precio basado en unidades de cómputo solicitadas
  ...
  // Adjunta la configuración dinámica a la solicitud
  req.x402Config = {
    amount: totalPrice,
    asset: 'USD'
  };
...
### Configuración de Facilitador Personalizada
...
Ruta de pagos a diferentes billeteras basadas en el endpoint:
...
// Pasa null como destinatario, usa destinatarios por endpoint
...
```typescript
// Ruta pública - no se requiere pago
app.get('/api/public', (req, res) => {
  res.json({ message: '¡Esto es gratis!' });
});

// Ruta protegida - se requiere pago
...
app.get('/api/premium', (req, res) => {
  res.json({ message: '¡Esto costó dinero!' });
});
```
