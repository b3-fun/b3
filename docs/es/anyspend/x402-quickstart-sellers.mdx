...
description: >-
  Acepta pagos en cualquier token para tus APIs y servicios utilizando el
...
const endpointConfig = {
  'POST /api/premium': {
    ...
    network: NETWORK,
    config: {
      description: 'Acceso a datos de precios ETH premium',
...
app.post('/api/premium', (req, res) => {
  ...
  res.json({
    data: 'Historial de precios ETH premium',
...
El paquete `@b3dotfun/anyspend-x402` incluye una instancia de facilitador preconfigurada:
...
const endpointConfig = {
  '/api/data': {
    amount: '1.00',      // $1.00 valor de cualquier token
    asset: 'USD'
  },
  '/api/compute': {
    amount: '2.50',
...
const endpointConfig = {
  'POST /api/premium': {
    ...
    network: NETWORK,
    config: {
      description: 'Acceso a datos de precios ETH premium de CoinGecko',
      mimeType: 'application/json',
    },
  }
};
...
Para configurar un token personalizado, necesitas conocer sus parámetros EIP-712. Aquí te mostramos cómo encontrarlos:
...
#### Método 3: Flexible (Permitir que el Cliente Elija)
...
app.use('/api/compute', (req, res, next) => {
  ...
  // Adjunta configuración dinámica a la solicitud
  req.x402Config = {
    amount: totalPrice,
    asset: 'USD'
  };
...
const customFacilitator = new Facilitator({
  ...
  timeout: 30000,                             // Tiempo de espera de 30s
...
Ruta de pagos a diferentes billeteras basadas en endpoint:
