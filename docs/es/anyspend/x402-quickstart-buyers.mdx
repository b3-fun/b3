...
Para un control más detallado, usa la función de bajo nivel `wrapFetchWithPayment`:
...
// Envuelve fetch con manejo automático de pago
...
### Uso de encabezados HTTP para preferencias de pago
...
Estos encabezados funcionan con cualquier cliente HTTP y son respetados automáticamente por los servidores de recursos habilitados para X402 que usan el middleware AnySpend. Si no se proporcionan encabezados, el servidor usará USDC por defecto.

#### Uso de encabezados con X402Client
  ...
  const fetchPremiumData = async () => {
    ...
    try {
      ...
      const fetchWithPayment = wrapFetchWithPayment(
        fetch,
        walletClient as any,
        '1000000', // Máximo 1 USDC
        undefined, // Usa el selector de requis
