...
Esta guía te muestra cómo usar el **AnySpend x402 Client SDK** para pagar recursos con muro de pago usando tokens compatibles con EIP-2612 o EIP-3009. El SDK maneja todo el flujo de pago automáticamente, incluida la generación de firmas y la lógica de reintento.
...
- **Una cartera criptográfica** con tokens compatibles (USDC, DAI u otros tokens EIP-2612/EIP-3009)
...
<Note>
**Importante**: Solo los tokens con soporte para EIP-2612 (Permit) o EIP-3009 (TransferWithAuthorization) son compatibles con AnySpend x402 para pagos sin gas. Usa el [Comprobador de Compatibilidad de Tokens](https://anyspend.com/x402-tokens) para verificar que tu token esté soportado.
</Note>
...
2. La API devuelve `402 Payment Required` con detalles del pago
...
<Note>
**Nuevo en X402**: Ahora puedes especificar preferencias de pago usando encabezados HTTP, lo cual es particularmente útil al realizar solicitudes HTTP directas o al integrarse con clientes HTTP existentes.
</Note>
...
Estos encabezados funcionan con cualquier cliente HTTP y son automáticamente respetados por los servidores de recursos habilitados para X402 que usan el middleware AnySpend. Si no se proporcionan encabezados, el servidor por defecto usará USDC.

#### Usando Encabezados con X402Client

El X402Client añade automáticamente estos encabezados basados en tu configuración:
...
// Los encabezados se añaden automáticamente a todas las solicitudes
const response = await x402Client.request('https://api.example.com/data');
```

#### Usando Encabezados con fetch

También puedes añadir estos encabezados manualmente al usar la API nativa `fetch` u otros clientes HTTP:
...
Para casos de uso avanzados, `wrapFetchWithPayment` proporciona control de nivel inferior:
...
function wrapFetchWithPayment(
  ...
  maxPaymentValue: string,             // Máximo pago en la unidad más pequeña (p. ej., '1000000' para 1 USDC)
  ...
  config?: {
    ...
    maxRetries?: number;               // Máximos intentos de reintento de pago
  },
  paymentPreferences?: {
    preferredToken?: string;           // Dirección del token con el que pagar
    preferredNetwork?: string;         // Identificador de red (p. ej., 'base', 'ethereum')
  }
): typeof fetch
```
...
**Solo los tokens con soporte para EIP-2612 o EIP-3009 son compatibles** con AnySpend x402 para pagos sin gas.
...
<Card title="Comprobador de Compatibilidad de Tokens" icon="check-circle" href="https://anyspend.com/x402-tokens">
  Explora todos los tokens compatibles con soporte para EIP-2612 o EIP-3009 en más de 19 redes
</Card>

Consulta [Soporte de Red](/anyspend/x402-network-support) para ver direcciones de tokens detalladas por cadena.
...
// Ayudante para obtener el nombre de la red a partir del ID de cadena
