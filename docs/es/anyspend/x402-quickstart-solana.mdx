...
<CardGroup cols={3}>
<Card title="Repositorio GitHub" icon="github" href="https://github.com/b3-fun/anyspend-x402">
...
- **Billetera Solana** con algo de USDC (para pruebas)
...
// Tu dirección de billetera Solana
...
app.listen(3001, () => {
  console.log('Servidor Solana X402 corriendo en el puerto 3001');
});
```

**Qué hace esto:**
1. El cliente solicita el endpoint
...
### Opción 1: Usando Fetch (Agentes de IA & Backend)
...
const privateKey = process.env.SOLANA_PRIVATE_KEY; // Clave privada Base58
const url = "https://api.example.com/api/solana/premium";

async function main() {
  // Crear firmante Solana desde la clave privada
  const signer = await createSigner("solana", privateKey);

  // Envolver fetch con soporte de pago
...
**Puntos Clave:**
- Usa la clave privada codificada en base58 (de un par de claves Solana)
...
function PremiumDataFetcher() {
  ...
  const fetchPremiumData = async () => {
    ...
    setLoading(true);
    try {
      // Crear firmante desde el adaptador de billetera
...
#### Configuración del Envoltorio de la Aplicación
...
3. Configura la red a `solana-devnet` en tu configuración

### Probar Tu Integración

```typescript
// Servidor - usar devnet
network: 'solana-devnet'

// Cliente - usar RPC de devnet
...
<CardGroup cols={2}>
<Card title="Resumen de X402" icon="book" href="/anyspend/x402-overview">
...
<Card title="Métodos de Firma" icon="signature" href="/anyspend/x402-signature-methods">
  Profundizar en las firmas de transacciones de Solana
</Card>
...
Verifica que:
- Tu billetera tenga suficiente USDC para el pago
- Estés en la red correcta (mainnet vs devnet)
- La URL del facilitador sea correcta
</Accordion>

<Accordion title="El adaptador de billetera no funciona">
Asegúrate de:
- Haber envuelto tu aplicación con `WalletProvider`
- La extensión de la billetera esté instalada (Phantom, Solflare, etc.)
- Estés usando el endpoint RPC correcto
</Accordion>

<Accordion title="El pago se realizó pero no se devolvieron datos">
Verifica:
- El endpoint del servidor esté configurado correctamente
- La cantidad de pago coincida con las expectativas del servidor
