...
<Card title="Demo en Vivo" icon="rocket" href="https://github.com/b3-fun/anyspend-x402-demo">
...
Esta guía te muestra cómo usar **AnySpend X402** en **Solana** tanto para aplicaciones de servidor (aceptando pagos) como para clientes (realizando pagos). AnySpend X402 en Solana permite pagos de tokens SPL sin gas usando firmas de transacciones, lo que lo hace perfecto para APIs con muro de pago, agentes de IA y servicios premium.
  ...
  paymentMiddleware(
    SOLANA_PAYTO_ADDRESS,
    {
      "POST /api/solana/premium": {
        ...
        network: 'solana',
        config: {
          description: 'Acceso a datos premium mediante pago en USDC',
...
2. El middleware devuelve `402 Payment Required` con detalles de pago de Solana
...
Hay dos formas de realizar pagos en Solana dependiendo de tu caso de uso.
...
async function main() {
  // Crea un firmante de Solana desde la clave privada
...
function PremiumDataFetcher() {
  ...
  const fetchPremiumData = async () => {
    ...
    setLoading(true);
    try {
      // Crea un firmante desde el adaptador de billetera
...
<Card title="Soporte de Red" icon="globe" href="/anyspend/x402-network-support">
  Ver todas las cadenas y tokens compatibles
</Card>
...
<Card title="Guía para Compradores" icon="wallet" href="/anyspend/x402-quickstart-buyers">
  Guía de configuración del cliente EVM
</Card>
</CardGroup>
...
<Accordion title="El pago se realizó con éxito pero no se devolvieron datos">
Verifica:
- El punto final del servidor está configurado correctamente
- La cantidad de pago coincide con las expectativas del servidor
...
- **Demo en Vivo**: Pruébalo tú mismo en [x402-demo.anyspend.com](https://x402-demo.anyspend.com/)
