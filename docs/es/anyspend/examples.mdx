...
---
Ejemplos de copiar y pegar para los patrones de integración más comunes. Ve todos estos funcionando en vivo en la [demostración del SDK (Widget Lab)](https://sdk-demo.anyspend.com/).
...
function FullCheckout({ cart }) {
  return (
    <AnySpendCheckout
      ...
      // Recopilar información del cliente mediante esquema JSON
      formSchema={{
        fields: [
...
function SubscriptionCheckout({ plan }) {
  return (
    <AnySpendCheckoutTrigger
      ...
      // Activar flujo de trabajo con el pago exitoso
...
### Depósito a contrato personalizado (staking)
  ...
  return (
    <div>
      ...
      <AnySpendCustom
        ...
        onSuccess={() => toast.success("¡Stakeado exitosamente!")}
      />
    </div>
  );
}
