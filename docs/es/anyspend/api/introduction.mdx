...
description: >-
  Acceso programático a AnySpend - crea enlaces de pago, gestiona productos,
  rastrea transacciones y más
lang: es
originalPath: anyspend/api/introduction.mdx
---
La API de la plataforma AnySpend te brinda control programático completo sobre tu infraestructura de pagos. Construye integraciones, automatiza flujos de trabajo y deja que agentes de IA gestionen tu pila de comercio electrónico -- todo a través de una limpia API REST al estilo de Stripe.

## ¿Para quién es esto?

- **Desarrolladores** construyendo integraciones personalizadas con AnySpend
- **Agentes de IA** que crean enlaces de pago, gestionan productos o extraen análisis
- **Plataformas de automatización** orquestando flujos de trabajo de comercio (Zapier, n8n, scripts personalizados)
...
### Patrón 2: Sesiones del lado del servidor (empresarial)

Crea una sesión de pago en tu backend con el contexto completo del cliente:
...
Todas las solicitudes de API se realizan a:
...
La API utiliza tres niveles de autenticación dependiendo del endpoint:

| Nivel | Autenticación requerida | Límite de tasa | Caso de uso |
|------|--------------|------------|----------|
| **Abierto** | Ninguna (limitación de tasa basada en IP) | 5 solicitudes/min por IP | Pago rápido -- pagos de una sola vez sin necesidad de cuenta |
| **Clave API** | clave `asp_xxx` vía cabecera | 120 solicitudes/min por clave | Todas las operaciones estándar de la API -- CRUD en enlaces de pago, productos, transacciones, etc. |
| **JWT** | token de sesión del panel de control | Límites de sesión estándar | Rutas solo del panel de control (gestionadas automáticamente por la aplicación web) |

La mayoría de los desarrolladores utilizarán la autenticación de Clave API. Consulta la página de [Autenticación](/anyspend/api/authentication) para detalles sobre la creación y gestión de claves.
...
```json
{
  ...
  "data": [
    ...
    { "object": "payment_link", "id": "pl_def456...", "name": "Básico" }
  ],
  "has_more": true,
  "total_count": 142,
...
Los límites de tasa varían según el nivel de autenticación. Cuando excedes el límite, la API devuelve un estado `429` con un `error de límite de tasa`:

| Nivel | Límite | Ventana |
|------|-------|--------|
| Abierto (Pago rápido) | 5 solicitudes | 1 minuto |
| Clave API | 120 solicitudes | 1 minuto |
...
<Card title="Autenticación" icon="key" href="/anyspend/api/authentication">
  Crea y gestiona claves API, comprende los niveles de permiso y sigue las mejores prácticas de seguridad.
</Card>

<Card title="Enlaces de Pago" icon="link" href="/anyspend/api/payment-links">
  Crea, actualiza y gestiona enlaces de pago con artículos, estadísticas, sesiones y seguimiento de visitantes.
</Card>
...
<Card title="Sesiones de Pago" icon="credit-card" href="/anyspend/api/checkout-sessions-api">
  Crea y rastrea sesiones de pago, monitorea el estado del pago y maneja las completaciones.
</Card>
...
<Card title="Webhooks" icon="bell" href="/anyspend/api/webhooks-api">
  Registra endpoints de webhook para recibir notificaciones en tiempo real de eventos de pago.
</Card>
...
<Step title="Obtén tu clave API">
  Ve a tu [Panel de Control de AnySpend](https://anyspend.com/dashboard) bajo **Configuración > Claves API** y crea una clave con los permisos que necesitas.
</Step>
...
<Step title="Configura webhooks">
  Registra un endpoint de webhook para recibir notificaciones en tiempo real cuando los pagos se completen.
</Step>
</Steps>
