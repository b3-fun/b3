...
---
La plataforma AnySpend SDK client es un cliente TypeScript **sin interfaz** (sin dependencia de React) para la API completa de la plataforma AnySpend. Funciona en todos los entornos donde se ejecuta JavaScript moderno: Node.js, navegadores, Cloudflare Workers, Deno y Bun.
...
Para mayor comodidad, cada método `list` tiene un ayudante `listAutoPaginate` correspondiente que devuelve un iterador asíncrono. Este obtiene las páginas en segundo plano a medida que consumes los elementos.
...
<Tip>
  La auto-paginación respeta automáticamente los límites de tasa. Si la API devuelve un `429`, el iterador espera la duración de `Retry-After` antes de obtener la siguiente página.
</Tip>
...
try {
  ...
  if (err instanceof NotFoundError) {
    ...
    console.log(`Límite de tasa alcanzado. Reintentar después de ${err.retryAfter} segundos`);
    ...
    // Atrapar cualquier otro error de API
...
El cliente reintenta automáticamente las solicitudes fallidas en los siguientes casos:

- **Errores de servidor 5xx** -- el servidor tuvo un fallo transitorio
...
Los reintentos utilizan **backoff exponencial** con jitter. El predeterminado es 3 reintentos, configurable a través de la opción `maxRetries`.
...
Todas las solicitudes `POST` y `PATCH` incluyen automáticamente un encabezado `Idempotency-Key`. Esto asegura que si una solicitud se reintenta (debido a un error de red, por ejemplo), el servidor no la procesará dos veces.

El generador predeterminado utiliza `crypto.randomUUID()`. Puedes suministrar el tuyo propio:
...
| Entorno de ejecución | Compatible |
|---------|-----------|
| Node.js 18+ | Sí |
