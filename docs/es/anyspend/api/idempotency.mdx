...
La API detecta la clave duplicada, verifica que el hash del cuerpo coincida y devuelve inmediatamente la respuesta almacenada en caché con un encabezado `Idempotent-Replayed: true`. No se crea un nuevo recurso.
</Step>
...
Las claves de idempotencia se almacenan en caché durante **24 horas** desde la primera solicitud. Después de 24 horas, la clave expira y se puede reutilizar.

<Info>
  Solo las respuestas exitosas (HTTP 2xx) se almacenan en caché. Si la solicitud original falló con un error 4xx o 5xx, la clave no se consume y puedes volver a intentarlo con la misma clave.
</Info>
...
Si vuelves a intentar la misma solicitud exacta, obtienes la respuesta almacenada en caché con el encabezado de repetición:
...
async function createPaymentLinkSafe(data: PaymentLinkInput) {
  ...
  // Si se agotó el tiempo de espera o hubo un error de red, reintenta de manera segura
  ...
  if (response.headers.get("Idempotent-Replayed") === "true") {
    console.log("La respuesta fue repetida desde la caché (solicitud duplicada).");
  }
    ...
    for attempt in range(3):
        try:
            ...
            if response.ok:
                ...
                if replayed:
                    print("Respuesta repetida desde la caché.")
                return response.json()

            if response.status_code < 500:
                # Error del cliente -- no reintentar
                raise Exception(f"Error de la API: {response.json()}")

        except requests.exceptions.Timeout:
            print(f"Intento {attempt + 1} agotó el tiempo de espera, reintentando...")
...
Este es un mecanismo de seguridad. Previene errores donde la misma clave se asocia accidentalmente con dos operaciones diferentes.

## ¿Qué métodos soportan idempotencia?
...
Las claves deterministas son útiles cuando quieres que la misma operación lógica siempre use la misma clave -- por ejemplo, asegurando que un usuario solo pueda crear un enlace de pago para un producto específico.
</Tab>

<Tab title="Hash de la solicitud">
```typescript
...
Hashear la solicitud asegura que solicitudes idénticas siempre produzcan la misma clave. Útil para sistemas basados en colas donde el mismo mensaje podría procesarse varias veces.
</Tab>
</Tabs>

## Alcance de las claves de idempotencia

Las claves de idempotencia están limitadas a tu **organización**. Dos organizaciones diferentes pueden usar la misma cadena de clave sin conflicto. Dentro de tu organización, cada clave solo se puede usar una vez por ventana de 24 horas.
...
if (error.code === "idempotency_conflict") {
  // Genera una clave nueva y reintenta
...
Cuando el encabezado `Idempotent-Replayed: true` está presente, la respuesta es una repetición almacenada en caché. Esto puede ser útil para el registro y depuración para distinguir entre respuestas frescas y repetidas.
</Accordion>

<Accordion title="Usa claves deterministas para semántica de exactamente-una-vez">
