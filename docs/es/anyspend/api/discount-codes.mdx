...
---
Los códigos de descuento te permiten ofrecer descuentos basados en porcentaje o en una cantidad fija en enlaces de pago. Crea códigos individuales, genera lotes en masa y valida los códigos en el momento del pago.
...
<ParamField path="min_order_amount" type="string">
  Cantidad mínima de pedido requerida para usar este código, en la unidad más pequeña del token.
</ParamField>
...
<ParamField path="POST /discount-codes/validate" type="read">
  Validar un código de descuento contra un enlace de pago y una cantidad de pedido. Devuelve los detalles del descuento si es válido, o un mensaje de error explicando por qué el código es inválido.
</ParamField>
...
<ParamField path="amount" type="string" required>
  La cantidad del pedido en la unidad más pequeña del token, utilizada para verificar `min_order_amount` y calcular el total con descuento.
</ParamField>
...
### Crear Lotes de Códigos de Descuento
...
<ParamField path="codes" type="string[]" required>
  Array de cadenas de código para crear. Cada uno debe ser único.
</ParamField>
...
<ParamField path="value" type="number" required>
  Valor del descuento aplicado a todos los códigos.
</ParamField>
...
<ParamField path="min_order_amount" type="string">
  Cantidad mínima de pedido para todos los códigos.
</ParamField>
...
| `max_uses` | `number \| null` | Máximas redenciones, o `null` para ilimitadas |
| `current_uses` | `number` | Número de veces canjeado hasta ahora |
| `min_order_amount` | `string \| null` | Cantidad mínima de pedido requerida |
...
Un código de descuento se considera **inválido** si se cumple alguna de las siguientes condiciones:
...
| La cantidad del pedido está por debajo de `min_order_amount` | `La cantidad del pedido está por debajo del mínimo requerido` |
