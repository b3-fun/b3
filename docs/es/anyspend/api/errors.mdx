...
|-------|------|-------------|
| `type` | `string` | La categoría del error. Úsalo para la lógica de manejo de errores amplia. |
| `code` | `string` | Un código de error legible por máquina. Úsalo para el manejo de errores específicos. |
| `message` | `string` | Una descripción legible por humanos de qué salió mal. Seguro para mostrar a los desarrolladores (pero no a los usuarios finales). |
| `param` | `string` o ausente | El parámetro de la solicitud que causó el error, si corresponde. |

## Tipos de error

Los tipos de error agrupan errores en categorías amplias. Usa estos para el manejo de errores de nivel superior.
...
| `authentication_error` | `401` | La clave API falta, es inválida, ha expirado o fue revocada. |
...
Los códigos de error proporcionan identificadores específicos y legibles por máquina para cada condición de error.

### Errores de validación de solicitud

| Código | Tipo | HTTP | Descripción |
|------|------|------|-------------|
| `missing_required_field` | `invalid_request_error` | `400` | Un campo requerido no se incluyó en el cuerpo de la solicitud. El campo `param` indica qué campo falta. |
...
| `duplicate_resource` | `invalid_request_error` | `409` | Un recurso con un campo único en conflicto ya existe. |
...
| `insufficient_permissions` | `permission_error` | `403` | La clave API no tiene el nivel de permiso requerido para esta operación (por ejemplo, una clave `read` intentando un `POST`). |
...
| Código | Tipo | HTTP | Descripción |
|------|------|------|-------------|
| `internal_error` | `api_error` | `500` | Un error de servidor inesperado. Estos se rastrean automáticamente. Si persiste, contacta al soporte. |
...
```json
{
  "error": {
    ...
    "message": "No se encontró ningún payment_link con id 'pl_nonexistent'."
  }
}
```
...
```json
{
  "error": {
    ...
    "message": "Una clave de idempotencia se usó con un cuerpo de solicitud diferente."
  }
}
```
...
<AccordionGroup>
<Accordion title="Siempre revisa el tipo de error primero">
Usa `error.type` para el control de flujo amplio (por ejemplo, reintenta en `rate_limit_error`, falla rápido en `authentication_error`). Usa `error.code` para el manejo específico dentro de un tipo.
</Accordion>
...
Errores del cliente como `400`, `401`, `403`, y `404` no se resolverán al reintentar. Corrige la solicitud o las credenciales antes de reintentar.
</Accordion>

<Accordion title="Registra el objeto de error completo para depuración">
Siempre registra `error.type`, `error.code`, `error.message`, y `error.param` juntos. Esto te da la imagen completa al depurar problemas de producción.
</Accordion>
</AccordionGroup>
