...
---
La plataforma AnySpend API utiliza claves API para la autenticación. Cada clave está limitada a una sola organización y lleva un conjunto específico de permisos.
...
El prefijo facilita la identificación de claves AnySpend en tu base de código y previene el uso accidental de claves de otros servicios.
...
Cada clave API se asigna uno o más permisos. Los permisos siguen una jerarquía donde los niveles superiores incluyen todas las capacidades de los niveles inferiores.
...
| **admin** | Todo en `write` + gestionar claves API, configuraciones de la organización y facturación | Gestión de infraestructura, CI/CD, herramientas de administración |
...
Las claves revocadas dejan de funcionar inmediatamente. Cualquier solicitud en curso autenticada con la clave revocada recibirá un error `401`.

## Quick Pay -- nivel abierto (sin autenticación)

El endpoint de Quick Pay no requiere autenticación. Está diseñado para pagos únicos donde no necesitas una cuenta de AnySpend:
...
<Step title="Verifica que la nueva clave funciona">
  Confirma que tu aplicación está realizando solicitudes con éxito con la nueva clave revisando el timestamp de `last_used_at`.
</Step>
...
<Tip>
  Utiliza el campo opcional `expires_at` al crear claves para hacer cumplir la expiración automática. Esto añade una red de seguridad en caso de que olvides rotar.
</Tip>
...
Guarda tu clave API en una variable de entorno en lugar de codificarla directamente:
...
Añade `.env` a tu `.gitignore` para evitar commits accidentales.
</Accordion>
...
- Solo las herramientas de infraestructura deben usar **admin**
</Accordion>
...
Para integraciones temporales, tokens de CI/CD o acceso de contratistas, siempre establece un timestamp de `expires_at` para que las claves se vuelvan inactivas automáticamente.
</Accordion>
</AccordionGroup>
...
| `401` | `key_expired` | La clave API ha pasado su timestamp de `expires_at` |
