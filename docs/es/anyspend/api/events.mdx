...
<ParamField path="GET /events" type="read">
  Lista eventos de la API con filtrado opcional. Los resultados se devuelven en orden cronológico inverso (el más nuevo primero).
</ParamField>

<ParamField path="type" type="string" query>
  Filtrar por tipo de evento (por ejemplo, `payment_link.created`, `webhook.updated`, `discount_code.deleted`).
</ParamField>

<ParamField path="resource_type" type="string" query>
  Filtrar por tipo de recurso (por ejemplo, `payment_link`, `webhook`, `discount_code`, `widget`, `notification`).
</ParamField>

<ParamField path="from" type="string" query>
  Marca de tiempo ISO 8601. Solo devuelve eventos creados en o después de este tiempo.
</ParamField>

<ParamField path="to" type="string" query>
  Marca de tiempo ISO 8601. Solo devuelve eventos creados en o antes de este tiempo.
</ParamField>
...
|-------|------|-------------|
| `id` | `string` | Identificador único del evento (por ejemplo, `evt_abc123`) |
...
| `request_path` | `string` | Ruta de la API que fue llamada |
| `status_code` | `number` | Código de estado HTTP devuelto |
| `created_at` | `string` | Marca de tiempo ISO 8601 cuando se registró el evento |
...
| `webhook.created` | Se creó un endpoint de webhook |
| `webhook.updated` | Se actualizó un endpoint de webhook |
| `webhook.deleted` | Se eliminó un endpoint de webhook |
...
| `webhook` | Configuraciones de endpoints de webhook |
...
### Todos los eventos relacionados con webhooks
...
<Note>
  Los eventos se retienen durante 90 días. Para una retención más prolongada, exporta eventos periódicamente utilizando los filtros de rango de fechas.
</Note>
