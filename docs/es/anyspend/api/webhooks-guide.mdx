...
<Tabs>
  <Tab title="SDK">
    ...
    console.log("ID del Webhook:", webhook.id);
  ...
  <Tab title="Dashboard">
    Navega a **Configuración > Webhooks > Añadir Endpoint** en el [Dashboard de AnySpend](https://app.anyspend.com). Introduce tu URL, selecciona los eventos y haz clic en **Crear**. El secreto de firma se mostrará una vez -- cópialo inmediatamente.
  </Tab>
</Tabs>
...
El webhook `payment.completed` siempre incluye un bloque `checkoutSession` que contiene `clientReferenceId`, `metadata`, `customerEmail` y `customerName` -- todo lo que necesitas para hacer coincidir los pagos con tus registros internos.

### Usando client_reference_id

Pasa tu ID de pedido o usuario al crear el checkout (a través de parámetro de URL o API):

```javascript
function handlePaymentCompleted(data) {
  const { clientReferenceId, metadata } = data.checkoutSession;

  // Busca tu pedido interno
...
function handlePaymentCompleted(data) {
  ...
  // metadata contiene lo que pasaste a través de URL o API
...
app.post(
  ...
  (req, res) => {
    ...
    // La firma es válida -- parsear y procesar el evento
    ...
    // Siempre responder con 200 para acusar recibo
...
function handlePaymentCompleted(data) {
  ...
  // Cumplir pedido, enviar correo de confirmación, actualizar base de datos, etc.
}

function handlePaymentFailed(data) {
  console.log(`Pago ${data.id} fallido`);
  // Notificar al cliente, actualizar estado del pedido, etc.
}
...
<Info>
  Si tu endpoint falla consistentemente (10+ entregas fallidas consecutivas), el webhook será automáticamente **deshabilitado** y recibirás una notificación por correo electrónico. Reactívalo desde el Dashboard después de solucionar el problema.
</Info>
...
Usa el endpoint de prueba para enviar un evento sintético a tu URL de webhook. Esto ayuda a verificar que tu endpoint es accesible y que tu lógica de verificación de firma es correcta.
...
El evento de prueba utiliza el tipo de evento `payment.completed` con datos ficticios. Tu manejador debería procesarlo como cualquier otro evento, pero puedes verificar el encabezado `X-AnySpend-Test: true` si quieres omitir efectos secundarios durante las pruebas.

## Mejores Prácticas

<CardGroup cols={2}>
  <Card title="Responder rápidamente" icon="bolt">
    Devuelve un `200` inmediatamente y procesa el evento de forma asincrónica (p. ej., en una cola de trabajos en segundo plano). Las entregas de webhook tienen un tiempo de espera después de 30 segundos.
  </Card>
