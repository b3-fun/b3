...
title: Webhooks
description: >-
  Configura puntos de conexión de webhook para notificaciones de eventos en
  tiempo real
lang: es
originalPath: anyspend/api/webhooks-api.mdx
---
Los webhooks te permiten recibir notificaciones HTTP POST en tiempo real cuando ocurren eventos en tu cuenta de AnySpend. Configura endpoints, elige a qué eventos suscribirte y monitorea el historial de entregas.
...
<ParamField path="POST /webhooks" type="admin">
  Crea un nuevo endpoint de webhook. La respuesta incluye un campo `secret` utilizado para verificar las firmas de los webhooks. Almacena esto de forma segura -- solo se devuelve una vez en el momento de la creación.
</ParamField>

<ParamField path="url" type="string" required>
  La URL HTTPS que recibirá las solicitudes POST del webhook.
</ParamField>
...
<Warning>
  El campo `secret` solo se incluye en la respuesta de creación. Cópialo inmediatamente y guárdalo en tus variables de entorno o gestor de secretos.
</Warning>
...
<ParamField path="POST /webhooks/:id/test" type="admin">
  Envía un evento de prueba al URL del webhook para que puedas verificar que tu endpoint está recibiendo y procesando eventos correctamente. La carga de prueba utiliza un tipo de evento `test.ping`.
</ParamField>
...
<Tabs>
  ...
  <Tab title="SDK">
    ...
    // result.delivery_id se puede utilizar para verificar el estado de la entrega
    ```
  </Tab>
</Tabs>
...
|-------|------|-------------|
| `id` | `string` | Identificador único del webhook (p. ej., `wh_abc123`) |
| `url` | `string` | URL del endpoint HTTPS |
| `events` | `string[]` | Tipos de eventos suscritos |
| `secret` | `string` | Secreto de firma (solo se devuelve al crear) |
...
|----------------|-------------|
| `payment.completed` | Un pago fue procesado exitosamente |
| `payment.failed` | Un intento de pago falló |
| `checkout.completed` | Una sesión de checkout se completó (incluye datos de formulario, envío, información de descuento) |

## Verificando Firmas de Webhook

Cada solicitud de webhook incluye una firma en el encabezado `X-AnySpend-Signature`. Verifícala usando tu secreto de webhook para asegurar que la solicitud sea auténtica.
