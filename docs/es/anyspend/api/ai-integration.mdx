...
---
La plataforma AnySpend API está diseñada con la **integración AI-first** en mente. Cada punto final tiene un `operationId` único, esquemas JSON detallados y convenciones consistentes que la hacen sencilla de usar con llamadas de función LLM, agentes autónomos y marcos de orquestación de IA.
...
- Un `operationId` único para cada punto final (por ejemplo, `createPaymentLink`, `listTransactions`)
- Esquemas JSON detallados de solicitud y respuesta con descripciones
- Requisitos de autenticación
- Esquemas de respuesta de error

<Tip>
  Usa la especificación OpenAPI para generar automáticamente definiciones de herramientas para tu LLM. La mayoría de los marcos de IA pueden convertir operaciones OpenAPI en definiciones de función/herramienta de forma programática.
</Tip>

## Llamadas de Función LLM
...
```json
{
  ...
  "description": "Crear un nuevo enlace de pago que se puede compartir con los clientes para recoger pagos en cripto. Devuelve el objeto del enlace de pago incluyendo la URL compartible.",
  "input_schema": {
    "type": "object",
    "properties": {
      "name": {
      ...
      "recipient_address": {
        "type": "string",
        "description": "Dirección Ethereum (prefijo 0x, 42 caracteres) que recibirá el pago"
      },
      "token_address": {
        "type": "string",
...
La definición de función equivalente para la llamada de función de OpenAI:
...
Aquí está cómo un asistente de IA podría usar las herramientas de AnySpend en una conversación:

<Steps>
  <Step title="El usuario pide crear un enlace de pago">
    **Usuario:** "Crear un enlace de pago por 50 USDC para la suscripción al plan Pro"
  ...
  <Step title="La IA responde al usuario">
    "He creado un enlace de pago por 50 USDC para la Suscripción al Plan Pro. Aquí está el enlace: https://anyspend.com/pay/pl_abc123"
  </Step>
</Steps>

## Construyendo un Agente de Pago de IA
...
const tools: Anthropic.Tool[] = [
  {
    name: "create_payment_link",
    description: "Crear un enlace de pago para recoger pagos en cripto",
    input_schema: {
  ...
  },
  {
    ...
    description: "Listar transacciones recientes con filtro de estado opcional",
    input_schema: {
      type: "object" as const,
      properties: {
  ...
  },
  {
    ...
    description: "Obtener estadísticas agregadas de transacciones",
    input_schema: {
      type: "object" as const,
      properties: {},
    },
...
// Validar campos de cadena requeridos de la entrada de herramienta LLM
...
// Ejecutar una llamada de herramienta
...
const SYSTEM_PROMPT =
  "Eres un asistente de gestión de pagos. Usa las herramientas proporcionadas para ayudar a los usuarios a crear enlaces de pago, ver transacciones y gestionar su cuenta de AnySpend. Los montos están en la unidad más pequeña del token -- para USDC con 6 decimales, 1 USDC = 1000000.";
...
async function chat(userMessage: string) {
  ...
  // Manejar el uso de herramientas en un bucle
...
// Uso
const reply = await chat("Crear un enlace de pago de 25 USDC para un pedido de camiseta");
console.log(reply);
```
...
<AccordionGroup>
  <Accordion title="list_payment_links">
    ```json
    {
      ...
      "description": "Listar todos los enlaces de pago, opcionalmente filtrados por estado",
      "input_schema": {
        "type": "object",
        "properties": {
          "status": {
  ...
  <Accordion title="get_analytics_overview">
    ```json
    {
      ...
      "description": "Obtener una visión general de análisis de alto nivel incluyendo el volumen total, conteo de transacciones y los enlaces de pago principales",
      "input_schema": {
        "type": "object",
        "properties": {
          "from": {
  ...
  <Accordion title="create_discount_code">
    ```json
    {
      ...
      "description": "Crear un código de descuento que los clientes pueden aplicar en el pago",
      "input_schema": {
        "type": "object",
        "properties": {
          "code": {
          ...
          "value": {
            "type": "string",
            "description": "Valor del descuento: porcentaje (por ejemplo, '20' para 20%) o cantidad fija en la unidad más pequeña"
          },
          "max_uses": {
            "type": "number",
  ...
  <Accordion title="export_transactions">
    ```json
    {
      ...
      "description": "Exportar transacciones como CSV o JSON para informes y análisis",
      "input_schema": {
        "type": "object",
        "properties": {
          "format": {
...
<CardGroup cols={2}>
  <Card title="Usar llaves de solo lectura para agentes de consulta" icon="eye">
    Los agentes de IA que solo leen datos (listar transacciones, ver análisis) deben usar llaves API con permiso de **solo lectura**. Esto previene mutaciones accidentales por llamadas de herramientas alucinadas.
  </Card>
  <Card title="Usar llaves de escritura para agentes de acción" icon="pen">
    Los agentes que necesitan crear enlaces de pago o gestionar recursos necesitan llaves API de **lectura-escritura**. Limítalas a los permisos mínimos requeridos.
  </Card>
  <Card title="Validar siempre los montos" icon="calculator">
    Los LLM pueden alucinar números. Antes de ejecutar una llamada de herramienta que involucre un monto, valida que el monto sea razonable. Considera agregar un paso de confirmación para montos por encima de un umbral.
  </Card>
  <Card title="Registrar todas las llamadas de herramientas" icon="file-lines">
    Registra cada llamada de herramienta y su resultado para auditabilidad. Esto es especialmente importante para agentes que manejan dinero real.
  </Card>
</CardGroup>

<Warning>
  **Nunca expongas tu llave API en código del lado del cliente o en prompts de IA que sean visibles para los usuarios finales.** Usa un proxy del lado del servidor para hacer llamadas API en nombre de tu agente de IA. Si usas llamadas de función de Claude o OpenAI, la llave API solo debe existir en tu capa de ejecución de herramientas del backend -- nunca en el prompt del sistema LLM.
</Warning>
...
Este enfoque mantiene tus definiciones de herramientas sincroniz
