...
---
Las sesiones de pago permiten crear un objeto de seguimiento en tu servidor antes de que el usuario elija un método de pago. La sesión es un registro de base de datos ligero -- sin llamadas a API externas, sin creación de pedidos. El pedido llega más tarde, cuando el usuario se compromete a pagar.
...
<Steps>
  ...
  <Step title="El usuario paga">
    **Cripto:** enviar a `globalAddress` | **Pasarela de pago fiat:** redirigir a `oneClickBuyUrl`
  </Step>
  <Step title="Consultar por la finalización">
...
- La creación de la sesión es una escritura local en la DB, por lo que nunca falla debido a errores de API de terceros.
...
stateDiagram-v2
    [*] --> open
    open --> processing : Pedido creado y pago recibido
...
Cuando se establece `checkoutSession`, el componente crea una sesión antes del pedido y utiliza el `success_url` de la sesión para redirecciones. Sin la propiedad, los flujos existentes funcionan igual que antes.
...
// 2. Crear pedido vinculado a sesión
...
// 4. Consultar sesión hasta completar
...
### Pasarela de pago fiat (Coinbase/Stripe)

```typescript
const BASE_URL = "https://api.anyspend.com";

// Pasos 1-2 igual que arriba, pero incluye configuración de pasarela de pago en la creación del pedido:
const order = await fetch(`${BASE_URL}/orders`, {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({
    // ... mismos campos de pedido ...
    checkoutSessionId: session.data.id,
    onramp: {
      vendor: "coinbase",
      payment_method: "card",
...
// Después de que el proveedor redirija de vuelta, consultar GET /checkout-sessions/:id para completar
