...
flowchart LR
    ...
    B -- Sí --> C[Recolectar Info]
    B -- No --> D[Elegir Pago]
    C --> D
    D --> E{¿Cripto o Fiat?}
    E -- Cripto --> F[Conectar Cartera & Pagar]
    E -- Fiat --> G[Redirección a Onramp]
    F --> H[Pedido Creado]
    G --> H
...
El componente principal de pago muestra un diseño de dos paneles con un panel de resumen del pedido/carrito y un panel de pago que soporta opciones de cripto y fiat. Opcionalmente incluye un panel de formulario para recolectar información del cliente, selección de envío y códigos de descuento.
...
<ParamField path="summaryLines" type="CheckoutSummaryLine[]">
  Artículos de línea adicionales en el resumen del pedido como tarifas de plataforma, propinas o cargos por servicio
</ParamField>
...
<ParamField path="senderAddress" type="string">
  Precargar la dirección del remitente para mostrar saldos de tokens antes de la conexión de la cartera
</ParamField>
...
<ParamField path="mode" type="'page' | 'embedded'" default="'page'">
  `page` para autónomo, `embedded` para en línea dentro de tu diseño
</ParamField>
...
Recolectar información del cliente durante el pago usando un esquema JSON o un componente React personalizado.

<ParamField path="formSchema" type="CheckoutFormSchema">
  Esquema JSON que define los campos a recolectar del cliente (correo electrónico, nombre, dirección, etc.). Ver [CheckoutFormSchema](#checkoutformschema) abajo.
</ParamField>
...
<ParamField path="shippingOptions" type="ShippingOption[]">
  Array de opciones de envío para mostrar como un selector de botones de radio. El monto de la opción seleccionada se agrega automáticamente al total del pedido.
</ParamField>

<ParamField path="collectShippingAddress" type="boolean">
  Cuando `true`, muestra un formulario de dirección de envío (calle, ciudad, estado, código postal, país). La dirección se incluye en el `callbackMetadata` del pedido.
</ParamField>
...
<ParamField path="enableDiscountCode" type="boolean">
  Mostrar un campo de entrada para código de descuento. Requiere que se establezca `validateDiscount`.
</ParamField>
...
Todas las propiedades de `<AnySpendCheckout>` son soportadas, más:
...
<ParamField path="callbackMetadata" type="object">
  Metadatos fusionados en el pedido. El campo `inputs` es accesible en flujos de trabajo vía `{{root.result.inputs.*}}`.
</ParamField>
...
<ParamField path="totalAmount" type="string">
  Requerido cuando no se proporciona `items` (ya que no hay de dónde computar un total).
</ParamField>
...
Artículos de línea adicionales en el resumen del pedido:
...
<Tip>
  El tipo de campo `"address"` renderiza automáticamente un formulario de dirección completo (calle, ciudad, estado, código postal, país) — no es necesario definir cada subcampo.
</Tip>
...
Estructura para direcciones de envío recolectadas:
...
Propiedades pasadas a componentes de formulario personalizados (vía `formComponent` o el slot `checkoutForm`):
