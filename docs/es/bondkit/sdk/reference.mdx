  ...
  <Accordion title="deployBondkitToken">
    ...
    **Parámetros:**
    ...
    interface BondkitTokenConfig {
      ...
      lpSplitRatioFeeRecipientBps: bigint; // Porcentaje de comisión para LP (puntos básicos)
      targetAmount: bigint;             // Cantidad objetivo en token de trading (18 decimales)
      migrationAdminAddress: Address;   // Puede activar la migración
      bondingPhaseSplitter: Address;    // Divisor de comisiones de la fase de bono
      v4PoolManager: Address;           // Gestor de pool Uniswap V4
      v4Hook: Address;                  // Gancho personalizado Uniswap V4
      v4PoolFee: number;                // Nivel de comisión del pool (3000 = 0.3%)
      v4TickSpacing: number;            // Espaciado de ticks para el pool
      tradingToken: Address;            // Token utilizado para trading (B3/ETH/etc)
    }
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Métodos de Curva de Bonificación">
    ...
    // Precio actual en token de trading
    ...
    async getBondingProgress(): Promise<{
      ...
      raised: number;        // Cantidad recaudada (convertido a número)
      threshold: number;     // Cantidad objetivo (convertido a número)
    }>
  ...
  <Accordion title="Métodos de Token de Trading">
    **Métodos para interactuar con el token de trading**
    
    ```typescript
    // Obtener dirección del token de trading (B3/ETH/etc)
    async getTradingTokenAddress(): Promise<Address>
    
    // Obtener símbolo del token de trading  
    async getTradingTokenSymbol(): Promise<string>
    
    // Obtener balance del token de trading del usuario
    ...
    **Ejemplo:**
    ```typescript
    // Verificar qué token de trading se utiliza
    const tradingTokenAddress = await token.getTradingTokenAddress();
    const tradingTokenSymbol = await token.getTradingTokenSymbol();
    console.log(`Trading con: ${tradingTokenSymbol} (${tradingTokenAddress})`);
    
    // Verificar balance del token de trading del usuario
    const tradingBalance = await token.getTradingTokenBalanceOf(userAddress);
    console.log(`Balance del token de trading: ${formatEther(tradingBalance)}`);
    
    // Verificar fase actual
    const status = await token.currentStatus();
    if (status === TokenStatus.Dex) {
      console.log("El token está en fase de trading DEX");
    }
    ```
  </Accordion>
  
  <Accordion title="Métodos de Intercambio en Fase DEX">
    **Métodos de intercambio integrados para trading en fase DEX**
    ...
    **Ejemplo:**
    ...
    const quote = await token.getSwapQuoteForBondkitToken(
      "100",      // 100 tokens de trading
      0.005       // 0.5% de tolerancia al deslizamiento
    );
    
    if (quote) {
      console.log(`Se recibirán: ${quote.amountOut} tokens bondkit`);
...
<AccordionGroup>
  <Accordion title="buy">
    **Comprar tokens con token de trading**
    ...
    **Parámetros:**
    - `amount`: Cantidad de token de trading a gastar (como string o bigint) 
    - `minTokensOut`: Mínimos tokens a recibir (protección contra deslizamiento)
    
    **Ejemplo:**
    ```typescript
    // Obtener cotización primero
    const quote = await token.getAmountOfTokensToBuy(
      parseEther("100") // 100 tokens de trading
    );
    
    // Comprar con 5% de tolerancia al deslizamiento
    const minTokens = quote * 95n / 100n;
    const txHash = await token.buy(
      parseEther("100"), // cantidad: 100 tokens de trading
  ...
  <Accordion title="sell">
    **Vender tokens por token de trading**
    ...
    **Parámetros:**
    ...
    - `minTradingTokenOut`: Mínimo token de trading a recibir (protección contra deslizamiento)
