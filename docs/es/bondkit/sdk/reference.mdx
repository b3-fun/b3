  ...
  <Accordion title="deployBondkitToken">
    ...
    **Parámetros:**
    ...
    interface BondkitTokenConfig {
      ...
      feeRecipient: Address;            // Recibe las tarifas de trading
      finalTokenSupply: bigint;         // Suministro total (18 decimales)
      aggressivenessFactor: number;     // 0-100 inclinación de la curva
      lpSplitRatioFeeRecipientBps: bigint; // Porcentaje de tarifa para LP (puntos básicos)
      ...
      bondingPhaseSplitter: Address;    // Divisor de tarifas de la fase de bono
      v4PoolManager: Address;           // Gestor de pool de Uniswap V4
      v4Hook: Address;                  // Hook personalizado de Uniswap V4
...
<AccordionGroup>
  ...
  <Accordion title="Métodos de Curva de Bono">
    ```typescript
  ...
  <Accordion title="Métodos de Intercambio en Fase DEX">
    ...
    **Ejemplo:**
    ...
    if (!canSwap) {
      console.log("Aún en fase de bono - usar buy()/sell()");
      return;
    }
...
<AccordionGroup>
  ...
  <Accordion title="sell">
    ...
    **Parámetros:**
    ...
    - `minTradingTokenOut`: Mínimo de token de trading a recibir (protección contra deslizamiento)
