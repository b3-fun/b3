  ...
  <Accordion title="deployBondkitToken">
    ...
    **Parámetros:**
    ...
    interface BondkitTokenConfig {
      ...
      aggressivenessFactor: number;     // 0-100 pendiente de la curva
      lpSplitRatioFeeRecipientBps: bigint; // Parte de la comisión del LP (puntos básicos)
      targetAmount: bigint;             // Objetivo de migración en token de trading (18 decimales)
      ...
      v4PoolManager: Address;           // Gestor del pool de Uniswap V4
...
<AccordionGroup>
  ...
  <Accordion title="Métodos de la Curva de Bonos">
    ```typescript
  ...
  <Accordion title="Métodos del Token de Trading">
  ...
  <Accordion title="Métodos de Intercambio en Fase DEX">
    ...
    **Ejemplo:**
    ...
    if (quote) {
      console.log(`Recibirás: ${quote.amountOut} tokens bondkit`);
...
<AccordionGroup>
  <Accordion title="buy">
    ...
    **Parámetros:**
    - `amount`: Monto de token de trading a gastar (como string o bigint) 
    ...
    **Ejemplo:**
    ...
    const txHash = await token.buy(
      parseEther("100"), // amount: 100 tokens de trading
  ...
  <Accordion title="sell">
    ...
    **Parámetros:**
    - `tokenAmount`: Monto de tokens a vender
