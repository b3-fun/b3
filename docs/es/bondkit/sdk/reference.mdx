  ...
  <Accordion title="deployBondkitToken">
    ...
    **Parámetros:**
    ...
    interface BondkitTokenConfig {
      ...
      feeRecipient: Address;            // Recibe tarifas de trading
      finalTokenSupply: bigint;         // Suministro total (18 decimales)
      aggressivenessFactor: number;     // 0-100 inclinación de la curva
      lpSplitRatioFeeRecipientBps: bigint; // Participación en tarifas de LP (puntos básicos)
      ...
      bondingPhaseSplitter: Address;    // Divisor de tarifas de fase de bonificación
      v4PoolManager: Address;           // Administrador de pool Uniswap V4
...
<AccordionGroup>
  ...
  <Accordion title="Métodos de Analítica">
    ```typescript
    // Titulares del token
  ...
  <Accordion title="Métodos de Token de Trading">
    **Métodos para interactuar con el token de trading**
    ...
    // Obtener saldo del token de trading del usuario
    ...
    **Ejemplo:**
    ...
    // Verificar saldo del token de trading del usuario
    const tradingBalance = await token.getTradingTokenBalanceOf(userAddress);
    console.log(`Saldo del token de trading: ${formatEther(tradingBalance)}`);
  ...
  <Accordion title="Métodos de Intercambio en Fase DEX">
    ...
    **Ejemplo:**
    ...
    if (quote) {
      ...
      console.log(`Impacto en el precio: ${quote.priceImpact}%`);
