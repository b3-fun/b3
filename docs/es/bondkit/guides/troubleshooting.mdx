...
Comienza aquí para la identificación rápida de problemas:

```typescript
import { BondkitToken, BondkitTokenFactory } from "@b3dotfun/sdk/bondkit";

// Función rápida de chequeo de salud
...
<AccordionGroup>
  <Accordion title="Error: 'Se requiere una dirección de contrato válida para BondkitToken'">
    **Causa:** La dirección del token está ausente, es inválida o no está en checksum.
  ...
  <Accordion title="Error: 'chainId no soportado: X. Este SDK está configurado solo para Base'">
    **Causa:** Intentando usar un ID de cadena no-Base.
  ...
  <Accordion title="Error: 'DeployBondkitToken: ejecución revertida'">
    ...
    **Soluciones:**
    ...
    const config = {
      ...
      v4TickSpacing: 60, // Debe coincidir con el nivel de tarifa
...
<AccordionGroup>
  <Accordion title="Error: 'Comprar: ejecución revertida' o 'Vender: ejecución revertida'">
    ...
    **Soluciones:**
    ...
    async function diagnoseBuyError(token, amount, minTokensOut) {
      ...
      if (tradingTokenAddress !== "0x0000000000000000000000000000000000000000") {
        ...
        if (balance < required) {
          return `Saldo insuficiente de ${await token.getTradingTokenSymbol()}. Necesita: ${formatEther(required)}, Tiene: ${formatEther(balance)}`;
        }
      }
      
      return "Error de compra desconocido - verificar detalles de transacción";
    }
    ...
    async function diagnoseSellError(token, tokenAmount) {
      ...
      if (balance < tokenAmount) {
        return `Saldo de token insuficiente. Necesita: ${formatEther(tokenAmount)}, Tiene: ${formatEther(balance)}`;
      }
      ...
      return "Error de venta desconocido - verificar detalles de transacción";
    }
    ```
  </Accordion>
  
  <Accordion title="Error: 'Dirección de token de trading no disponible'">
    **Causa:** Token no completamente inicializado o problemas de red.
    
    **Soluciones:**
    ```typescript
    // Forzar la actualización de la información del token de trading
    const tradingTokenAddress = await token.getTradingTokenAddress();
    ...
    **Soluciones:**
    ...
    async function getOptimalTradingMethod(tokenAddress) {
      ...
      switch (status) {
          ...
          return {
            ...
            quoteMethod: async (amount) => {
              const params = {
                ...
                tokenOutDecimals: 18,
                slippageTolerance: 0.005,
                recipient: "0x0000000000000000000000000000000000000000" // Dummy para cotización
              };
        ...
        default:
          throw new Error(`Estado de token desconocido: ${status}`);
      }
    }
    ```
  </Accordion>
...
### Problemas de Integración de Carteras
  ...
  <Accordion title="Error: 'No se pudo determinar la cuenta para la transacción'">
    ...
    **Soluciones:**
    ...
    async function ensureWalletConnected(token) {
      ...
      const connected = await token.connect(window.ethereum);
      if (!connected) {
        throw new Error("Falló la conexión del token a la cartera");
      }
  ...
  <Accordion title="Error: 'Tiempo de espera de confirmación de transacción'">
    ...
    **Soluciones:**
    ...
    async function waitForTransactionWithTimeout(token, hash, timeoutMs = 300000) {
      ...
      throw new Error(`Transacción ${hash} tiempo de espera después de ${timeoutMs}ms`);
    }
    ```
  </Accordion>
...
<AccordionGroup>
  <Accordion title="Problemas de Conexión RPC">
    ...
    **Soluciones:**
    ...
    // Sobrescribir RPC de SDK si es necesario
