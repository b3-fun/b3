...
<AccordionGroup>
  <Accordion title="Error: 'Se requiere una dirección de contrato válida para BondkitToken'">
    **Causa:** La dirección del token falta, es inválida o no está en formato checksum.
...
<AccordionGroup>
    ...
    **Soluciones:**
    ...
    async function ensureWalletConnected(token) {
      ...
      if (accounts.length === 0) {
        throw new Error("No hay cuentas disponibles - conectar la cartera primero");
      }
  ...
  <Accordion title="Error: 'Tiempo de espera de confirmación de transacción'">
    **Causa:** Red lenta, gas bajo o problemas con la cartera OKX.
...
<AccordionGroup>
  <Accordion title="Problemas de Conexión RPC">
    ...
    **Soluciones:**
    ...
    // Usar RPC personalizado con fallback
