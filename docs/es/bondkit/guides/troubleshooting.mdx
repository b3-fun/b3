...
description: >-
  ...
  billetera e desafíos de integración.
lang: es
...
<AccordionGroup>
  <Accordion title="Error: 'Se requiere una dirección de contrato válida para BondkitToken'">
    **Causa:** La dirección del token falta, es inválida o no está en checksum.
...
<AccordionGroup>
    ...
    **Soluciones:**
    ...
    async function diagnoseSellError(token, tokenAmount) {
      ...
      if (balance < tokenAmount) {
        return `Saldo insuficiente de token. Necesario: ${formatEther(tokenAmount)}, Disponible: ${formatEther(balance)}`;
      }
  ...
  <Accordion title="Error: 'Dirección del token de trading no disponible'">
    ...
    **Soluciones:**
    ...
    if (!tradingTokenAddress) {
      ...
      if (!retryAddress) {
        throw new Error("El contrato de token puede no estar correctamente inicializado");
      }
    }
    
    // Verificar si el contrato de token es válido
    try {
  ...
  <Accordion title="Error: 'Fase de Bonding Terminada' o problemas de trading DEX">
...
<AccordionGroup>
  <Accordion title="Error: 'No se pudo determinar la cuenta para la transacción'">
    ...
    **Soluciones:**
    ...
    async function ensureWalletConnected(token) {
      if (!window.ethereum) {
        throw new Error("No se encontró proveedor de cartera - instale MetaMask o similar");
      }
      ...
      if (accounts.length === 0) {
        throw new Error("No hay cuentas disponibles - conecte la cartera primero");
      }
  ...
  <Accordion title="Error: 'Tiempo de espera de confirmación de transacción'">
    **Causa:** Red lenta, gas bajo o problemas con carteras OKX.
...
<AccordionGroup>
  <Accordion title="Problemas de Conexión RPC">
    ...
    **Soluciones:**
    ...
    // Usar RPC personalizado con respaldo
  ...
  <Accordion title="Problemas de Estimación de Gas">
    **Problema:** Transacciones fallan por "falta de gas" o errores de estimación de gas.
