...
description: >-
  Soluciones para problemas comunes del SDK de Bondkit, problemas con billeteras
...
<AccordionGroup>
  ...
  <Accordion title="Error: 'DeployBondkitToken: ejecución revertida'">
    ...
    **Soluciones:**
    ...
    const config = {
      name: "MyToken",
      symbol: "MTK", // Debe tener 1-11 caracteres
    ...
    function validateConfig(config) {
      ...
      if (!config.symbol || config.symbol.length > 11) {
        errors.push("El símbolo debe tener 1-11 caracteres");
      }
    ...
    **Problemas Comunes de Cartera:**
    ...
    async function addTokenToWallet(tokenAddress, symbol, decimals = 18) {
      try {
        ...
        console.error('Falló al agregar token a la cartera:', error);
      }
    }
    ```
  </Accordion>
