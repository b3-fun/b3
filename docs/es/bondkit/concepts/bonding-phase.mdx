...
<Tabs>
  <Tab title="Cómo Funciona">
    **Flujo del Proceso:**
    ...
    3. El contrato calcula la cantidad de token de comercio basada en la curva
...
<AccordionGroup>
  <Accordion title="Comisiones de Compra">
    ...
    **Ejemplo:**
    ...
    // Se acuñan tokens basados en 0.95 token de comercio
    ```
  </Accordion>
...
async function trackProgress(token: BondkitToken) {
  ...
  la cantidad restante = progress.threshold - progress.raised;
...
<CardGroup cols={2}>
  ...
  <Card title="Gestión de Riesgos" icon="shield">
    ...
    - No caer en FOMO cerca del objetivo
