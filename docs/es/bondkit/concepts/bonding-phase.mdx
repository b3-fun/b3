...
3. **Independencia de la Ruta**: El precio depende solo de la oferta, no de la historia
...
<Tabs>
  ...
  <Tab title="Casos Límite">
    **Desbordamiento del Objetivo:**
    - Si la compra superara el objetivo, el exceso se reembolsa
...
<AccordionGroup>
  <Accordion title="Comisiones de Compra">
    **En una compra de 1 token de comercio:**
    ...
    - Los tokens se calculan sobre el valor de 0.95 tokens de comercio
...
async function trackProgress(token: BondkitToken) {
  ...
  if (progress.progress < 0.5) {
    ...
    console.log("🚀 Construyendo momentum - considere comprar");
  ...
  console.log(`Necesita ${formatEther(remaining)} más tokens de comercio`);
}
```
...
<Warning>
  **Crítico: Después de la Migración**
  ...
  - ❌ Comercio en la curva de vinculación deshabilitado permanentemente
