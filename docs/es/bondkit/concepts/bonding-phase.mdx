...
La fase de vinculación es el período inicial de comercio donde los tokens se compran y venden directamente de un contrato inteligente utilizando una curva de precios algorítmica. Piénsalo como una preventa descentralizada con creación de mercado automática.
...
3. **Independencia del Camino**: El precio depende solo de la oferta, no de la historia
...
<Tabs>
  <Tab title="Cómo Funciona">
    **Flujo de Proceso:**
    ...
    3. Se deduce una tarifa del 5% y se envía al destinatario
    4. Los tokens se acuñan y se envían al comprador
  ...
  <Tab title="Parámetros">
    ...
    |-----------|------|-------------|
    | `minTokensOut` | uint256 | Mínimo de tokens a recibir (protección contra deslizamiento) |
  ...
  <Tab title="Casos Límite">
    **Desbordamiento del Objetivo:**
    - Si la compra excediera el objetivo, se reembolsa el exceso
...
<Tabs>
  <Tab title="Cómo Funciona">
    **Flujo de Proceso:**
    ...
    4. Se deduce una tarifa del 5% de los ingresos
  ...
  <Tab title="Parámetros">
    ...
    | `minTradingTokenOut` | uint256 | Mínimo de token de comercio a recibir (protección contra deslizamiento) |
  ...
  <Tab title="Impacto en el Precio">
...
graph LR
    ...
    B -->|5%| D[Destinatario de la Tarifa]
...
<AccordionGroup>
  ...
  <Accordion title="Distribución de Tarifas">
    **A dónde van las tarifas:**
...
| Volumen Diario | Ingresos por Tarifas | Ingresos Mensuales |
|-------------|------------|----------------|
...
async function trackProgress(token: BondkitToken) {
  ...
  if (progress.progress < 0.5) {
    ...
    console.log("🚀 Construyendo impulso - considera comprar");
    ...
    **Escuchando en SDK:**
    ...
    token.onBuy((event) => {
      console.log({
        ...
        tarifaPagada: formatEther(event.fee),
...
<CardGroup cols={2}>
  ...
  <Card title="Maximizar el Éxito" icon="trophy">
    ...
    - Programar anuncios adecuadamente
...
<Warning>
  **Crítico: Después de la Migración**
  ...
  Una vez se llame a `migrateToDex()`:
