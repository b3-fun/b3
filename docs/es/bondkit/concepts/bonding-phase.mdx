...
La fase de vinculación es el período inicial de comercio donde los tokens se compran y venden directamente desde un contrato inteligente utilizando una curva de precios algorítmica. Piénselo como una preventa descentralizada con creación de mercado automática.
...
<Tabs>
  ...
  <Tab title="Casos Límite">
    **Desbordamiento del Objetivo:**
    ...
    - El usuario envía 1 token → se usan 0.5 tokens, 0.5 tokens reembolsados
...
<Tabs>
  ...
  <Tab title="Parámetros">
    ...
    **Notas Importantes:**
    - Requiere aprobación de token antes de vender
...
<AccordionGroup>
  <Accordion title="Comisiones por Compra">
  ...
  <Accordion title="Comisiones por Venta">
    **Al vender por un valor de 1 token de comercio:**
...
async function trackProgress(token: BondkitToken) {
  ...
  if (progress.progress < 0.5) {
    ...
    console.log("🚀 Construyendo momento - considere comprar");
...
<CardGroup cols={2}>
  ...
  <Card title="Gestión de Riesgos" icon="shield">
    - Usar siempre protección contra deslizamiento
...
<Warning>
  **Crítico: Después de la Migración**
  ...
  - ❌ Comercio de curva de vinculación deshabilitado permanentemente
