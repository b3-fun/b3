...
stateDiagram-v2
    ...
    DEX --> [*]: Totalmente Descentralizado
...
2. **Liquidez Instantánea**: Siempre posible comprar o vender
3. **Independencia de Ruta**: Precio depende solo de la oferta, no de la historia
...
<Tabs>
  <Tab title="Cómo Funciona">
    **Flujo de Proceso:**
    1. Usuario envía token de comercio (B3/ETH) al contrato
    2. Contrato calcula tokens basado en la curva
    3. Se deduce una comisión del 5% y se envía al destinatario
    4. Tokens acuñados y enviados al comprador
    5. Estado de la curva actualizado
  ...
  <Tab title="Parámetros">
    ...
    **Notas Importantes:**
    - Establecer `minTokensOut` para proteger contra el front-running
    - Usar 0 para `minTokensOut` solo en períodos de baja actividad
    - La transacción se revierte si el deslizamiento excede la tolerancia
  </Tab>
  
  <Tab title="Casos Límite">
    **Desbordamiento del Objetivo:**
    - Si la compra excediera el objetivo, el exceso se reembolsa
    - Ejemplo: El objetivo es 10 tokens de comercio, recaudado es 9.5 tokens
    - Usuario envía 1 token → 0.5 tokens utilizados, 0.5 tokens reembolsados
    
    **Gas Insuficiente:**
    - Límite de gas recomendado: 200,000
    - Cálculos complejos pueden requerir más
    
    **Cantidad Cero:**
    - Transacciones con 0 tokens de comercio se revertirán
  </Tab>
</Tabs>
...
<Tabs>
  <Tab title="Cómo Funciona">
    **Flujo de Proceso:**
    1. Usuario aprueba el gasto de tokens
    2. Usuario llama a vender con cantidad de tokens
    3. Contrato calcula cantidad de token de comercio basado en la curva
    4. Se deduce una comisión del 5% de los ingresos
    5. Token de comercio enviado al vendedor, tokens quemados
    6. Estado de la curva actualizado (precio disminuye)
  ...
  <Tab title="Parámetros">
    ...
    **Notas Importantes:**
    - Requiere aprobación de tokens antes de vender
    - El impacto de precio puede ser significativo para ventas grandes
    - No se pueden vender más que el suministro circulante
  </Tab>
  
  <Tab title="Impacto de Precio">
    **Entendiendo el Impacto de Venta:**
    
    Vender reduce la cantidad recaudada de la curva, causando que el precio caiga:
    
    | Tamaño de Venta | Impacto de Precio | Recuperación Necesaria |
    |-----------|--------------|-----------------|
...
<AccordionGroup>
  <Accordion title="Comisiones de Compra">
    ...
    **Ejemplo:**
    ```typescript
    // Usuario envía 1 token de comercio
    // Comisión: 0.05 token de comercio
    // Curva recibe: 0.95 token de comercio
  ...
  <Accordion title="Comisiones de Venta">
    **Al vender por valor de 1 token de comercio:**
    - Usuario recibe 0.95 tokens de comercio
    - 0.05 tokens de comercio van al destinatario de la comisión
    - Curva reducida por completo 1 token de comercio
    ...
    **Ejemplo:**
    ...
    // Comisión: 0.05 token de comercio
    // Usuario recibe: 0.95 tokens de comercio
    // Curva cae por 1 token de comercio
    ```
  </Accordion>
  ...
  <Accordion title="Distribución de Comisiones">
    ...
    **Reclamación de comisiones:**
    ...
    - Enviada directamente en cada comercio
...
async function trackProgress(token: BondkitToken) {
  ...
  if (progress.progress < 0.5) {
    ...
    console.log("🚀 Construyendo momento - considerar comprar");
...
2. **Reembolso Automático**: Exceso devuelto en la misma transacción
...
- Usuario obtiene tokens por 0.5 tokens de comercio
...
<Warning>
  **Crítico: Después de la Migración**
  ...
  - ✅ Comercio totalmente descentralizado
</Warning>
