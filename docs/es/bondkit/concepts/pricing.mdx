...
<Note>
**Conclusión Clave**: Con máxima agresividad (100), comprar 1M B3 en el lanzamiento le consigue a Alice 6.2x más tokens que esperar hasta el 90% de progreso. ¡La curva de bonificación recompensa fuertemente la participación temprana!
</Note>

## Calculadora de Precios Interactiva

```typescript
// Calcular tokens recibidos por una cantidad dada de token de comercio
...
```typescript
// Calcular token de comercio recibido por vender tokens
function calculateTradingTokenReceived(
  ...
  // Calcular cantidad recaudada correspondiente
