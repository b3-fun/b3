...
| Agresividad | Exponente | La Fórmula Se Convierte En | Comportamiento del Precio |
|---------------|----------|-----------------|----------------|
| 0 | 1.00 | S = S_final × (R/R_objetivo) | Lineal - tokens constantes por ETH |
| 25 | 0.80 | S = S_final × (R/R_objetivo)^0.80 | Ligeramente curvado |
...
2. **Comisión de transacción (3%)**: 30K B3 van a los destinatarios de la comisión  
...
<Note>
**Conclusión Clave**: Con una agresividad máxima (100), comprar 1M B3 en el lanzamiento le consigue a Alice 6.2x más tokens que esperar hasta el 90% de progreso. ¡La curva de bonificación recompensa enormemente la participación temprana!
</Note>

## Calculadora de Precios Interactiva

```typescript
// Calcular tokens recibidos por una cantidad dada de tokens de intercambio
...
2. **Deducción de Comisión**: Se aplica una comisión del 5% a los ingresos
...
```typescript
// Calcular tokens de intercambio recibidos por vender tokens
function calculateTradingTokenReceived(
  ...
  // ETH recibidos (antes de comisiones)
...
<Warning>
  **Consideraciones Importantes:**
  ...
  2. **Montos Mínimos**: Las transacciones muy pequeñas pueden revertirse debido a redondeo
