...
const tokenAddress = await factory.deployBondkitToken({
  ...
  // Configuración de la curva de bonos
...
<ParamField path="aggressivenessFactor" type="number" required>
  Controla la empinadura de la curva de bonos (0-100):
...
Los tokens BondKit utilizan un sistema de **token de comercio configurable**. Verifica qué token se utiliza para comerciar:
...
<Warning>
  **Importante**: Después de la migración:
  ...
  - El comercio de la curva de bonos se deshabilita permanentemente
...
<CardGroup cols={2}>
  <Card title="Conceptos" icon="book" href="/bondkit/concepts/bonding-phase">
    Profundiza en curvas de bonos y precios
  </Card>
