...
2. **Comerciar** durante la fase de emisión
3. **Monitorear** el progreso hacia el objetivo
4. **Migrar** a Uniswap v4

<Info>
  **Prerrequisitos**: Asegúrate de haber completado la [instalación](/bondkit/installation) y tener una billetera financiada en Base.
</Info>
...
const tokenAddress = await factory.deployBondkitToken({
  ...
  // Configuraciones de la curva de emisión
  ...
  lpSplitRatioFeeRecipientBps: 1000n, // 10% al receptor de la comisión
  ...
  v4PoolFee: 3000, // Nivel de comisión de 0.3%
  v4TickSpacing: 60,
...
<ParamField path="aggressivenessFactor" type="number" required>
  Controla la empinadura de la curva de emisión (0-100):
...
<ParamField path="feeRecipient" type="address" required>
  Dirección que recibe las comisiones de comercio (5% de todas las transacciones).
</ParamField>

<ParamField path="lpSplitRatioFeeRecipientBps" type="bigint" required>
  Puntos básicos (1/100 de 1%) para la división de la comisión de LP. 1000 = 10%.
</ParamField>

## Paso 2: Comerciar Durante la Fase de Emisión
...
Los tokens de BondKit usan un sistema de **token de comercio configurable**. Verifica qué token se usa para comerciar:
...
```typescript
// Verificar el progreso de emisión
...
if (progress.progress >= 1.0) {
  ...
  console.log("La migración creará pool con:", {
    tradingTokenLiquidity: formatEther(migrationData.ethForLp), // Nota: el nombre del campo es 'ethForLp'
...
<Warning>
  **Importante**: Después de la migración:
  ...
  - La comercialización de la curva de emisión se deshabilita permanentemente
...
Aquí tienes un ejemplo completo de trabajo que demuestra todo el ciclo de vida:
...
async function launchToken() {
  ...
  // 2. Comerciar durante la emisión
  const token = new BondkitToken(
    tokenAddress,
    process.env.WALLET_PRIVATE_KEY
  );
  ...
  console.log("Comprados tokens con 50 tokens de comercio");
...
<CardGroup cols={2}>
  <Card title="Conceptos" icon="book" href="/bondkit/concepts/bonding-phase">
    Profundizar en curvas de emisión y precios
  </Card>
  ...
  <Card title="Aplicación de Demostración" icon="desktop" href="https://github.com/b3dotfun/b3-monorepo/tree/main/apps/bondkit-demo">
