...
<Info>
  **Prerrequisitos**: Asegúrate de haber completado la [instalación](/bondkit/installation) y de tener una cartera financiada en Base.
</Info>
...
const tokenAddress = await factory.deployBondkitToken({
  ...
  v4PoolFee: 3000, // Nivel de comisión del 0.3%
  v4TickSpacing: 60,
...
<ParamField path="aggressivenessFactor" type="number" required>
  Controla la empinadez de la curva de vinculación (0-100):
...
<ParamField path="lpSplitRatioFeeRecipientBps" type="bigint" required>
  Puntos básicos (1/100 de 1%) para la división de comisiones de LP. 1000 = 10%.
</ParamField>
...
```typescript
// Verificar que se haya alcanzado el objetivo
const progress = await token.getBondingProgress();
...
async function launchToken() {
  ...
  // 3. Continuar comerciando hasta que se alcance el objetivo...
