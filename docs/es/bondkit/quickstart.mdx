...
2. **Comerciar** durante la fase de bonding
...
const tokenAddress = await factory.deployBondkitToken({
  ...
  // Configuraciones de la curva de bonding
...
<ParamField path="aggressivenessFactor" type="number" required>
  Controla la empinación de la curva de bonding (0-100):
  - **0-30**: Precios lineales, justo para todos los compradores
  - **30-60**: Curva moderada, enfoque equilibrado
  - **60-100**: Agresiva, recompensa a los primeros compradores
</ParamField>
...
## Paso 2: Comerciar Durante la Fase de Bonding
...
```typescript
// Verificar el progreso de bonding
...
<Warning>
  **Importante**: Después de la migración:
  ...
  - El comercio de la curva de bonding se deshabilita permanentemente
...
Aquí tienes un ejemplo de trabajo completo que demuestra todo el ciclo de vida:
...
async function launchToken() {
  ...
  // 2. Comerciar durante el bonding
  const token = new BondkitToken(
    tokenAddress,
    process.env.WALLET_PRIVATE_KEY
  );
...
<CardGroup cols={2}>
  <Card title="Conceptos" icon="book" href="/bondkit/concepts/bonding-phase">
    Profundizar en curvas de bonding y precios
  </Card>
