...
title: Acuñación
description: >-
  Guía completa para la acuñación de tokens con CreateKit incluyendo la
  implementación, acuñación regular y características avanzadas
lang: es
...
```typescript title="Despliegue y Primera Acuñación"
...
```typescript title="Acuñación Regular"
...
<Tabs>
<Tab title="Acuñación ERC721">
```typescript title="Tokens Únicos ERC721"
...
<Tab title="Acuñación ERC1155">
```typescript title="Tokens Semi-Fungibles ERC1155"
// ERC1155 - Puede acuñar múltiples del mismo o diferentes tokens
const erc1155Collection = collectionManager.createCollection(
  predictedAddress,
  "ERC1155"
)

// Puede acuñar múltiples tokens del mismo tipo
await erc1155Collection.mint(
  walletClient,
  10n, // Acuñar 10 tokens
  "https://example.com/metadata/sword.json", // Metadatos para tokens "espada"
  parseEther("0.005"),
  []
)

// Puede acuñar diferentes tipos de tokens cambiando el URI de metadatos
await erc1155Collection.mint(
  walletClient,
  5n,
...
```typescript title="Acuñación a Precio Fijo"
...
```typescript title="Acuñación Gratuita"
...
```typescript title="Lógica de Precios Dinámicos"
...
```typescript title="Configuración de Lista Blanca"
...
```typescript title="Acuñación con Prueba de Lista Blanca"
...
```typescript title="Límites de Cartera"
...
```typescript title="Acuñación por Lotes"
...
```typescript title="Acuñación Eficiente de Gas"
...
```typescript title="Optimización del Precio del Gas"
...
console.log("Acuñaciones recientes:", mintEvents.map(event => ({
...
```typescript title="Monitoreo de Acuñaciones en Tiempo Real"
...
<CardGroup cols={2}>
  <Card title="Experiencia del Usuario" icon="user">
    - Proporcionar retroalimentación clara durante el proceso de acuñación
    - Mostrar costos estimados de gas por adelantado
    - Implementar estados de carga adecuados
    - Manejar errores de manera amigable para el usuario
  </Card>
  <Card title="Interacción con Contratos Inteligentes" icon="code">
    - Validar siempre los parámetros antes de las transacciones
    - Implementar manejo de errores adecuado
    - Usar límites y precios de gas apropiados
