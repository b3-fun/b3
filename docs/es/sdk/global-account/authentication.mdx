...
<Note>
  Esta es una **demostración en vivo e interactiva** utilizando el SDK de B3 real. Cuando no especificas `strategies`, se muestran todas las opciones de autenticación disponibles. Ve la [página completa de la demostración](https://global.b3.fun/demos) para más ejemplos.
</Note>

## Estrategias de Autenticación

Las Cuentas Globales de B3 admiten múltiples estrategias de autenticación para adaptarse a las necesidades de tu aplicación.
...
<Note>
  El hook `useB3` proporciona acceso a la `account` autenticada (cuenta de billetera) y al `user` (datos del perfil del usuario). Utiliza `useAuthStore` para acceder a los estados de carga y autenticación.
</Note>
...
<Card title="ID del Socio" icon="id-badge">
...
<Card title="Configuración del Entorno" icon="settings">
  Utiliza variables de entorno apropiadas para diferentes etapas de despliegue.
</Card>
...
<ParamField path="partnerId" type="string" required>
  Tu ID de socio único para las Cuentas Globales de B3.
</ParamField>
...
Las Cuentas Globales de B3 admiten los siguientes métodos de autenticación:
