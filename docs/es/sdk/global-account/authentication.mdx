...
---
<Tip>
  **Para Desarrolladores**: Todas las demostraciones están alojadas en la aplicación [global-accounts](https://github.com/b3-fun/b3/tree/main/apps/global-accounts). Para probar localmente, ejecute `pnpm dev` en el directorio global-accounts y acceda a las demostraciones en `http://localhost:5173/demos`.
</Tip>
...
<Note>
  Esta es una **demo interactiva en vivo** utilizando el SDK de B3 actual. Cuando no especificas `strategies`, se muestran todas las opciones de autenticación disponibles. Consulta la [página completa de la demo](https://global.b3.fun/demos) para más ejemplos.
</Note>

## Estrategias de Autenticación

B3 Global Accounts soporta múltiples estrategias de autenticación para adaptarse a las necesidades de tu aplicación.
...
<iframe
  ...
  title="Demostración de Autenticación con Google"
/>
...
<Note>
  Especifica un array de estrategias para mostrar solo esas opciones de autenticación a tus usuarios.
</Note>
...
Accede a la información de la billetera y al estado de conexión:
...
<ParamField path="strategies" type="AllowedStrategy[]">
  Array de estrategias de autenticación para mostrar. Las opciones incluyen: `"google"`, `"github"`, `"email"`, `"discord"`, `"x"`, `"apple"`, `"walletConnect"`, `"io.metamask"`, `"com.coinbase.wallet"`. 
  
  Dejar indefinido para mostrar todas las opciones.
</ParamField>

<ParamField path="chain" type="Chain" required>
  Objeto de configuración de la cadena de bloques con `id`, `name`, `nativeCurrency` y `rpc`.
</ParamField>
...
<ParamField path="closeAfterLogin" type="boolean" default="false">
  Si cerrar el modal después del inicio de sesión exitoso.
</ParamField>
...
<ParamField path="withLogo" type="boolean" default="true">
  Si mostrar el logo de B3 en el botón.
</ParamField>

## Estrategias de Autenticación Disponibles

B3 Global Accounts soporta los siguientes métodos de autenticación:
...
<table>
  ...
  <tbody>
    <tr>
      <td>`"google"`</td>
      <td>Social</td>
      <td>Autenticación OAuth con Google</td>
    </tr>
    <tr>
      <td>`"discord"`</td>
      <td>Social</td>
      <td>Autenticación OAuth con Discord</td>
    </tr>
    <tr>
      <td>`"github"`</td>
      <td>Social</td>
      <td>Autenticación OAuth con GitHub</td>
    </tr>
    <tr>
      <td>`"x"`</td>
      <td>Social</td>
      <td>Autenticación con X (anteriormente Twitter)</td>
    </tr>
    <tr>
      <td>`"apple"`</td>
      <td>Social</td>
    ...
    <tr>
      ...
      <td>Autenticación como invitado sin registro</td>
    </tr>
    <tr>
      <td>`"email"`</td>
      <td>Sin contraseña</td>
      <td>Autenticación con código de verificación por email</td>
    </tr>
    <tr>
      <td>`"walletConnect"`</td>
      <td>Billetera</td>
...
<Card title="Ejemplos" icon="code" href="/sdk/global-account/examples">
  Consulta ejemplos completos de integración.
</Card>

<Card title="Probar la Demo" icon="play" href="https://login-minimal-b3.vercel.app/">
  Aplicación de demostración de autenticación interactiva.
</Card>
