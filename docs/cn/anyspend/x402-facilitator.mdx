---
title: AnySpend 协调者
description: 适用于 x402 多代币支付的生产级支付处理器
lang: cn
...
AnySpend x402 Facilitator 是一个生产级支付基础设施，为 x402 协议提供多代币、跨链支付能力。它处理所有代币交换、跨链桥接和结算的复杂性，允许开发者接受任何代币的支付，同时收到他们偏好的货币。
...
- **19+ 网络**：支持跨 EVM 链（Base、Ethereum、Arbitrum 等）和 SVM（Solana）
...
- **同链支付**：直接代币转账，最小开销
- **跨链支付**：自动桥接不同网络（例如，ETH → Base）
- **跨 VM 支付**：Solana ↔ EVM 转账的实验性支持
- **最优路由**：智能选择最佳执行路径
...
- ✅ **HTTP-native 支付** 通过 `402 Payment Required` 状态
...
**API 层**
- RESTful HTTP API 用于支付验证和结算
- 支持 WebSocket 实时支付状态更新
- 速率限制和 DDoS 防护
- 99.9% 正常运行时间 SLA
...
const client = new X402Client({
  walletClient,
  facilitatorUrl: 'https://mainnet.anyspend.com/x402'  // 可选，默认使用此地址
});
...
Abstract 测试网, Avalanche Fuji, Base Sepolia, Polygon Amoy, Sei 测试网

### SVM 网络（2）

Solana 主网, Solana Devnet

查看 [网络支持](/anyspend/x402-network-support) 了解完整详情。

## 为什么选择 AnySpend Facilitator？

<CardGroup cols={2}>
<Card title="标准合规" icon="check-circle">
  完全符合 x402 协议 - 与任何兼容的客户端或服务器一起工作
</Card>

<Card title="生产就绪" icon="rocket">
  经过实战测试的基础设施，具有企业级的可靠性和性能
</Card>

<Card title="多链" icon="globe">
  支持 19+ 区块链网络，自动跨链路由
</Card>

<Card title="开发者友好" icon="code">
  为 Express、Next.js、Hono 和客户端 TypeScript 提供简单的 API 和 SDK
</Card>
</CardGroup>
