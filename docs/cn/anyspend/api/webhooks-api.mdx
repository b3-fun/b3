...
---
Webhooks 允许您在您的 AnySpend 账户中发生事件时接收实时的 HTTP POST 通知。配置端点，选择要订阅的事件，并监控交付历史。
...
<ParamField path="POST /webhooks" type="admin">
  创建一个新的 webhook 端点。响应包括用于验证 webhook 签名的 `secret` 字段。请安全存储此字段——它仅在创建时返回一次。
</ParamField>
...
<ParamField path="GET /webhooks/:id/deliveries" type="read">
  查看 webhook 端点的交付日志。返回最近的交付尝试，包括状态代码和响应时间。
</ParamField>
...
<ParamField path="POST /webhooks/:id/deliveries/:did/retry" type="admin">
  手动重试失败的 webhook 交付。交付必须具有 `failed` 状态。
</ParamField>
...
| `success_count` | `number` | 总成功交付次数 |
| `failure_count` | `number` | 总失败交付次数 |
| `last_triggered_at` | `string \| null` | 最后一次交付尝试的 ISO 8601 时间戳 |
...
<Note>
  总是在 30 秒内返回一个 `2xx` 状态码。失败的交付会进行最多 3 次的重试，采用指数退避。
</Note>
