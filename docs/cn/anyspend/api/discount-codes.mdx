...
---
折扣码允许您在支付链接上提供基于百分比或固定金额的折扣。创建单个代码、批量生成批次，并在结账时验证代码。
...
<ParamField path="code" type="string" required>
  折扣码字符串（例如，`SUMMER25`）。必须唯一。自动转换为大写。
</ParamField>
...
<ParamField path="value" type="number" required>
  折扣值。对于 `percentage`，数字在 1-100 之间。对于 `fixed`，金额为代币最小单位（例如，`5000000` 表示 5 USDC）。
</ParamField>

<ParamField path="payment_link_id" type="string">
  将此代码限制为特定支付链接。如果省略，代码适用于所有支付链接。
</ParamField>
...
<ParamField path="amount" type="string" required>
  代币最小单位的订单金额，用于检查 `min_order_amount` 并计算折后总额。
</ParamField>
...
<Note>
  验证端点 **不** 消耗使用次数。只有在使用折扣完成支付时，才会计算使用次数。
</Note>
...
| `current_uses` | `number` | 到目前为止已兑换次数 |
| `min_order_amount` | `string \| null` | 所需最小订单金额 |
