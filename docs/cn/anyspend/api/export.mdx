...
<CardGroup cols={2}>
  <Card title="安排定期导出" icon="calendar">
    设置 cron 作业或计划函数，以每天或每周导出数据，用于备份和对账。
  </Card>
  <Card title="对大型数据集使用日期过滤器" icon="filter">
    如果您有成千上万的交易，使用 `from` 和 `to` 参数以更小的日期范围导出，以避免超时。
  </Card>
  <Card title="CSV 用于电子表格" icon="table">
    CSV 导出可以直接在 Excel、Google Sheets 和其他电子表格工具中打开，便于快速分析。
  </Card>
  <Card title="JSON 用于程序化使用" icon="code">
    JSON 导出是将数据输入分析管道、数据库或自定义仪表板的理想选择。
  </Card>
</CardGroup>

<Warning>
  导出受到速率限制。大型导出（超过 10,000 条记录）可能需要几秒钟时间生成。请求将阻塞直到导出完成 -- 对于非常大的数据集，请相应规划。
</Warning>
