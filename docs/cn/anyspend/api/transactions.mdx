...
<Info>
  交易金额以代币的最小单位存储（例如，USDC 的 6 个小数）。`amount_usd` 字段提供交易时的 USD 等值金额。
</Info>
...
<ResponseField name="shipping_address" type="object | null">
  ...
  <Expandable title="shipping address fields">
    ...
    <ResponseField name="state" type="string">州或省。</ResponseField>
...
<ResponseField name="shipping_option" type="object | null">
  ...
  <Expandable title="shipping option fields">
    ...
    <ResponseField name="amount" type="string">送货成本。</ResponseField>
  </Expandable>
</ResponseField>
...
<ParamField query="from" type="string">
  过滤在此 ISO 8601 日期或之后创建的交易（例如，`2026-01-01`）。
</ParamField>

<ParamField query="to" type="string">
  过滤在此 ISO 8601 日期或之前创建的交易（例如，`2026-02-28`）。
</ParamField>
...
以 CSV 或 JSON 格式导出交易数据。响应将以文件下载的形式流式传输。
...
<Info>
  导出限于 50,000 条记录。对于更大的数据集，使用日期范围过滤器将导出分成更小的块。
</Info>

<CodeGroup>
```bash cURL
# 导出 2026 年 2 月完成的交易为 CSV
