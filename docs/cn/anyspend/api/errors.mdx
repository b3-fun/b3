...
---
AnySpend 平台 API 在所有端点上使用一致、结构化的错误格式。错误遵循 Stripe 约定 -- 一个包含 `error` 键的 JSON 对象，该键包含类型、代码、人类可读的消息，以及（在适用的情况下）导致错误的参数。
...
| `message` | `string` | 关于出错原因的人类可读描述。安全地显示给开发者（但不是最终用户）。 |
| `param` | `string` 或不存在 | 导致错误的请求参数（如果适用）。 |

## 错误类型

错误类型将错误分组到广泛的类别中。用于顶级错误处理。

| 类型 | HTTP 状态 | 描述 |
|------|-------------|-------------|
| `invalid_request_error` | `400` | 请求格式不正确或包含无效参数。检查 `param` 字段以了解哪个字段导致了问题。 |
...
| `rate_limit_error` | `429` | 您已超过认证层的速率限制。 |
| `idempotency_error` | `409` | 重复使用了具有不同请求体的幂等性键。 |
...
| 代码 | 类型 | HTTP | 描述 |
|------|------|------|-------------|
| `missing_required_field` | `invalid_request_error` | `400` | 请求体中未包含必需的字段。`param` 字段指示缺少哪个字段。 |
| `invalid_field_value` | `invalid_request_error` | `400` | 包含了字段，但其值无效（类型、格式或范围错误）。 |
| `invalid_address` | `invalid_request_error` | `400` | 以太坊地址字段不是有效的 `0x` + 40 个十六进制字符地址。 |
| `no_updates_provided` | `invalid_request_error` | `400` | 发送了 `PATCH` 请求，但没有有效字段进行更新。 |
| `invalid_status_transition` | `invalid_request_error` | `400` | 请求的状态更改不允许（例如，完成一个已过期的会话）。 |
| `duplicate_resource` | `invalid_request_error` | `409` | 存在具有冲突唯一字段的资源。 |
...
| `key_revoked` | `authentication_error` | `401` | API 密钥已明确撤销。创建新密钥。 |
| `insufficient_permissions` | `permission_error` | `403` | API 密钥没有此操作所需的权限级别（例如，尝试使用 `read` 密钥执行 `POST`）。 |
...
| 代码 | 类型 | HTTP | 描述 |
|------|------|------|-------------|
| `rate_limit_exceeded` | `rate_limit_error` | `429` | 在当前时间窗口内请求过多。错误消息包括重试延迟。 |
| `idempotency_conflict` | `idempotency_error` | `409` | 重复使用了与原始请求不同的请求体的 `Idempotency-Key` 头。 |

### 服务器错误

| 代码 | 类型 | HTTP | 描述 |
|------|------|------|-------------|
| `internal_error` | `api_error` | `500` | 发生了意外的服务器错误。这些错误会自动跟踪。如果问题持续存在，请联系支持。 |
...
当 `error.param` 存在时，您可以直接将其映射到表单字段，以在 UI 中显示内联验证错误。
</Accordion>
...
客户端错误如 `400`、`401`、`403` 和 `404` 在重试时不会解决。在重试之前修正请求或凭证。
</Accordion>

<Accordion title="为调试记录完整的错误对象">
始终记录 `error.type`、`error.code`、`error.message` 和 `error.param`。这在调试生产问题时给您完整的画面。
</Accordion>
</AccordionGroup>
