---
title: 分页 & 筛选
description: 通过列表端点分页和过滤结果
lang: cn
originalPath: anyspend/api/pagination.mdx
---
所有 AnySpend 平台 API 的列表端点都返回分页响应。本页面涵盖了如何导航大型结果集、过滤数据和排序结果。
...
| `data` | `array` | 当前页面的资源对象数组。每个项目都包括自己的 `object` 类型字段。 |
| `has_more` | `boolean` | 如果此页面之后还有更多结果，则为 `true`。使用此字段来决定是否获取下一页。 |
...
| 参数 | 类型 | 默认值 | 最大值 | 描述 |
|-----------|------|---------|-----|-------------|
| `page` | `integer` | `1` | -- | 要检索的页面号（从1开始）。 |
| `limit` | `integer` | `20` | `100` | 每页的结果数。 |
...
大多数列表端点支持查询参数来过滤结果。可用的过滤器取决于资源类型。
...
async function main() {
  for await (const link of paginate<PaymentLink>("payment-links", { active: "true" })) {
    console.log(`${link.name}: ${link.current_uses} 完成次数`);
  }
}
```
...
使用任何页面的 `total_count` 来获取匹配资源的总数，而无需获取所有页面：
...
`has_more` 是是否获取下一页的权威信号。避免从 `total_count` 计算页面计数，因为总数可能会在请求之间发生变化。
</Accordion>

<Accordion title="为 UI 分页控件缓存 total_count">
构建分页 UI（例如，“第3页，共8页”）时，获取一次 `total_count` 并为会话缓存它。仅在用户刷新或应用新过滤器时重新获取。
</Accordion>
...
如果您正在分页浏览大型数据集，请注意每分钟120次请求的速率限制。对于非常大的数据集，在页面请求之间添加短暂延迟：
...
async function fetchWithDelay() {
  ...
  // 对于10,000条记录，即100次请求 —— 在120/分钟的限制内。
}
```
</Accordion>
</AccordionGroup>
