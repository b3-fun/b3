...
<ParamField path="from" type="string" query>
  ISO 8601 时间戳。仅返回在此时间之后创建的事件。
</ParamField>

<ParamField path="to" type="string" query>
  ISO 8601 时间戳。仅返回在此时间之前创建的事件。
</ParamField>
...
| 事件类型 | 描述 |
|------------|-------------|
| `payment_link.created` | 创建了一个支付链接 |
| `payment_link.updated` | 更新了一个支付链接 |
| `payment_link.deleted` | 删除了一个支付链接 |
| `product.created` | 创建了一个产品 |
| `product.updated` | 更新了一个产品 |
| `product.deleted` | 删除了一个产品 |
| `webhook.created` | 创建了一个 webhook 端点 |
| `webhook.updated` | 更新了一个 webhook 端点 |
| `webhook.deleted` | 删除了一个 webhook 端点 |
| `discount_code.created` | 创建了一个折扣码 |
| `discount_code.updated` | 更新了一个折扣码 |
| `discount_code.deleted` | 删除了一个折扣码 |
| `widget.created` | 创建了一个小部件 |
| `widget.updated` | 更新了一个小部件 |
| `widget.deleted` | 删除了一个小部件 |
| `notification.updated` | 更新了通知设置 |
| `api_key.created` | 创建了一个 API 密钥 |
| `api_key.revoked` | 吊销了一个 API 密钥 |
...
<Note>
  事件保留 90 天。为了更长时间的保留，使用日期范围过滤器定期导出事件。
</Note>
