...
---
AnySpend 平台 API 为您提供对支付基础设施的完全编程控制。通过一个干净、类 Stripe 风格的 REST API 构建集成、自动化工作流程，并让 AI 代理管理您的商务堆栈。

## 这适用于谁？

- **开发者** 与 AnySpend 构建自定义集成
- **AI 代理** 创建支付链接、管理产品或拉取分析数据
...
在您的支付链接 URL 后附加 `client_reference_id` 和 `metadata`：
...
<Tip>
  请参阅 [Webhooks 指南](/anyspend/api/webhooks-guide#reconciling-payments-with-your-system) 了解在您的 webhook 处理程序中对支付进行对账的完整示例。
</Tip>
...
| 等级 | 需要认证 | 速率限制 | 使用场景 |
|------|--------------|------------|----------|
| **开放** | 无（基于 IP 的速率限制） | 每 IP 5 次/分钟 | 快速支付 -- 无需账户的一次性支付 |
| **API 密钥** | 通过头部的 `asp_xxx` 密钥 | 每密钥 120 次/分钟 | 所有标准 API 操作 -- 在支付链接、产品、交易等上进行 CRUD |
...
请参阅 [分页与过滤](/anyspend/api/pagination) 页面了解如何导航列表响应的详情。
...
请参阅 [错误](/anyspend/api/errors) 页面了解完整的错误分类。
...
为了在 `POST` 和 `PATCH` 请求上安全重试，请包含一个 `Idempotency-Key` 头。带有相同键和主体的重复请求返回缓存的响应。请参阅 [幂等性](/anyspend/api/idempotency) 页面了解详情。
...
<Step title="获取您的 API 密钥">
  前往您的 [AnySpend 仪表板](https://anyspend.com/dashboard) 在 **设置 > API 密钥** 下创建一个您需要的权限的密钥。
</Step>
