...
---
AnySpend 平台 API 使用 API 密钥进行身份验证。每个密钥都限定于单个组织，并携带特定的权限集。
...
<Step title="复制并存储密钥">
  完整的密钥在创建时只显示 **一次**。立即复制它并将其存储在安全的位置（例如，环境变量、密钥管理器）。
</Step>
</Steps>

<Warning>
  您的 API 密钥在创建时只显示一次。如果您丢失了它，您将需要撤销旧密钥并创建一个新的。
</Warning>
...
该前缀使您能够轻松识别代码库中的 AnySpend 密钥，并防止意外使用来自其他服务的密钥。
...
`Authorization: Bearer` 方法是首选，因为它遵循标准的 OAuth 2.0 约定，并且得到大多数 HTTP 客户端和 API 测试工具的支持。

## 权限等级

每个 API 密钥都被分配一个或多个权限。权限遵循层次结构，更高级别包括所有较低级别的能力。
...
如果路由需要 `write` 权限，而您的密钥只有 `read`，您将收到一个 `403` 错误：
...
如果您拥有 `admin` 权限的密钥，您可以以编程方式创建新密钥：
...
列表响应包括元数据，但从不显示完整的密钥 — 只显示 `key_prefix` 用于识别：
...
撤销的密钥会立即停止工作。任何使用被撤销密钥认证的在途请求将收到 `401` 错误。
...
快速支付每分钟每个 IP 地址限制 **5 次请求**。不需要 API 密钥。

## 密钥轮换最佳实践

定期轮换您的 API 密钥可以减少凭证泄露的风险。这是一个推荐的方法：

<Steps>
<Step title="创建新密钥">
  使用仪表盘或 API 创建一个与正在轮换的密钥具有相同权限的新密钥。
</Step>

<Step title="更新您的应用程序">
  将新密钥部署到应用程序的环境变量或密钥管理器中。
</Step>

<Step title="验证新密钥是否有效">
  通过检查 `last_used_at` 时间戳确认您的应用程序成功使用新密钥进行请求。
</Step>
...
<Tip>
  创建密钥时使用可选的 `expires_at` 字段来强制自动过期。这为您忘记轮换提供了安全网。
</Tip>
...
定期检查您的密钥的 `last_used_at`，以识别应该撤销的未使用密钥。未使用的活跃密钥是安全风险。
</Accordion>

<Accordion title="设置过期日期">
对于临时集成、CI/CD 令牌或承包商访问，始终设置 `expires_at` 时间戳，以便密钥自动变为非活动状态。
</Accordion>
</AccordionGroup>
...
| `401` | `key_expired` | API 密钥已超过其 `expires_at` 时间戳 |
| `401` | `key_revoked` | API 密钥已明确撤销 |
| `403` | `insufficient_permissions` | API 密钥没有所需的权限等级 |

请参阅 [错误](/anyspend/api/errors) 页面以获取完整的错误参考。
