...
---
Webhooks 允许您的服务器在您的 AnySpend 组织中发生事件时接收实时 HTTP 回调 —— 完成支付、结账过期等。您不需要轮询 API，而是注册一个 URL，AnySpend 将事件推送给您。
...
<Steps>
  <Step title="注册 webhook 端点">
    通过仪表板或 API 创建 webhook，指定 URL 和要订阅的事件。
  </Step>
  <Step title="AnySpend 发送事件">
    当订阅的事件发生时，AnySpend 向您的 URL 发送一个带有 JSON 载荷和签名头的 `POST` 请求。
  </Step>
  <Step title="验证和处理">
    您的服务器验证 HMAC-SHA256 签名，处理事件，并在 30 秒内以 `200` 状态响应。
  </Step>
  <Step title="自动重试">
    如果您的服务器没有以 `2xx` 状态响应，AnySpend 将进行最多 3 次的重试，每次重试间隔呈指数级增长。
  </Step>
</Steps>
...
| 事件 | 描述 |
|-------|-------------|
| `payment.completed` | 一笔支付成功接收并在链上确认 |
| `payment.failed` | 一次支付尝试失败（回滚、超时等） |
| `checkout.completed` | 一次结账会话由支付者完成 |
| `checkout.expired` | 结账会话在支付者完成前过期 |
...
<Tabs>
  ...
  <Tab title="Dashboard">
    在 [AnySpend 仪表板](https://app.anyspend.com) 中导航至 **设置 > Webhooks > 添加端点**。输入您的 URL，选择事件，然后点击 **创建**。签名密钥将显示一次 —— 立即复制它。
  </Tab>
</Tabs>
...
`payment.completed` webhook 总是包含一个 `checkoutSession` 块，其中包含 `clientReferenceId`、`metadata`、`customerEmail` 和 `customerName` —— 您需要的一切以将支付与您的内部记录匹配。
...
<Tip>
  `client_reference_id` 和 `metadata` 总是包含在 webhook 载荷中。您可以通过 [URL 参数](/anyspend/api/payment-links#url-parameters) 设置它们以进行简单集成，或通过 [Checkout Sessions API](/anyspend/api/checkout-sessions-api) 进行服务器端控制。
</Tip>
...
<Warning>
  在处理 webhook 之前，始终验证 `X-AnySpend-Signature` 头部。没有验证，攻击者可以向您的端点发送伪造的事件。
</Warning>
...
app.post(
  ...
  (req, res) => {
    ...
    const expectedSignature = crypto
      ...
      .update(req.body) // 当使用 express.raw() 时，req.body 是一个 Buffer
      .digest("hex");
...
function handlePaymentCompleted(data) {
  console.log(`支付 ${data.id} 完成，金额为 ${data.amount}`);
  // 履行订单，发送确认电子邮件，更新数据库等。
}

function handlePaymentFailed(data) {
  console.log(`支付 ${data.id} 失败`);
  // 通知客户，更新订单状态等。
}
...
def handle_webhook():
    ...
    if event["event"] == "payment.completed":
        print(f"支付 {event['data']['id']} 完成")
        # 履行订单...
    elif event["event"] == "payment.failed":
        print(f"支付 {event['data']['id']} 失败")
        # 处理失败...
...
如果您的端点在 **30 秒** 内没有响应 `2xx` 状态码，AnySpend 将标记传送失败并重试。
...
<Info>
  如果您的端点持续失败（10+ 连续失败的传送），webhook 将自动**禁用**，并且您将收到电子邮件通知。在解决问题后，从仪表板重新启用它。
</Info>
...
<Tabs>
  <Tab title="SDK">
    ...
    console.log(testResult.response_code);  // 您的服务器的 HTTP 状态
    console.log(testResult.success);        // 如果 2xx 响应则为 true
    ```
  </Tab>
  <Tab title="cURL">
    ```bash
...
<CardGroup cols={2}>
  ...
  <Card title="处理重复" icon="clone">
    使用 `X-AnySpend-Delivery-Id` 头部或 `data.id` 字段来去重事件。重试可能会多次传送相同的事件。
  </Card>
  <Card title="验证签名" icon="shield-halved">
    始终验证 `X-AnySpend-Signature` 头部。永远不要在没有验证的情况下信任载荷。
  </Card>
  <Card title="使用 HTTPS" icon="lock">
    Webhook URL 在生产中必须使用 HTTPS。在开发期间，HTTP URL 仅允许用于 `localhost`。
  </Card>
</CardGroup>
