...
<Warning>
  如果具有相同 `wallet_address` 的客户已存在，请求将返回 `409 Conflict` 错误。使用更新端点修改现有客户。
</Warning>
...
<Tip>
  创建后无法更改 `wallet_address`。要将客户与不同的钱包关联，请创建新的客户记录。
</Tip>
...
<Warning>
  此操作不可逆。所有个人身份信息（姓名、电子邮件）将被永久删除。
</Warning>
...
以 CSV 或 JSON 格式导出您的客户数据。响应以文件下载的形式流式传输。
...
<Info>
  导出限于 10,000 条记录。对于更大的数据集，请使用列表端点的分页功能。
</Info>
