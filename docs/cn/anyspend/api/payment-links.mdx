...
<Info>
  支付链接通过AnySpend协议自动处理代币路由、气体抽象化和跨链交换。
</Info>
...
<ResponseField name="url" type="string">
  可分享的结账URL（例如，`https://anyspend.com/pay/pl_abc123`）。
</ResponseField>
...
<ResponseField name="shipping_options" type="array | null">
  ...
  <Expandable title="shipping option properties">
    ...
    <ResponseField name="name" type="string">显示名称（例如，`"Standard Shipping"`）。</ResponseField>
    <ResponseField name="amount" type="string">以代币最小单位表示的运输成本。</ResponseField>
    <ResponseField name="estimated_days" type="string">预计交货时间（例如，`"5-7 business days"`）。</ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="branding" type="object | null">
  ...
  <Expandable title="branding properties">
    ...
    <ResponseField name="business_name" type="string">结账时显示的商家名称。</ResponseField>
  </Expandable>
</ResponseField>
...
分享支付链接URL时，您可以附加查询参数以预填充会话数据、跟踪归因并将支付与您的客户关联。
...
| `success_url` | 成功支付后覆盖重定向URL | `?success_url=https://example.com/thanks` |
...
所有参数都通过webhooks传递，因此您可以在后端对支付进行对账。

<Tip>
  对于需要完全控制的企业集成，请使用[Checkout Sessions API](/anyspend/api/checkout-sessions-api)在服务器端创建会话，包括`customer_email`、`customer_name`、`metadata`和`client_reference_id`，然后使用返回的`url`重定向或将`?session_id=`附加到您的支付链接。
</Tip>
...
<ParamField body="form_schema" type="object">
  结账时收集的自定义表单字段。结构请参见上方的支付链接对象。
</ParamField>

<ParamField body="shipping_options" type="array">
  实物商品的运输选项。结构请参见上方的支付链接对象。
</ParamField>
...
<ParamField body="items" type="array">
  ...
  <Expandable title="item properties">
    ...
    <ParamField body="items[].amount" type="string" required>以代币最小单位表示的项目价格。</ParamField>
