...
<ResponseField name="status" type="string">
  会话的当前状态。可能为：`open`、`processing`、`completed`、`expired`、`failed`。
</ResponseField>
...
| `processing` | 客户已启动支付；交易正在链上处理中。 |
| `completed` | 支付已确认并由接收方收到。 |
...
在服务器端创建具有唯一支付 URL 的结账会话。当您从后端控制结账流程时使用此方法进行程序化集成。

<ParamField body="payment_link_id" type="string">
  从现有支付链接创建会话。这继承了链接的配置。需要 `payment_link_id` 或手动字段（`token_address`、`chain_id`、`recipient_address`、`amount`）之一。
</ParamField>

<ParamField body="token_address" type="string">
  接收代币的合约地址。如果未提供 `payment_link_id`，则此项为必填。
</ParamField>

<ParamField body="chain_id" type="number">
  您希望接收支付的链 ID。如果未提供 `payment_link_id`，则此项为必填。
</ParamField>

<ParamField body="recipient_address" type="string">
  接收支付的钱包地址。如果未提供 `payment_link_id`，则此项为必填。
</ParamField>

<ParamField body="amount" type="string">
  以代币的最小单位表示的支付金额。如果未提供 `payment_link_id`，则此项为必填。
</ParamField>
...
<ParamField body="success_url" type="string">
  客户支付成功后重定向的 URL。支持 `{SESSION_ID}` 模板变量。
</ParamField>
...
<ParamField body="metadata" type="object">
  通过到 Webhooks 和结果交易的任意键值对。最多 50 个键，500 个字符值。
</ParamField>

<ParamField body="customer_email" type="string">
  客户电子邮件地址。在休息时加密，并在 webhook 负载中包含，用于订单履行。
</ParamField>

<ParamField body="customer_name" type="string">
  客户姓名。在休息时加密，并在 webhook 负载中包含，用于订单履行。
</ParamField>
...
const manualSession = await anyspend.checkoutSessions.create({
  ...
  chainId: 1,
  recipientAddress:典型的服务器端集成在您的后端创建结账会话，并将客户重定向到会话 URL：
...
<Warning>
  重定向后总是在服务器端验证会话状态。不要仅依赖重定向来确认支付——使用 webhooks 进行可靠的支付确认。
</Warning>

### 轮询会话完成

如果您更喜欢轮询而不是使用 webhooks，您可以定期检查会话状态：
...
async function waitForPayment(sessionId: string, timeoutMs = 300000) {
  ...
  throw new Error(`等待支付超时：${sessionId}`);
}
```

<Tip>
  对于生产用途，我们强烈推荐使用 [webhooks](/anyspend/api/webhooks) 而不是轮询。Webhooks 提供实时通知，不需要保持连接开放。
</Tip>
