...
---
产品代表您目录中的物品或服务。每个产品定义了定价、代币和链配置，这些配置可以在多个支付链接和结账会话中重用。

<Info>
  产品支持三种定价类型：`one_time`用于单次购买，`subscription`用于重复支付，以及`variable`用于客户选择的金额。
</Info>
...
<ParamField body="amount" type="string">
  代币最小单位的价格。`one_time`和`subscription`类型必需。
</ParamField>
...
所有来自[创建端点](#create-a-product)的body参数都被接受。此外：
...
<Warning>
  更新产品不会对从中创建的支付链接进行回溯更新。每个支付链接维护其自己的配置。
</Warning>
...
软删除产品。产品将被标记为非活跃状态，并且默认不再出现在列表结果中。使用此产品的现有支付链接将继续工作。
...
<Tip>
  删除产品是一种软删除。产品数据为现有交易和支付链接的历史参考而保留。
</Tip>
...
自动创建一个预配置了产品设置的支付链接。这是一种便捷方法，使用产品的`name`、`amount`、`token_address`、`chain_id`、`recipient_address`、`description`、`image_url`、`form_schema`和`shipping_options`创建一个新的支付链接。
...
任何额外的[支付链接字段](/anyspend/api/payment-links#create-a-payment-link)都可以作为覆盖传递。
...
<Tabs>
  <Tab title="Response">
    返回一个新的[支付链接对象](/anyspend/api/payment-links#the-payment-link-object)，其中`product_id`设置为源产品。
