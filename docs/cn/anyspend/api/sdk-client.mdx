...
<Info>
  您可以从 [AnySpend 仪表板](https://app.anyspend.com) 下的 **设置 > API 密钥** 中获取 API 密钥。密钥限定于您的组织，并且可以具有只读或读写权限。
</Info>
...
为了方便，每个 `list` 方法都有一个对应的 `listAutoPaginate` 帮助器，它返回一个异步迭代器。在您消费项目时，它会在后台获取页面。
...
try {
  ...
  if (err instanceof NotFoundError) {
    ...
    console.log("API 密钥缺乏所需权限");
  } else if (err instanceof RateLimitError) {
    console.log(`速率限制。在 ${err.retryAfter} 秒后重试`);
  } else if (err instanceof IdempotencyError) {
    console.log("幂等性冲突——请求已使用不同参数处理");
...
重试使用 **指数退避** 和抖动。默认是 3 次重试，可通过 `maxRetries` 选项配置。
...
<Warning>
  对于 `429` 以外的 `4xx` 错误，不执行重试，因为这些错误表明客户端问题，重试不会解决。
</Warning>

## 幂等性

所有的 `POST` 和 `PATCH` 请求自动包含一个 `Idempotency-Key` 头。这确保了如果请求被重试（例如，由于网络错误），服务器不会处理它两次。
...
<Tip>
  建议对关键操作使用确定性幂等性密钥（如订单 ID）。如果您意外地发送了相同的请求两次，第二次调用会返回原始响应而不是创建重复项。
</Tip>

## TypeScript 类型

所有请求和响应类型都被导出，供您在自己的代码中使用：
...
客户端在内部使用标准的 `fetch` API。在 Node.js 18+ 中，使用内置的 `fetch`。不需要 polyfills。
