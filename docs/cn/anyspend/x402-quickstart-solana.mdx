...
<Card title="实时演示" icon="rocket" href="https://github.com/b3-fun/anyspend-x402-demo">
  完整的 Solana 演示应用
</Card>
</CardGroup>
...
本指南展示了如何在 **Solana** 上使用 **AnySpend X402**，适用于服务器端（接受支付）和客户端（进行支付）应用。Solana 上的 AnySpend X402 通过使用交易签名来实现无 Gas 费用的 SPL 代币支付，非常适合付费 API、AI 代理和高级服务。

**关键特性：**
- **无 Gas 交易** - 由中介方赞助所有交易费用
- **SPL 代币支持** - 在 Solana 上用 USDC 和其他 SPL 代币支付
- **浏览器钱包集成** - 支持 Phantom、Solflare、Ledger 等
- **对 AI 代理友好** - 用于自主支付的简单 fetch 封装
...
**这样做的目的：**
1. 客户端请求端点
2. 中间件返回 `402 Payment Required` 与 Solana 支付详情
3. 客户端签署一个包含 USDC 转账的 Solana 交易
4. 中介方验证并提交交易（无 Gas 费用）
5. 支付确认后执行您的路由处理程序
...
根据您的用例，有两种在 Solana 上进行支付的方式。

### 选项 1：使用 Fetch（AI 代理 & 后端）

适用于 Node.js 脚本、AI 代理和服务器端应用。
...
适用于使用 Phantom、Solflare、Ledger 或其他 Solana 钱包的 Web 应用。
...
- 您是否处于正确的网络（mainnet vs devnet）
- 中介方 URL 是否正确
</Accordion>

<Accordion title="钱包适配器不工作">
请确保：
- 您已使用 `WalletProvider` 包装您的应用
...
- **实时演示**：在 [x402-demo.anyspend.com](https://x402-demo.anyspend.com/) 上亲自尝试
