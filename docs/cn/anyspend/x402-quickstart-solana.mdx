...
本指南展示了如何在 **Solana** 上使用 **AnySpend X402**，适用于服务器（接受支付）和客户端（进行支付）应用。Solana 上的 AnySpend X402 通过使用交易签名实现无需支付燃料费的 SPL 代币支付，非常适合付费 API、AI 代理和高级服务。

**主要特性：**
- **无需支付燃料费的交易** - 由促进者赞助所有交易费用
- **支持 SPL 代币** - 在 Solana 上使用 USDC 和其他 SPL 代币支付
- **浏览器钱包集成** - 支持 Phantom、Solflare、Ledger 等
- **对 AI 代理友好** - 用于自动支付的简单 fetch 封装
...
app.listen(3001, () => {
  console.log('Solana X402 服务器在 3001 端口运行');
});
```

**这样做的目的：**
1. 客户端请求端点
2. 中间件返回 `402 Payment Required` 和 Solana 支付详情
...
<CardGroup cols={2}>
<Card title="X402 概述" icon="book" href="/anyspend/x402-overview">
