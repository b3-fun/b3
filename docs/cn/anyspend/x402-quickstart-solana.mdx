...
本指南展示了如何在 **Solana** 上为服务器（接受支付）和客户端（进行支付）应用使用 **AnySpend X402**。Solana 上的 AnySpend X402 通过交易签名启用无 gas SPL 代币支付，非常适合付费 API、AI 代理和高级服务。

**关键特性：**
- **无 gas 交易** - 由促进者赞助所有交易费用
- **SPL 代币支持** - 在 Solana 上使用 USDC 和其他 SPL 代币支付
- **浏览器钱包集成** - 支持 Phantom、Solflare、Ledger 等
- **AI 代理友好** - 用于自主支付的简单 fetch 包装器
