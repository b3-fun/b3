...
本指南展示了如何使用 **AnySpend x402 客户端 SDK** 通过支持 EIP-2612 或 EIP-3009 的兼容代币支付付费墙资源。SDK 自动处理整个支付流程，包括签名生成和重试逻辑。

您可以使用同一个客户端支付任何启用了 x402 的服务，不仅限于使用 AnySpend 的服务。
...
- **一个加密钱包**，内含兼容的代币（USDC、DAI 或其他 EIP-2612/EIP-3009 代币）
- **一个钱包库**，如 [viem](https://viem.sh) 或 [ethers.js](https://docs.ethers.org)
- **访问 x402 启用的服务**（如付费 API）

<Note>
**重要**：只有支持 EIP-2612（Permit）或 EIP-3009（TransferWithAuthorization）的代币才与 AnySpend x402 兼容，用于无燃料费支付。使用 [代币兼容性检查器](https://anyspend.com/x402-tokens) 验证您的代币是否受支持。
</Note>
...
1. 客户端发起初始请求
2. API 返回 `402 Payment Required` 并附带支付详情
...
const tokenAddress = '0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913'; // Base 上的 USDC
...
如果资源服务器支持 AnySpend 中间件，您将以偏好代币等价金额支付，而不是 USDC。

### 使用 HTTP 头部指定支付偏好

<Note>
**X402 新增**：您现在可以使用 HTTP 头部指定支付偏好，这在直接进行 HTTP 请求或与现有 HTTP 客户端集成时特别有用。
</Note>

向 X402 启用的服务发起请求时，您可以使用这些 HTTP 头部指定您偏好的支付代币和网络：
...
**常见代币地址：**
- **Base 上的 USDC**：`0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913`
- **Base 上的 USDT**：`0xfde4C96c8593536E31F229EA8f37b2ADa2699bb2`
...
**只有支持 EIP-2612 或 EIP-3009 的代币** 才与 AnySpend x402 兼容，用于无燃料费支付。

### 什么是 EIP-2612 和 EIP-3009？

- **EIP-2612（Permit）**：允许通过离线签名进行无燃料费代币授权
- **EIP-3009（TransferWithAuthorization）**：通过离线签名启用无燃料费转账（USDC 使用）

### 常见兼容代币

- **USDC** - 所有链上均支持 EIP-3009
- **B3** - 支持 EIP-2612 permit
- **许多其他代币** - 使用下面的工具检查兼容性

<Card title="代币兼容性检查器" icon="check-circle" href="https://anyspend.com/x402-tokens">
  浏览支持 EIP-2612 或 EIP-3009 的所有兼容代币，覆盖 19+ 网络
</Card>

查看 [网络支持](/anyspend/x402-network-support) 了解每个链上的详细代币地址。
...
export function PremiumDataFetcherAdvanced() {
  ...
  const tokenAddresses: Record<number, string> = {
    8453: '0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913', // Base 上的 USDC
    1: '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48', // 以
