...
如果资源服务器支持 AnySpend 中间件，您将以偏好代币而非 USDC 支付等价金额。
...
当向 X402 启用的服务发起请求时，您可以使用这些 HTTP 头部指定您偏好的支付代币和网络：
...
这些头部适用于任何 HTTP 客户端，并且 X402 启用的资源服务器使用 AnySpend 中间件时会自动识别。如果没有提供头部，服务器将默认使用 USDC。
...
- **EIP-2612 (Permit)**：通过离线签名允许无 Gas 代币授权
