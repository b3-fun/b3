...
本指南展示了如何使用 **AnySpend x402 客户端 SDK** 通过支持 EIP-2612 或 EIP-3009 的兼容代币支付需要付费的资源。SDK 自动处理整个支付流程，包括签名生成和重试逻辑。

x402 的美妙之处在于，您可以使用这个相同的客户端支付**任何启用了 x402 的服务**，而不仅仅是使用 AnySpend 的服务。
...
如果资源服务器支持 AnySpend 中间件，您将以首选代币支付等价金额，而不是 USDC。
...
当向启用了 X402 的服务发起请求时，您可以使用这些 HTTP 头部指定您的首选支付代币和网络：
...
function wrapFetchWithPayment(
  ...
  maxPaymentValue: string,             // 最大支付额（最小单位，例如，'1000000' 表示 1 USDC）
