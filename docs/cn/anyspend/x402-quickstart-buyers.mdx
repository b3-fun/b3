...
x402 的美妙之处在于，您可以使用同一个客户端支付**任何启用 x402 的服务**，而不仅仅是使用 AnySpend 的服务。
...
- **一个加密钱包**，内含兼容代币（USDC、DAI 或其他 EIP-2612/EIP-3009 代币）
...
为了更多控制权，使用更底层的 `wrapFetchWithPayment` 函数：
...
如果资源服务器支持 AnySpend 中间件，您将以偏好代币而不是 USDC 支付等值金额。
...
向 X402 启用的服务发起请求时，您可以使用以下 HTTP 头部指定偏好的支付代币和网络：
...
查看 [网络支持](/anyspend/x402-network-support) 了解每个链上详细的代币地址。
...
export function PremiumDataFetcherAdvanced() {
  ...
  const tokenAddresses: Record<number, string> = {
    ...
    1: '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48', // 以太坊上的 USDC
  };
