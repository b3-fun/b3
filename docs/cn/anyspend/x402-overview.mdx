...
- **使用兼容代币支付**：使用任何支持 EIP-2612 或 EIP-3009 的代币进行无需燃料费的支付
...
<Card title="代币兼容性检查器" icon="check-circle" href="https://anyspend.com/x402-tokens">
  浏览支持 EIP-2612 或 EIP-3009 的兼容代币，覆盖 19+ 个网络
</Card>
...
```
客户端使用 DAI (EIP-2612) 支付 → AnySpend 换成 1 USDC → 卖家收到 1 USDC
```
...
使用 AnySpend 的客户端**完全不知道**他们正在使用一个特殊的服务提供者。整个流程使用标准 x402 协议消息 - 多代币魔法在资源服务器端通过中间件透明地发生。
...
<Card title="客户端 SDK" icon="code">
  **anyspend-x402-client**
  ...
  平台无关的 TypeScript SDK，用于客户端进行支付，包含签名助手
</Card>
</CardGroup>

## 由 AnySpend 服务提供者提供支持

所有 x402 支付都通过 **AnySpend 服务提供者** 处理 - 一个生产级支付基础设施，处理：
...
<Card title="了解更多关于服务提供者的信息" icon="server" href="/anyspend/x402-facilitator">
  探索 AnySpend 服务提供者的全部功能、API 端点和架构
</Card>
