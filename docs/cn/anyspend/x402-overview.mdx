...
AnySpend x402 是 B3 对 **x402 支付协议**的实现，该协议支持通过 HTTP 直接进行即时、自动的加密货币支付。与仅支持 USDC 的传统 x402 服务提供者不同，**AnySpend 允许您使用任何兼容的代币**（EIP-2612 或 EIP-3009）支付，而资源服务器可以接收他们偏好的任何代币。

x402 协议使 HTTP `402 Payment Required` 状态码现代化，将其转变为机器对机器微支付、付费 API 和自治代理交互的实际解决方案。
...
- **使用兼容代币支付**：使用任何支持 EIP-2612 或 EIP-3009 的代币进行无燃料支付
...
<Card title="代币兼容性检查器" icon="check-circle" href="https://anyspend.com/x402-tokens">
  浏览跨 19+ 网络支持 EIP-2612 或 EIP-3009 的兼容代币
</Card>
...
在您的客户用他们偏好的代币支付时，接受 USDC 支付：

- **接收 USDC**：无论客户使用什么代币，都以 USDC 获得支付
...
<Card title="我想支付服务" icon="wallet" href="/anyspend/x402-quickstart-buyers">
  **对于买家**：了解如何使用 x402 客户端 SDK 用任何代币支付付费资源
</Card>

<Card title="我想接受支付" icon="cash-register" href="/anyspend/x402-quickstart-sellers">
  **对于卖家**：将 Express 中间件添加到您的 API 并开始接受多代币支付
</Card>
</CardGroup>
...
<Card title="客户端 SDK" icon="code">
  **anyspend-x402-client**
  ...
  平台无关的 TypeScript SDK，用于客户端进行支付并提供签名助手
</Card>
</CardGroup>

## 由 AnySpend Facilitator 提供支持

所有 x402 支付都通过 **AnySpend Facilitator** 处理 - 一个生产级支付基础设施，处理：
