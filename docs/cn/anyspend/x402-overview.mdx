...
AnySpend x402 是 B3 实现的 **x402 支付协议**，它支持通过 HTTP 直接进行即时、自动的加密货币支付。与仅支持 USDC 的传统 x402 服务提供商不同，**AnySpend 允许您使用任何兼容的代币**（EIP-2612 或 EIP-3009）支付，而资源服务器可以接收他们偏好的任何代币。

x402 协议使 HTTP `402 Payment Required` 状态码现代化，将其转变为机器对机器微支付、付费 API 和自主代理交互的实用解决方案。
...
- **使用兼容代币支付**：使用任何支持 EIP-2612 或 EIP-3009 的代币进行无燃料费支付
- **标准协议**：使用标准 HTTP 与任何支持 x402 的服务工作
...
<Card title="代币兼容性检查器" icon="check-circle" href="https://anyspend.com/x402-tokens">
  浏览 19+ 网络上支持 EIP-2612 或 EIP-3009 的兼容代币
</Card>
...
**AnySpend x402**：客户端可以使用他们持有的**任何兼容代币**（需要 EIP-2612 或 EIP-3009 支持）。AnySpend 处理转换。
...
AnySpend 的服务提供商处理所有区块链操作，并为验证和结算提供简单的 HTTP 端点。

### 标准协议合规

使用 AnySpend 的客户端不需要知道他们正在使用一个特殊的服务提供商。流程使用标准 x402 协议消息，多代币转换在资源服务器端通过中间件发生。
...
<Card title="客户端 SDK" icon="code">
  **anyspend-x402-client**
  ...
  平台无关的 TypeScript SDK，用于客户端进行支付，附带签名助手
</Card>
</CardGroup>

## 由 AnySpend 服务提供商提供支持

所有 x402 支付都通过 AnySpend 服务提供商处理，该服务提供商处理：

- 跨 19+ 区块链网络的多代币支持
- 自动代币兑换和跨链路由
- 用户的无燃料费支付
- 符合标准的 x402 实现
- 任何 x402 服务提供商的即插即用替代品

**生产端点：** `https://mainnet.anyspend.com/x402`

<Card title="AnySpend 服务提供商" icon="server" href="/anyspend/x402-facilitator">
  API 端点、架构和配置
</Card>
