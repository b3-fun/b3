...
AnySpend x402 是 B3 实现的 **x402 支付协议**，它支持通过 HTTP 直接进行即时、自动的加密货币支付。与仅支持 USDC 的传统 x402 服务提供者不同，**AnySpend 允许您使用任何兼容的代币**（EIP-2612 或 EIP-3009）支付，而资源服务器可以接收他们偏好的任何代币。

x402 协议使 HTTP `402 Payment Required` 状态码现代化，将其转变为机器对机器微支付、付费墙 API 和自治代理交互的实用解决方案。
...
- **使用兼容代币支付**：使用任何支持 EIP-2612 或 EIP-3009 的代币进行无需燃料的支付
- **标准协议**：使用标准 HTTP 与任何支持 x402 的服务兼容
...
<Card title="代币兼容性检查器" icon="check-circle" href="https://anyspend.com/x402-tokens">
  浏览支持 EIP-2612 或 EIP-3009 的兼容代币，跨越 19+ 个网络
</Card>
...
- **Express 中间件**：仅需几行代码即可将支付添加到您的 API
...
<Card title="我想支付服务" icon="wallet" href="/anyspend/x402-quickstart-buyers">
  **对买家**：了解如何使用 x402 客户端 SDK 用任何代币支付付费墙资源
</Card>
...
```
客户端用 DAI (EIP-2612) 支付 → AnySpend 兑换为 1 USDC → 卖家收到 1 USDC
```
...
使用 AnySpend 的客户端**完全不知道**他们正在使用一个特殊的服务提供者。整个流程使用标准的 x402 协议消息 - 多代币魔法在资源服务器端通过中间件透明地发生。
...
<Card title="服务器 SDK" icon="server">
  **@b3dotfun/anyspend-x402**
  ...
  通用服务器中间件，支持 Express、Next.js 和 Hono，使资源服务器能够轻松接受多代币支付
</Card>
...
<Card title="了解更多关于 Facilitator 的信息" icon="server" href="/anyspend/x402-facilitator">
  探索 AnySpend Facilitator 的全部功能、API 端点和架构
</Card>
```
