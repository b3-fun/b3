...
<CardGroup cols={2}>
<Card title="插槽" icon="puzzle-piece">
  用您自己的组件替换整个 UI 部分
</Card>
...
插槽允许您用自己的 React 组件替换整个 UI 部分。每个插槽都从 AnySpend 接收属性，您可以在自定义实现中使用这些属性。
...
<Tip>
  `checkoutForm`、`shippingSelector` 和 `discountInput` 插槽仅在 `<AnySpendCheckout>` 和 `<AnySpendCheckoutTrigger>` 中使用。有关表单/配送/折扣功能的详细信息，请参阅[结账指南](/anyspend/checkout)。
</Tip>
    ...
    connectWalletButton: ({ onPayment, txLoading, connectedAddress, paymentLabel }) => (
      <div className="my-wallet-section">
        {connectedAddress ? (
          ...
          <button onClick={onPayment}>连接并支付</button>
        )}
      </div>
    ),
  }}
...
<AnySpendCheckout
  ...
  content={{
    successTitle: "订单已确认！",
    successDescription: "您的订单正在准备中。检查您的电子邮件以获取配送更新。",
    failureTitle: "支付失败",
    failureDescription: "您的卡未被扣费。请重试或使用其他支付方式。",
...
interface AnySpendTheme {
  /** 主要品牌颜色（十六进制）。作为主要的强调色应用。 */
  brandColor?: string;
  /** 个别颜色覆盖（十六进制值） */
  colors?: Partial<{
    primary: string;         // 主文本颜色
    secondary: string;       // 次要文本颜色
    tertiary: string;        // 静音文本颜色
    surfacePrimary: string;  // 主背景颜色
    surfaceSecondary: string; // 次要背景（卡片、面板）
    brand: string;           // 强调/品牌颜色（按钮、链接）
    borderPrimary: string;   // 主边框颜色
...
<Note>
  `brandColor` 属性是一个快捷方式，用于设置 `brand` 颜色。如果同时提供了 `brandColor` 和 `colors.brand`，则 `colors.brand` 优先。
</Note>
...
将自定义 CSS 类应用于 AnySpend 组件内的特定元素。每个组件都有自己的类接口。
...
<AccordionGroup>
<Accordion title="布局与结构" defaultOpen>
```typescript
...
```typescript
{
  ...
  cartItemRow?: string;             // 各个项目行
  cartItemImage?: string;           // 项目图片
  cartItemName?: string;            // 项目名称
  cartItemDescription?: string;     // 项目描述
  cartItemPrice?: string;           // 项目价格
...
```typescript
{
  ...
  formField?: string;          // 各个表单字段
  formFieldLabel?: string;     // 字段标签
  formFieldInput?: string;     // 字段输入
  shippingSelector?: string;   // 配送选项选择器
