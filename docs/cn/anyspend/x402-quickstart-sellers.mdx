...
本指南展示了如何为您的 Express API 添加 **AnySpend x402** 支付支持，使您能够接受任何代币的支付，同时收到您选择的代币。只需几行代码，您的 API 就可以对端点进行货币化，并处理自动支付验证和结算。
...
- 安装了 **Node.js 18+**
- 有 **一个 Express.js API**（或愿意创建一个）
- 有 **一个钱包地址** 用于接收 USDC 支付
- 对 Express 中间件有 **基本了解**
...
要求在特定网络上使用特定代币。您可以接收 **任何代币**，不仅仅是 USDC！
...
const endpointConfig = {
  'POST /api/premium': {
    ...
    network: NETWORK,
    config: {
      description: '访问来自 CoinGecko 的高级 ETH 价格历史数据',
      mimeType: 'application/json',
    },
  }
};
