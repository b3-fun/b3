...
本指南展示了如何为您的Express API添加 **AnySpend x402** 支付支持，使您能够接受任何代币的支付，同时收到您选择的代币。仅需几行代码，您的API就可以实现端点的货币化，并处理自动支付验证和结算。
...
app.use(paymentMiddleware(
  recipientAddress,
  endpointConfig,
  facilitator                    // 预配置的Anyspend facilitator
));
...
const endpointConfig = {
  'POST /api/premium': {
    price: {
      amount: '100000000000000000000', // 100 B3代币（18位小数）
      asset: {
...
- 买家仍然可以使用**任何代币**支付（ETH、USDC、DAI 等）
- AnySpend 将他们的支付转换为**B3代币**
- 您的钱包准确收到**100个B3代币**
- 为买家和卖家提供完全的灵活性！
...
`@b3dotfun/anyspend-x402` 包包含一个预配置的facilitator实例：
...
在特定网络上要求特定代币。您可以接收**任何代币**，不仅仅是USDC！
...
const endpointConfig = {
  'POST /api/premium': {
    price: {
      amount: '100000000000000000000', // 100 B3代币（100 * 10^18）
      asset: {
        address: '0xB3B32F9f8827D4634fE7d973Fa1034Ec9fdDB3B3' as Address,
        decimals: 18,
        eip712: {
    ...
    network: NETWORK,
    config: {
      description: '从CoinGecko访问高级ETH价格历史数据',
      mimeType: 'application/json',
    },
  }
};
...
const endpointConfig = {
  '/api/ai-inference': {
    amount: '1000000000000000000', // 1 DAI (18位小数)
    asset: '0x50c5725949A6F0c72E6C4a641F24049A917DB0Cb', // Base上的DAI
    network: 'base-mainnet'
  }
};
```
...
const endpointConfig = {
  '/api/compute': {
    amount: '500000000000000', // 0.0005 ETH (18位小数)
    asset: '0x0000000000000000000000000000000000000000', // 原生ETH
    network: 'base-mainnet'
  }
};
```
...
const endpointConfig = {
  '/api/data': {
    price: {
      amount: '1000000000000000000', // 1代币（调整小数位）
      asset: {
...
要配置自定义代币，您需要知道其EIP-712参数。以下是查找它们的方法：
...
const endpointConfig = {
  '/api/data': {
    amount: '1.00',
    asset: 'USD',
    flexible: true       // 接受 X-PREFERRED-TOKEN 头
  }
};
```
