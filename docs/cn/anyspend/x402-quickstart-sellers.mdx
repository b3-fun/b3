...
本指南展示了如何为您的 Express API 添加 **AnySpend x402** 支付支持，使您能够接受任何代币的支付，同时接收您选择的代币。通过几行代码，您的 API 可以实现端点的货币化，并处理自动支付验证和结算。
...
- 安装了 **Node.js 18+**
- 有一个 **Express.js API**（或愿意创建一个）
- 有一个 **钱包地址** 用于接收 USDC 支付
- 对 Express 中间件有 **基本了解**
...
要求在特定网络上接收特定代币。您可以接收 **任何代币**，不仅仅是 USDC！
...
const endpointConfig = {
  '/api/data': {
    ...
    asset: '0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913', // USDC 在 Base 上
...
const endpointConfig = {
  '/api/ai-inference': {
    ...
    asset: '0x50c5725949A6F0c72E6C4a641F24049A917DB0Cb', // DAI 在 Base 上
    network: 'base-mainnet'
  }
};
```
