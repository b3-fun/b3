...
本指南展示了如何为您的 Express API 添加 **AnySpend x402** 支付支持，使您能够接受任何代币的支付，同时收到您选择的代币。通过几行代码，您的 API 可以实现端点的货币化，并处理自动支付验证和结算。
...
- 安装了 **Node.js 18+**
- 拥有 **一个 Express.js API**（或愿意创建一个）
- 拥有 **一个钱包地址** 以接收 USDC 支付
- 具备 **Express 中间件的基础知识**
...
const endpointConfig = {
  '/api/premium-data': {
    amount: '1.00',        // USDC 中的 $1.00
    asset: 'USD'           // 将转换为 USDC
  },
  '/api/ai-inference': {
    amount: '0.50',        // USDC 中的 $0.50
    asset: 'USD'
  }
};
