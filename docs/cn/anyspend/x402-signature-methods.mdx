...
AnySpend x402 使用加密签名而不是传统的令牌授权，为用户启用**无需燃气费的支付**。服务商支付所有燃气费，而用户只需签署授权消息。
...
直接转账授权 - 签名授权从发送方到接收方的即时转账，无需单独的批准步骤。
...
基于签名的批准，设置允许额度，然后进行单独的 `transferFrom()` 调用。这是大多数现代 ERC-20 令牌的标准方法。

**使用者：** 支持 permit 的大多数现代 ERC-20 令牌
...
服务商调用 `permit()` 在链上设置允许额度
</Step>

<Step title="Nonce 自动增加">
令牌合约自动增加用户的 nonce
</Step>

<Step title="服务商转账令牌">
服务商调用 `transferFrom()` 使用批准转账令牌
</Step>
</Steps>
...
**EIP-2612（permit）：**
- 大多数现代 ERC-20s
- USDT（在一些链上 - Base、Arbitrum、Optimism）
- 许多 DeFi 令牌
...
<Note>
AnySpend x402 客户端会根据令牌合约自动检测使用哪种签名方法。您不需要手动指定。
</Note>
...
- 永远不要使用 `type(uint256).max` 来进行无限批准
