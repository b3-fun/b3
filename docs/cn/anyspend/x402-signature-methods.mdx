...
<Steps>
<Step title="用户签署授权">
用户签署一个 EIP-712 消息，授权使用随机随机数进行转账
</Step>

<Step title="服务商执行转账">
服务商调用 `receiveWithAuthorization()` 并使用签名
</Step>

<Step title="USDC 被转账">
USDC 直接从用户转账给服务商，一次性交易完成
</Step>
...
基于签名的批准，设置一个额度，然后进行单独的 `transferFrom()` 调用。这是大多数现代 ERC-20 代币的标准方法。
...
<Note>
AnySpend x402 客户端会根据代币合约自动检测使用哪种签名方法。您不需要手动指定此信息。
</Note>
