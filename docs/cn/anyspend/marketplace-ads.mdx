...
<Tabs>
  ...
  <Tab title="文章提交">
    <Steps>
      ...
      <Step title="AI 验证">
        立即获得 AI 反馈，包括评分（0-100）和建议
      </Step>
      <Step title="审查 & 提交">
        如果获批准（评分 ≥70），则使用一个 x402 签名提交
      </Step>
      <Step title="跟踪进度">
        监控状态并在发布时获得通知
      </Step>
...
<Note>
  需要在 Base 上使用 USDC？使用 [Base 桥](https://bridge.base.org) 从以太坊转移
</Note>
...
<Steps>
  ...
  <Step title="AI 验证（免费）">
    AI 评分您的内容（0-100）并在需要时生成建议
  </Step>
  <Step title="获得批准">
    - **评分 ≥90：** 您的输入完美，按原样接受
    - **评分 70-89：** 获批准，附带 AI 改进的建议
    - **评分 &lt;70：** 拒绝 - 尝试不同的主题
  </Step>
  <Step title="提交 & 支付">
    使用一个 x402 签名提交您的文章请求
  </Step>
  <Step title="发布">
...
价格因网站而异，通常为 **每篇文章 10-50 USDC**。每个网站根据流量和受众设定自己的费率。
...
返回此广告位中当前运行的广告，或指示广告位是否可用。此端点免费调用。
...
跟踪您所有的广告预订 - 活跃的、待处理的和已完成的。此端点免费调用。
