...
1. **你的应用生成一个存款URL**，包含接收地址、链和代币。
2. **用户连接他们的钱包**（MetaMask、Coinbase、Rainbow、Rabby、Trust Wallet）。
3. **用户从他们的可用余额中选择一个代币**。
4. **AnySpend处理支付**，如有需要进行跨链转换至目的代币。
5. **完成** -- 可选地将用户重定向回你的应用。
...
| `toChainId` | 是* | 目的链ID（例如，Base的8453） |
| `toCurrency` | 是* | 目的链上的代币合约地址 |
| `amount` | 否 | 以wei/最小单位固定金额。提供时，用户不能更改金额 |
...
要注册为合作伙伴，请联系AnySpend团队添加你的配置。

## 安全性

- 仅允许HTTPS重定向URL（防止开放重定向攻击）
- 金额必须是有效的正整数（wei）
- 处理前验证地址
