...
当在达到目标后调用`migrateToDex()`：

1. **合约状态**：代币状态变更为`TokenStatus.Dex`
...
**SDK助手（可选）：**
SDK提供了`BondkitSwapService`以简化集成（如果需要）。
...
Uniswap v4上的Bondkit代币包括用于增强功能的自定义钩子：
...
```typescript
// 迁移前：直接控制
await token.buy(amount, minOut);    // ✅ 工作
await token.migrateToDex();         // 一次性动作

// 迁移后：基于市场
await token.buy(amount, minOut);    // ❌ 回退
await swapService.executeSwap(...); // ✅ 改用此
```
...
<CardGroup cols={2}>
  <Card title="开始DEX交易" icon="arrow-right" href="/bondkit/sdk/reference#bondkitswapservice">
    学习如何使用BondkitSwapService进行DEX交易
  </Card>
