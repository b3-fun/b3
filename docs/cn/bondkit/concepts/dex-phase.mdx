---
title: DEX 阶段
description: 在迁移到 Uniswap v4 后进行交易：交换、流动性和去中心化交易。
lang: cn
...
当达到目标后调用`migrateToDex()`：

1. **合约状态**：代币状态变为`TokenStatus.Dex`
2. **Uniswap v4池**：创建新池并累积流动性
3. **价格初始化**：初始池价格根据债券曲线退出价格计算
4. **所有权放弃**：合约所有权转移给零地址
5. **交易启用**：完整DEX交易立即开始
...
const poolConfig = {
  ...
  fee: 3000,        // 0.3%标准费用
  tickSpacing: 60,  // 标准tick间距
  hooks: customHook // Bondkit特定的钩子
};
```

**初始流动性：**
- **交易代币**：债券阶段累积的所有资金
- **Bondkit代币**：计算数量以匹配退出价格
- **LP代币**：在费用接收者和燃烧/分配之间拆分
...
**SDK助手（可选）：**
SDK提供`BondkitSwapService`以简化集成（如有需要）。
...
// 迁移的初始流动性
const migrationData = {
  tradingTokenLiquidity: "10.0",    // 债券阶段的ETH/B3
  bondkitTokenLiquidity: "500000",  // 计算的代币数量
  initialPrice: "0.00002",          // 每个代币的价格
  totalValueLocked: "20.0"          // 组合的TVL
};
...
<AccordionGroup>
  ...
  <Accordion title="未使用的债券曲线流动性会怎样？">
    债券阶段累积的所有交易代币成为Uniswap v4池的初始流动性，与按退出价格计算的bondkit代币配对。
  </Accordion>
  
  <Accordion title="DEX阶段有限制吗？">
    没有限制 - 代币成为标准ERC20，在Uniswap v4上交易。完全启用DeFi可组合性。
  </Accordion>
  
  <Accordion title="交易费用有何不同？">
