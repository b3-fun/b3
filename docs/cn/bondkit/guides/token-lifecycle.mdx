---
title: 完整的Token生命周期
description: 端到端指南：从代币创建、通过绑定阶段，到完整的 DEX 交易和去中心化。
lang: cn
originalPath: bondkit/guides/token-lifecycle.mdx
---
## 概览

本指南将带您完整了解Bondkit令牌的生命周期，从初始创建到完全去中心化。理解此流程对于成功的令牌启动和管理至关重要。
...
<AccordionGroup>
  <Accordion title="选择您的参数">
    **关键决策：**
    ...
    | 参数 | 影响 | 建议 |
    |-----------|---------|----------------|
    | **finalTokenSupply** | 总共创建的令牌数量 | 通常为1M-100M令牌 |
    | **aggressivenessFactor** | 价格曲线陡峭度 | 30-60，平衡启动 |
    | **targetAmount** | 迁移阈值 | 5-50交易令牌 |
    | **tradingToken** | 用户支付使用的令牌 | 使用B3用于Base主网 |
    ...
    **示例配置：**
    ...
    const tokenConfig = {
      name: "MyToken",
      symbol: "MTK",
      finalTokenSupply: parseEther("10000000"), // 10M令牌
      aggressivenessFactor: 45, // 中等曲线
      targetAmount: parseEther("20"), // 20 B3令牌
...
console.log(`✅ 令牌部署成功: ${tokenAddress}`);
```
...
<Tabs>
  <Tab title="社区建设">
    ...
    **启动日：**
    ...
    // 初始购买以确定价格
    await token.buy(parseEther("1"), 0n); // 1 B3令牌
    
    // 使用真实数据宣布启动
    const price = await token.getCurrentPrice();
    const progress = await token.getBondingProgress();
    
    console.log(`🚀 现场价格 ${formatEther(price)} B3每令牌`);
    ...
    **关键指标仪表板：**
    ...
    async function getDashboardData(token) {
      ...
      return {
        ...
        // 状态
        phase: status === 1 ? '绑定' : status === 2 ? 'DEX' : '其他'
      };
    }
  ...
  <Tab title="社区参与">
    **实时更新：**
    ...
    if (progress.progress >= 0.5 && progress.progress < 0.6) {
      postToDiscord("🎯 50%资金筹集！迁移的一半路程！");
    } else if (progress.progress >= 0.9) {
      postToDiscord("🔥 90%资金筹集！迁移即将到来！");
    }
    ```
  </Tab>
</Tabs>
  ...
  <Accordion title="最佳时机">
    ...
    **迁移前公告：**
    ...
    const announcement = `
      ...
      • 分发令牌：${formatEther(totalSupply)}
      
      🔄 迁移将创建：
      • 初始LP：${formatEther(migrationData.ethForLp)} B3 + 令牌
      • 开盘价格：~${calculateOpeningPrice(migrationData)} B3/令牌
...
if (isSwapAvailable) {
  ...
  console.log(`DEX价格：${quote.executionPrice} B3每令牌`);
}
```
  ...
  <Tab title="流动性管理">
    ...
    **流动性激励计划：**
    ...
    const lpIncentives = {
      ...
      dailyRewards: parseEther("1000"), // 每天1000令牌
    ...
    **教育内容：**
    ...
    const migrationGuideForUsers = {
      trading: {
        ...
        walletSetup: "将令牌添加到MetaMask，使用Uniswap界面"
      },
