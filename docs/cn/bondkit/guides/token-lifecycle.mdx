...
本指南将带您了解Bondkit令牌的完整生命周期，从初始创建到完全去中心化。理解这一流程对于成功的令牌启动和管理至关重要。
...
<AccordionGroup>
  <Accordion title="选择您的参数">
    **关键决策:**
    ...
    | **tradingToken** | 用户支付什么 | 使用B3用于Base主网 |
...
<Tabs>
  <Tab title="社区建设">
    ...
    **启动日:**
    ...
    console.log(`🚀 现在以 ${formatEther(price)} B3每令牌上线`);
  ...
  <Tab title="社区参与">
    **实时更新:**
    ...
    if (progress.progress >= 0.5 && progress.progress < 0.6) {
      postToDiscord("🎯 50%资金到位！迁移半途！");
    } else if (progress.progress >= 0.9) {
      postToDiscord("🔥 90%资金到位！迁移迫在眉睫！");
    }
    ```
  </Tab>
</Tabs>
