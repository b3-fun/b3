...
<AccordionGroup>
  <Accordion title="错误：'Buy: 执行回滚' 或 'Sell: 执行回滚'">
    ...
    **解决方案：**
    ...
    if (status !== 1) {
      throw new Error("令牌不在绑定阶段 - 改用 DEX 方法");
    }
...
<AccordionGroup>
  ...
  <Accordion title="错误：'交易确认超时'">
    ...
    **解决方案：**
    ...
    const options = {
      gas: 300000n, // 从默认值增加
      maxFeePerGas: parseGwei("20"), // 更高的费用以更快包含
...
<AccordionGroup>
  ...
  <Accordion title="燃气估算问题">
    ...
    **解决方案：**
    ```typescript
    // 燃气估算助手
    async function estimateGasWithBuffer(contract, method,
