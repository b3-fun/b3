...
<AccordionGroup>
  <Accordion title="错误：'BondkitToken 需要有效的合约地址'">
    **原因：** 代币地址缺失、无效或未校验。
    ...
    **解决方案：**
    ...
    const token = new BondkitToken("0x123"); // 地址太短
  ...
  <Accordion title="错误：'DeployBondkitToken: 执行回退'">
    **原因：** 无效的部署参数或权限不足。
    ...
    **解决方案：**
    ...
    const config = {
      name: "MyToken",
      symbol: "MTK", // 必须是 1-11 个字符
      feeRecipient: "0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb1", // 有效地址
      finalTokenSupply: parseEther("1000000"), // 必须 > 0
      aggressivenessFactor: 45, // 必须是 0-100
      lpSplitRatioFeeRecipientBps: 1000n, // 必须是 0-10000（基点）
...
<AccordionGroup>
  <Accordion title="错误：'购买：执行回退' 或 '出售：执行回退'">
    **原因：** 在绑定阶段期间各种交易问题。
    ...
    **解决方案：**
    ...
    if (status !== 1) {
      throw new Error("代币不在绑定阶段 - 改用 DEX 方法");
    }
    ...
    async function diagnoseSellError(token, tokenAmount) {
      ...
      if (balance < tokenAmount) {
        return `代币余额不足。需要：${formatEther(tokenAmount)}，拥有：${formatEther(balance)}`;
      }
      
      const progress = await token.getBondingProgress();
      if (progress.raised === 0n) {
        return "没有可用流动性 - 没有筹集到资金时无法出售";
      }
      
      return "未知出售错误 - 检查交易详情";
    }
    ```
  </Accordion>
  
  <Accordion title="错误：'交易代币地址不可用'">
    **原因：** 代币未完全初始化或网络问题。
    ...
    **解决方案：**
    ...
    try {
      ...
      throw new Error("无效的代币合约或网络问题");
    }
    ```
  </Accordion>
  
  <Accordion title="错误：'绑定阶段已结束' 或 DEX 交易问题">
    **原因：** 代币迁移到 DEX 阶段，但仍使用绑定方法。
...
<AccordionGroup>
  <Accordion title="错误：'无法确定交易账户'">
    ...
    **解决方案：**
    ...
    async function ensureWalletConnected(token) {
      if (!window.ethereum) {
        throw new Error("未找到钱包提供者 - 安装 MetaMask 或类似应用");
      }
  ...
  <Accordion title="错误：'交易确认超时'">
    ...
    **解决方案：**
    ...
    const options = {
      gas: 300000n, // 从默认值增加
      maxFeePerGas: parseGwei("20"), // 更高的费用以加快包含
    ...
    async function testRPCHealth() {
      ...
      for (const rpcUrl of rpcUrls) {
        try {
          ...
          return rpcUrl; // 返回第一个有效的 RPC
        } catch (error) {
          console.log(`❌ ${rpcUrl}: ${error.message}`);
        }
      }
      
      throw new Error("所有 RPC 端点均失败");
    }
