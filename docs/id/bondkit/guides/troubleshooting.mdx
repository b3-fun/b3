...
Mulai di sini untuk identifikasi masalah secara cepat:

```typescript
import { BondkitToken, BondkitTokenFactory } from "@b3dotfun/sdk/bondkit";

// Fungsi pemeriksaan kesehatan cepat
...
<AccordionGroup>
  ...
  <Accordion title="Error: 'Unsupported chainId: X. SDK ini dikonfigurasi untuk Base saja'">
...
<AccordionGroup>
    ...
    **Solusi:**
    ...
    async function diagnoseBuyError(token, amount, minTokensOut) {
      ...
      if (tradingTokenAddress !== "0x0000000000000000000000000000000000000000") {
        ...
        if (balance < required) {
          return `Saldo ${await token.getTradingTokenSymbol()} tidak cukup. Dibutuhkan: ${formatEther(required)}, Dimiliki: ${formatEther(balance)}`;
        }
      }
    ...
    async function diagnoseSellError(token, tokenAmount) {
      ...
      if (balance < tokenAmount) {
        return `Saldo token tidak cukup. Dibutuhkan: ${formatEther(tokenAmount)}, Dimiliki: ${formatEther(balance)}`;
      }
  ...
  <Accordion title="Error: 'Alamat token trading tidak tersedia'">
    ...
    **Solusi:**
    ...
    // Periksa jika kontrak token valid
    try {
  ...
  <Accordion title="Error: 'FaseBondingBerakhir' atau masalah trading DEX">
...
### Masalah Integrasi Wallet

<AccordionGroup>
  <Accordion title="Error: 'Akun untuk transaksi tidak dapat ditentukan'">
    **Penyebab:** Wallet tidak terhubung atau dikonfigurasi dengan benar.
    
    **Solusi:**
    ```typescript
    // Opsi 1: Gunakan kunci privat (hanya server)
    ...
    if (!connected) {
      throw new Error("Gagal terhubung wallet");
    }
    
    // Opsi 3: Verifikasi koneksi sebelum transaksi
    async function ensureWalletConnected(token) {
      if (!window.ethereum) {
        throw new Error("Tidak ada penyedia wallet ditemukan - instal MetaMask atau serupa");
      }
      ...
      if (accounts.length === 0) {
        throw new Error("Tidak ada akun tersedia - hubungkan wallet terlebih dahulu");
      }
      
      // Terhubung ke token
      const connected = await token.connect(window.ethereum);
      if (!connected) {
        throw new Error("Gagal terhubung token ke wallet");
      }
  ...
  <Accordion title="Error: 'Waktu konfirmasi transaksi habis'">
    **Penyebab:** Jaringan lambat, gas rendah, atau masalah wallet OKX.
  ...
  <Accordion title="Masalah spesifik MetaMask/Wallet">
    **Masalah Wallet Umum:**
          ...
          try {
            await window.ethereum.request({
              ...
              params: [{
                chainId: '0x2105',
                chainName: 'Base',
                mata uang asli: { name: 'ETH', symbol: 'ETH', decimals: 18 },
    ...
    async function addTokenToWallet(tokenAddress, symbol, decimals = 18) {
      try {
        ...
        console.log(`${symbol} ditambahkan ke wallet`);
      } catch (error) {
        console.error('Gagal menambahkan token ke wallet:', error);
      }
    }
    ```
  </Accordion>
    ...
    async function testRPCHealth() {
      ...
      for (const rpcUrl of rpcUrls) {
        try {
          ...
          return rpcUrl; // Kembalikan RPC pertama yang berhasil
        } catch (error) {
