...
<AccordionGroup>
  <Accordion title="Error: 'Invalid contract address is required for BondkitToken'">
    **Penyebab:** Alamat token hilang, tidak valid, atau tidak disemak.
...
<AccordionGroup>
  ...
  <Accordion title="Error: 'BondingPhaseEnded' atau masalah perdagangan DEX">
  ...
  <Accordion title="Error: 'Akun untuk transaksi tidak dapat ditentukan'">
    ...
    **Solusi:**
    ...
    async function ensureWalletConnected(token) {
      if (!window.ethereum) {
        throw new Error("Tidak ada penyedia dompet ditemukan - instal MetaMask atau serupa");
      }
