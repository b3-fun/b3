...
<AccordionGroup>
  ...
  <Accordion title="Error: 'chainId yang tidak didukung: X. SDK ini dikonfigurasi untuk Base saja'">
    ...
    **Solusi:**
    ...
    const factory = new BondkitTokenFactory(8453); // ID Base secara eksplisit
    ```
  </Accordion>
    ...
    **Solusi:**
    ...
    function validateConfig(config) {
      ...
      if (config.lpSplitRatioFeeRecipientBps > 10000n) {
        errors.push("Rasio pemisahan LP tidak bisa melebihi 100% (10000 bps)");
      }
    ...
    **Solusi:**
    ...
    async function diagnoseBuyError(token, amount, minTokensOut) {
      ...
      if (progress.progress >= 1.0) {
        return "Target tercapai - migrasi tersedia, tidak bisa membeli lagi";
      }
    ...
    async function diagnoseSellError(token, tokenAmount) {
      ...
      if (progress.raised === 0n) {
        return "Tidak ada likuiditas tersedia - tidak bisa menjual saat tidak ada yang terkumpul";
      }
...
<AccordionGroup>
  ...
  <Accordion title="Masalah MetaMask/Dompet spesifik">
    **Masalah Dompet Umum:**
