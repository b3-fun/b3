...
- **Token Bondkit**: Jumlah yang dihitung untuk cocok dengan harga keluar
- **Token LP**: Dibagi antara penerima biaya dan dibakar/didistribusikan

## Perdagangan di Fase DEX
...
- Harga ditentukan oleh penawaran/permintaan
...
const migrationData = {
  ...
  bondkitTokenLiquidity: "500000",  // Jumlah token yang dihitung
...
|--------|--------------|-----------|
| **Harga** | Kurva algoritmik | Penawaran/permintaan pasar |
| **Likuiditas** | Tidak terbatas (kontrak mencetak) | Likuiditas kolam terbatas |
| **Biaya** | 5% ke penerima biaya | 0.3% ke LP + protokol |
| **Tipe Pesanan** | Hanya beli/jual | Fitur DEX penuh |
| **Dampak Harga** | Kurva yang dapat diprediksi | Tergantung pada kedalaman likuiditas |
...
**Ketersediaan Metode:**
- ✅ `token.buy()` / `token.sell()` - **Dinonaktifkan** (akan revert)
...
**Menjadi LP:**
```typescript
// Tambah likuiditas ke kolam token Bondkit
...
// Dapatkan biaya perdagangan + insentif potensial
```
...
- ✅ Dapat diperdagangkan di pasar terbuka
...
- 🚀 Desentralisasi sejati tercapai
...
await token.buy(amount, minOut);    // ❌ Revert
await swapService.executeSwap(...); // ✅ Gunakan ini sebagai gantinya
```

**Tanggung Jawab Baru:**
- **Pembangunan Komunitas**: Dorong volume perdagangan organik
- **Insentif Likuiditas**: Dorong partisipasi LP  
- **Pemasaran**: Promosikan utilitas dan adopsi token
...
<AccordionGroup>
  <Accordion title="Apakah token dapat kembali ke fase obligasi?">
    **Tidak** - Migrasi permanen dan tidak dapat dibalik. Setelah berada di fase DEX, token akan selalu diperdagangkan di Uniswap v4.
  </Accordion>
  ...
  <Accordion title="Apakah ada pembatasan di fase DEX?">
...
<CardGroup cols={2}>
  ...
  <Card title="Menjadi LP" icon="coins" href="https://app.uniswap.org">
    Sediakan likuiditas dan dapatkan biaya perdagangan
  </Card>
  <Card title="Pantau Analitik" icon="analytics" href="https://info.uniswap.org">
    Lacak performa DEX token Anda  
  </Card>
</CardGroup>
