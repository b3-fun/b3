...
1. Klien melakukan permintaan awal
...
### Menentukan token pembayaran

Ada dua cara untuk menentukan token yang ingin Anda bayar:
...
Untuk kontrol lebih lanjut, gunakan fungsi `wrapFetchWithPayment` tingkat rendah:
...
**Keuntungan dari `wrapFetchWithPayment`:**
...
Jika server sumber daya mendukung middleware AnySpend, Anda akan membayar dengan jumlah setara dalam token yang diinginkan alih-alih USDC.
...
Saat melakukan permintaan ke layanan yang diaktifkan X402, Anda dapat menentukan token pembayaran dan jaringan yang diinginkan menggunakan header HTTP ini:
...
Untuk kontrol lebih lanjut, tangani alur pembayaran secara manual:
...
interface RequestOptions {
  ...
  body?: any;                         // Badan permintaan (otomatis diserialisasi ke JSON)
...
// Helper untuk mendapatkan nama jaringan dari ID rantai
