...
const fetchWithPayment = wrapFetchWithPayment(
  ...
  paymentPreferences, // Tentukan token dan jaringan yang Anda inginkan
);
...
Jika server sumber daya mendukung middleware AnySpend, Anda akan membayar jumlah yang setara dalam token pilihan Anda alih-alih USDC.

### Menggunakan HTTP Headers untuk Preferensi Pembayaran

<Note>
**Baru di X402**: Sekarang Anda dapat menentukan preferensi pembayaran menggunakan HTTP headers, yang sangat berguna saat membuat permintaan HTTP langsung atau saat mengintegrasikan dengan klien HTTP yang ada.
</Note>

Saat membuat permintaan ke layanan yang diaktifkan X402, Anda dapat menentukan token pembayaran dan jaringan yang Anda inginkan menggunakan header HTTP ini:
...
interface RequestOptions {
  ...
  body?: any;                         // Isi permintaan (di-serialisasi otomatis ke JSON)
...
const fetchWithPayment = wrapFetchWithPayment(
  ...
  undefined, // Pemilih persyaratan default
  {
    ...
    maxRetries: 3      // Coba hingga 3 kali
  },
  {
...
Untuk aplikasi yang membutuhkan lebih banyak kontrol atau menggunakan kode berbasis fetch yang ada:
...
export function PremiumDataFetcherAdvanced() {
  ...
  const fetchPremiumData = async () => {
    ...
    try {
      ...
      const paymentPreferences = {
        preferredToken: tokenAddress,
        preferredNetwork: getNetworkName(selectedChain)
