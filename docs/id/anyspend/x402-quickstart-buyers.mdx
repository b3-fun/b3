...
Ada dua cara untuk menentukan token yang ingin Anda gunakan untuk pembayaran:
...
Untuk kontrol lebih, gunakan fungsi `wrapFetchWithPayment` tingkat lebih rendah:
...
Jika server sumber daya mendukung middleware AnySpend, Anda akan membayar jumlah yang setara dalam token yang diinginkan alih-alih USDC.

### Menggunakan HTTP Headers untuk Preferensi Pembayaran

<Note>
**Baru di X402**: Anda sekarang dapat menentukan preferensi pembayaran menggunakan HTTP headers, yang sangat berguna saat melakukan permintaan HTTP langsung atau saat mengintegrasikan dengan klien HTTP yang ada.
</Note>

Saat membuat permintaan ke layanan yang diaktifkan X402, Anda dapat menentukan token pembayaran dan jaringan yang diinginkan menggunakan header HTTP ini:
...
interface X402ClientOptions {
  ...
  autoRetry?: boolean;                // Otomatis mengulang pada 402 (default: true)
  timeout?: number;                   // Timeout permintaan dalam ms (default: 30000)
}
```
...
interface RequestOptions {
  ...
  body?: any;                         // Body permintaan (otomatis diserialisasi ke JSON)
...
Untuk kasus penggunaan lanjutan, `wrapFetchWithPayment` memberikan kontrol tingkat lebih rendah:
...
function wrapFetchWithPayment(
  ...
  config?: {
    timeout?: number;                  // Timeout permintaan dalam ms
...
const fetchWithPayment = wrapFetchWithPayment(
  ...
  undefined, // Pemilih persyaratan default
  {
    ...
    maxRetries: 3      // Mencoba hingga 3 kali
  },
  {
...
- **Banyak token lain** - Periksa kompatibilitas menggunakan alat di bawah ini
