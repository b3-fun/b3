...
Panduan ini menunjukkan cara menggunakan **AnySpend x402 Client SDK** untuk membayar sumber daya berbayar menggunakan token yang kompatibel dengan EIP-2612 atau EIP-3009. SDK menangani seluruh alur pembayaran secara otomatis, termasuk pembuatan tanda tangan dan logika pengulangan.

Keindahan x402 adalah Anda dapat menggunakan klien yang sama untuk membayar **layanan yang diaktifkan x402 apa pun**, tidak hanya yang menggunakan AnySpend.
...
<Note>
**Penting**: Hanya token dengan dukungan EIP-2612 (Permit) atau EIP-3009 (TransferWithAuthorization) yang kompatibel dengan AnySpend x402 untuk pembayaran tanpa gas. Gunakan [Pemeriksa Kompatibilitas Token](https://anyspend.com/x402-tokens) untuk memverifikasi token Anda didukung.
</Note>
...
SDK menggunakan [viem](https://viem.sh) untuk operasi dompet dan pembuatan tanda tangan.
...
try {
  ...
  console.log('Transaksi Pembayaran:', response.paymentResponse?.txHash);
} catch (error) {
...
Untuk kontrol lebih lanjut, gunakan fungsi `wrapFetchWithPayment` yang lebih rendah:
...
const fetchWithPayment = wrapFetchWithPayment(
  fetch,
  walletClient,
  '1000000', // Nilai pembayaran maksimum (1 USDC dalam 6 desimal)
...
- Dapat menentukan nilai pembayaran maksimum
...
Saat melakukan permintaan ke layanan yang diaktifkan X402, Anda dapat menentukan token pembayaran dan jaringan yang diinginkan Anda menggunakan header HTTP ini:
...
Untuk kontrol lebih banyak, tangani alur pembayaran secara manual:

```typescript
// 1. Buat permintaan awal
...
interface X402ClientOptions {
  ...
  autoRetry?: boolean;                // Otomatis-ulangi pada 402 (default: true)
...
interface RequestOptions {
  ...
  body?: any;                         // Isi permintaan (otomatis-serialisasi ke JSON)
...
function wrapFetchWithPayment(
  ...
  config?: {
    ...
    maxRetries?: number;               // Maks upaya pengulangan pembayaran
  },
  paymentPreferences?: {
    preferredToken?: string;           // Alamat token untuk pembayaran
    preferredNetwork?: string;         // Pengidentifikasi jaringan (mis., 'base', 'ethereum')
  }
): typeof fetch
```
...
Untuk aplikasi yang memerlukan lebih banyak kontrol atau menggunakan kode berbasis fetch yang ada:
