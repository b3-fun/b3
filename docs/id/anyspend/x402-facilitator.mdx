...
Fasilitator AnySpend x402 menangani pembayaran multi-token, lintas-rantai untuk protokol x402. Ini mengurus pertukaran token, jembatan lintas-rantai, dan penyelesaian sehingga pengembang dapat menerima pembayaran dalam token apa pun sambil menerima mata uang pilihan mereka.

**Endpoint Produksi:** `https://mainnet.anyspend.com/x402`

## Kemampuan Utama

### Pemrosesan pembayaran multi-token
...
- **Token khusus**: Token apa pun dengan dukungan permit, termasuk token proyek seperti B3
...
- Pembayaran berbasis HTTP melalui status `402 Payment Required`
...
- Verifikasi kriptografi tanpa pihak ketiga yang dipercaya
...
- **Tanpa kunci vendor**: API HTTP standar, ganti fasilitator kapan saja
...
- Mendapatkan kurs tukar saat ini
...
app.use(paymentMiddleware(
  '0xAlamatAnda...',
  endpointConfig,
  facilitator  // Sudah dikonfigurasi ke mainnet.anyspend.com/x402
));
```
...
Semua tanda tangan mencakup cap waktu kedaluwarsa:
