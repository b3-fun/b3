...
AnySpend x402 menggunakan tanda tangan kriptografis daripada persetujuan token tradisional, memungkinkan **pembayaran tanpa gas** bagi pengguna. Fasilitator membayar semua biaya gas sementara pengguna hanya menandatangani pesan otorisasi.
...
| **Eksekusi** | Dua langkah: setujui + transfer | Satu langkah: transfer langsung |
...
const signature = await signTransferWithAuthorization({
  ...
  nonce: randomBytes32()         // Generate nonce acak
});
```
...
Persetujuan berbasis tanda tangan yang menetapkan batas, diikuti oleh panggilan `transferFrom()` terpisah. Ini adalah metode standar untuk sebagian besar token ERC-20 modern.
...
- ✅ **Persetujuan terbatas waktu** - Kadaluarsa berbasis deadline
- ✅ **Kompatibel ERC-20** - Bekerja dengan infrastruktur yang ada
- ✅ **Tanpa gas untuk pembayar** - Fasilitator membayar gas
- ✅ **Dukungan ekosistem** - Didukung oleh dompet dan dapps utama
...
permit(
    ...
    uint256 deadline,      // Timestamp kadaluarsa
...
// Hasilkan nonce acak yang aman secara kriptografis
...
Kedua metode menegakkan deadline untuk mencegah tanda tangan usang:
