...
- **EIP-2612 (permit)** - Digunakan oleh kebanyakan token ERC-20 modern, dua langkah persetujuan + transfer

## Perbandingan cepat

| Fitur | EIP-2612 (Permit) | EIP-3009 (transferWithAuthorization) |
|---------|-------------------|--------------------------------------|
| **Digunakan oleh** | Kebanyakan ERC-20 modern | USDC (semua jaringan) |
...
| **Ketergantungan** | Harus menunggu nonce sebelumnya | Tidak memerlukan urutan |
...
## EIP-2612: Permit (ERC-20 Standar)

### Ikhtisar

Persetujuan berbasis tanda tangan yang menetapkan batas, diikuti oleh panggilan `transferFrom()` terpisah. Ini adalah metode standar untuk kebanyakan token ERC-20 modern.

**Digunakan oleh:** Kebanyakan token ERC-20 modern dengan dukungan permit

### Keuntungan utama

- Banyak diadopsi - standar di banyak token
- Persetujuan terbatas waktu - kedaluwarsa berbasis deadline
...
Fasilitator memanggil `permit()` untuk menetapkan batas di rantai
</Step>
...
<Step title="Fasilitator Mentransfer Token">
...
**Keuntungan:**
- Tidak ada ketergantungan urutan - beberapa tanda tangan dapat digunakan dalam urutan apa pun
...
function permit(/* ... */) external {
    require(nonce == nonces[owner], "Nonce tidak valid");
    nonces[owner]++; // Auto-increment
    // ... logika permit sisanya
}
```
...
- Transaksi gagal memblokir tanda tangan selanjutnya sampai ditandatangani ulang

### Penegakan deadline

Kedua metode menegakkan deadline untuk mencegah tanda tangan kedaluwarsa:
...
**Praktik Terbaik:**
- Gunakan deadline pendek (5-10 menit) untuk keamanan
- Deadline lebih panjang (30-60 menit) untuk UX yang lebih baik jika diperlukan
...
```typescript
// Pulihkan pengirim dari tanda tangan
...
// Verifikasi pengirim cocok dengan alamat yang diharapkan
