...
description: >-
  Memahami tanda tangan pembayaran tanpa gas - EIP-2612 Permit dan EIP-3009
...
AnySpend x402 menggunakan tanda tangan kriptografis alih-alih persetujuan token tradisional, memungkinkan **pembayaran tanpa gas** untuk pengguna. Fasilitator membayar semua biaya gas sementara pengguna hanya menandatangani pesan otorisasi.
...
- **EIP-2612 (permit)** - Digunakan oleh DAI dan sebagian besar token ERC-20 modern, persetujuan dua langkah + transfer

## Perbandingan Cepat

| Fitur | EIP-2612 (Permit) | EIP-3009 (transferWithAuthorization) |
|---------|-------------------|--------------------------------------|
| **Digunakan oleh** | DAI, sebagian besar ERC-20 modern | USDC (semua jaringan) |
| **Eksekusi** | Dua langkah: setuju + transfer | Satu langkah: transfer langsung |
...
Persetujuan berbasis tanda tangan yang menetapkan batasan, diikuti oleh pemanggilan `transferFrom()` terpisah. Ini adalah metode standar untuk sebagian besar token ERC-20 modern.

**Digunakan oleh:** DAI, dan sebagian besar token ERC-20 modern dengan dukungan permit

### Keuntungan Utama

- ✅ **Sangat diadopsi** - Standar di banyak token
- ✅ **Persetujuan terbatas waktu** - Kadaluwarsa berbasis batas waktu
- ✅ **Kompatibel ERC-20** - Bekerja dengan infrastruktur yang ada
- ✅ **Tanpa gas untuk pembayar** - Fasilitator membayar gas
- ✅ **Dukungan ekosistem** - Didukung oleh dompet dan dapp utama
...
permit(
    ...
    uint256 deadline,      // Timestamp kadaluwarsa
...
const signature = await signPermit({
  ...
  value: '1000000000000000000',  // 1 DAI (18 desimal)
  deadline: deadline,
...
Fasilitator memanggil `permit()` untuk menetapkan batasan di rantai
</Step>

<Step title="Nonce Auto-Increments">
Kontrak token secara otomatis menambahkan nonce pengguna
</Step>
...
```typescript
// Periksa jika token mendukung EIP-3009 (gaya USDC)
const hasTransferWithAuth = await token.read.transferWithAuthorization !== undefined;

// Periksa jika token mendukung EIP-2612 (Permit standar)
...
<Note>
Klien AnySpend x402 secara otomatis mendeteksi metode tanda tangan mana yang digunakan berdasarkan kontrak token. Anda tidak perlu menentukan ini secara manual.
</Note>
...
// Menghasilkan nonce acak yang aman secara kriptografi
const nonce = '0x' + randomBytes(32).toString('hex');
```

**Pelacakan Nonce:**
```solidity
// Periksa jika nonce telah digunakan
...
Kedua metode menegakkan batas waktu untuk mencegah tanda tangan usang:
...
Klien AnySpend x402 menangani semua kompleksitas tanda tangan secara otomatis:
...
const client = new X402Client({
  walletClient,
  preferredToken: tokenAddress // USDC atau DAI
});

// Klien secara otomatis:
// 1. Mendeteksi jika token menggunakan permit atau transferWithAuthorization
// 2. Mendapatkan nonce saat ini (untuk permit) atau menghasilkan nonce acak
// 3. Membangun data bertipe EIP-712 yang benar
