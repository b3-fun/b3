...
---
Sesi checkout memungkinkan Anda membuat objek pelacakan di server Anda sebelum pengguna memilih metode pembayaran. Sesi tersebut merupakan catatan database ringan -- tanpa panggilan API eksternal, tanpa pembuatan pesanan. Pesanan datang kemudian, ketika pengguna berkomitmen untuk membayar.

<Info>
  Mencari komponen checkout React? Lihat [Checkout](/anyspend/checkout). Halaman ini membahas **REST API** untuk alur yang dipimpin server.
</Info>

## URL Dasar

Semua endpoint sesi checkout berada di layanan AnySpend:
...
<Steps>
  ...
  <Step title="Pengguna membayar">
    **Crypto:** kirim ke `globalAddress` | **Fiat onramp:** arahkan ke `oneClickBuyUrl`
  </Step>
...
- Pembuatan sesi adalah penulisan DB lokal, sehingga tidak pernah gagal karena kesalahan API pihak ketiga.
...
Membuat sesi ringan. Tanpa pesanan, tanpa panggilan API eksternal.
...
### Membuat pesanan terkait dengan sesi
...
// 2. Buat pesanan terkait dengan sesi
...
const poll = setInterval(async () => {
  ...
  if (s.data.status === "complete") {
    clearInterval(poll);
    // arahkan ke success_url atau tampilkan konfirmasi
  }
}, 3000);
```
...
// Arahkan pengguna ke halaman checkout vendor
window.location.href = order.data.oneClickBuyUrl;

// Setelah vendor mengarahkan kembali, poll GET /checkout-sessions/:id untuk penyelesaian
