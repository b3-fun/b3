...
app.use(paymentMiddleware(
  recipientAddress,
  endpointConfig,
  facilitator                    // Fasilitator Anyspend yang telah dikonfigurasi
));
...
5. Handler rute Anda dieksekusi dan mengembalikan data

---

### 2. Menerima Token Kustom (mis., B3)
...
// Konfigurasikan untuk menerima token B3
const endpointConfig = {
  'POST /api/premium': {
    price: {
...
- Fleksibilitas lengkap untuk pembeli dan penjual!
...
Paket `@b3dotfun/anyspend-x402` termasuk instance fasilitator yang telah dikonfigurasi:
...
#### Metode 1: Jumlah Dolar Sederhana (Direkomendasikan)
...
**Keuntungan:**
- Klien dapat membayar dengan token apa pun yang didukung
- AnySpend mengonversi ke USDC secara otomatis
- Anda selalu menerima USDC

#### Metode 2: Persyaratan Token Spesifik

Memerlukan token spesifik di jaringan tertentu. Anda dapat menerima **token apa pun**, tidak hanya USDC!
...
**Contoh 2: Menerima Token B3 (Token Kustom)**
...
**Contoh 5: Menerima Token ERC-20 Kustom**
...
- Fleksibilitas pembeli dan penjual tetap terjaga
</Note>

### Menemukan Detail Token

Untuk mengonfigurasi token kustom, Anda perlu mengetahui parameter EIP-712-nya. Begini cara menemukannya:

**Metode 1: Periksa Kontrak Token**

Gunakan penjelajah blockchain seperti [Basescan](https://basescan.org):
...
Izinkan klien menentukan token pembayaran yang mereka inginkan:
...
app.get('/api/compute', (req, res) => {
  const units = parseInt(req.query.units as string) || 1;
  res.json({
    result: `Dihitung ${units} unit`,
    cost: req.x402Config.amount
  });
});
```

### Konfigurasi Fasilitator Kustom

Jika Anda memerlukan pengaturan fasilitator kustom:
...
// Lewati null sebagai penerima, gunakan penerima per-endpoint
...
Hanya lindungi endpoint yang memerlukan pembayaran:
...
const testFacilitator = new Facilitator({
  baseUrl: 'https://testnet.anyspend.com/x402
