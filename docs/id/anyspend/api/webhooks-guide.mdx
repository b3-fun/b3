...
---
Webhook memungkinkan server Anda menerima callback HTTP secara real-time ketika terjadi event di organisasi AnySpend Anda -- pembayaran selesai, checkout kadaluarsa, dan lainnya. Alih-alih polling API, Anda mendaftarkan URL dan AnySpend mendorong event ke Anda.
...
<Steps>
  ...
  <Step title="Pengulangan otomatis">
    Jika server Anda tidak merespons dengan status `2xx`, AnySpend akan mencoba kembali hingga 3 kali dengan penundaan eksponensial.
  </Step>
</Steps>
...
<Tip>
  Anda dapat berlangganan semua event dengan melewati `["*"]` sebagai array event, atau pilih hanya yang Anda butuhkan.
</Tip>
...
| `X-AnySpend-Signature` | Digest heksadesimal HMAC-SHA256 dari badan permintaan mentah |
...
Lewatkan ID pesanan atau pengguna Anda saat membuat checkout (melalui parameter URL atau API):
...
<Tip>
  Baik `client_reference_id` maupun `metadata` selalu termasuk dalam payload webhook. Anda dapat mengaturnya melalui [parameter URL](/anyspend/api/payment-links#url-parameters) untuk integrasi sederhana atau [API Sesi Checkout](/anyspend/api/checkout-sessions-api) untuk kontrol sisi server.
</Tip>
...
// PENTING: Gunakan badan mentah untuk verifikasi tanda tangan
app.post(
  "/webhooks/anyspend",
  express.raw({ type: "application/json" }),
  (req, res) => {
    const signature = req.headers["x-anyspend-signature"];
    rahasia = process.env.ANYSPEND_WEBHOOK_SECRET;
...
Jika endpoint Anda tidak merespons dengan kode status `2xx` dalam **30 detik**, AnySpend menandai pengiriman sebagai gagal dan mencoba kembali.

| Percobaan | Jeda setelah percobaan sebelumnya |
|---------|------------------------------|
| Percobaan ulang pertama | 1 menit |
| Percobaan ulang kedua | 10 menit |
| Percobaan ulang ketiga | 1 jam |

Setelah 3 percobaan gagal, pengiriman ditandai sebagai `gagal` secara permanen. Anda masih dapat mencoba kembali secara manual dari Dashboard atau API.
...
<Tabs>
  <Tab title="SDK">
    ```typescript
    // Daftar pengiriman terbaru untuk webhook
...
```typescript
// Coba kembali pengiriman yang gagal secara spesifik
...
Event tes menggunakan tipe event `payment.completed` dengan data tiruan. Handler Anda harus memprosesnya seperti event lain, tetapi Anda dapat memeriksa header `X-AnySpend-Test: true` jika Anda ingin melewatkan efek samping selama pengujian.
...
<CardGroup cols={2}>
  ...
  <Card title="Tangani duplikat" icon="clone">
    Gunakan header `X-AnySpend-Delivery-Id` atau field `data.id` untuk menduplikasi event. Pengulangan mungkin mengirimkan event yang sama lebih dari sekali.
  </Card>
