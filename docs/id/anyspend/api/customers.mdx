...
<ResponseField name="total_spent" type="string">
  Total jumlah yang dibelanjakan di semua transaksi yang selesai (dalam setara USD).
</ResponseField>
...
Buat catatan pelanggan secara manual. Ini berguna untuk mendaftarkan pelanggan sebelum mereka melakukan pembayaran.
...
<ParamField body="metadata" type="object">
  Pasangan kunci-nilai sembarang (mis., `{"plan": "pro"}`). Hingga 50 kunci, nilai karakter 500.
</ParamField>
...
Perbarui pelanggan yang ada. Hanya bidang yang Anda sertakan dalam badan permintaan yang akan diperbarui.
...
<ParamField body="metadata" type="object">
  Metadata yang diperbarui. Ini **menggantikan** seluruh objek metadata. Untuk menambahkan satu kunci, sertakan metadata yang ada secara lengkap ditambah kunci baru.
</ParamField>
...
<Warning>
  Tindakan ini tidak dapat dibalik. Semua informasi yang dapat mengidentifikasi pribadi (nama, email) akan dihapus secara permanen.
</Warning>
...
Ekspor data pelanggan Anda dalam format CSV atau JSON. Respons dikirim sebagai unduhan file.
