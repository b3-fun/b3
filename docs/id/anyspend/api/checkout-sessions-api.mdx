...
---
Sesi checkout mewakili upaya pembayaran individu. Anda dapat membuat sesi di sisi server untuk menghasilkan URL checkout unik untuk setiap pelanggan, mengisi detail pembayaran secara otomatis, dan melacak konversi dengan ID referensi Anda sendiri.
...
<ResponseField name="payment_link_id" type="string | null">
  ID link pembayaran yang terkait, jika sesi dibuat dari link pembayaran.
</ResponseField>
...
<ResponseField name="token_address" type="string">
  Alamat kontrak dari token yang akan diterima.
</ResponseField>
...
<ResponseField name="metadata" type="object | null">
  Pasangan kunci-nilai sembarang yang diteruskan ke webhook dan transaksi.
</ResponseField>

<ResponseField name="customer_email" type="string | null">
  Alamat email pelanggan. Di-enkripsi saat istirahat, termasuk dalam muatan webhook.
</ResponseField>

<ResponseField name="customer_name" type="string | null">
  Nama pelanggan. Di-enkripsi saat istirahat, termasuk dalam muatan webhook.
</ResponseField>

<ResponseField name="payer_address" type="string | null">
  Alamat dompet pembayar (terisi setelah koneksi).
</ResponseField>

<ResponseField name="tx_hash" type="string | null">
  Hash transaksi on-chain (terisi setelah pengajuan).
</ResponseField>

<ResponseField name="customer_id" type="string | null">
  ID pelanggan yang terkait (terisi setelah pembayaran).
</ResponseField>

<ResponseField name="form_data" type="object | null">
  Data formulir kustom yang diserahkan oleh pelanggan.
</ResponseField>
...
<ParamField query="payment_link_id" type="string">
  Filter berdasarkan ID link pembayaran.
</ParamField>
...
<ParamField body="payment_link_id" type="string">
  Buat sesi dari link pembayaran yang ada. Ini mewarisi konfigurasi link. Baik `payment_link_id` atau field manual (`token_address`, `chain_id`, `recipient_address`, `amount`) diperlukan.
</ParamField>

<ParamField body="token_address" type="string">
  Alamat kontrak dari token yang akan diterima. Diperlukan jika `payment_link_id` tidak disediakan.
</ParamField>
...
<ParamField body="metadata" type="object">
  Pasangan kunci-nilai sembarang yang diteruskan ke webhook dan transaksi yang dihasilkan. Hingga 50 kunci, nilai karakter 500.
</ParamField>

<ParamField body="customer_email" type="string">
  Alamat email pelanggan. Di-enkripsi saat istirahat dan termasuk dalam muatan webhook untuk pemenuhan pesanan.
</ParamField>

<ParamField body="customer_name" type="string">
  Nama pelanggan. Di-enkripsi saat istirahat dan termasuk dalam muatan webhook untuk pemenuhan pesanan.
</ParamField>
...
<Tabs>
  <Tab title="Response">
    Mengembalikan [Objek Sesi Checkout](#the-checkout-session-object) lengkap.
  </Tab>
</Tabs>

---

## Kedaluwarsa sesi checkout

Kedaluwarsakan sesi checkout terbuka secara manual. Ini berguna ketika pesanan yang mendasarinya dibatalkan atau pembayaran tidak lagi diperlukan.
...
<Warning>
  Hanya sesi dengan `status: "open"` yang dapat kedaluwarsa. Mencoba untuk kedaluwarsa sesi dalam status lain akan mengembalikan kesalahan `400 Bad Request`.
</Warning>
