...
---
API Platform AnySpend menyediakan titik akhir ekspor untuk mengunduh data transaksi dan pelanggan Anda secara massal. Ekspor mendukung format CSV dan JSON dengan penyaringan opsional.

<Info>
  Kedua titik akhir ekspor memerlukan kunci API dengan izin **read**.
</Info>
...
Ekspor catatan transaksi dengan filter opsional untuk status, rentang tanggal, dan format.
...
Ekspor JSON mengembalikan array dari objek transaksi:
...
<CardGroup cols={2}>
  <Card title="Jadwalkan ekspor reguler" icon="calendar">
    Atur pekerjaan cron atau fungsi terjadwal untuk mengekspor data setiap hari atau mingguan untuk cadangan dan rekonsiliasi.
  </Card>
  <Card title="Gunakan filter tanggal untuk dataset besar" icon="filter">
    Jika Anda memiliki ribuan transaksi, gunakan parameter `from` dan `to` untuk mengekspor dalam rentang tanggal yang lebih kecil untuk menghindari waktu tunggu.
  </Card>
  ...
  <Card title="JSON untuk penggunaan programatik" icon="code">
    Ekspor JSON ideal untuk memasukkan data ke dalam pipa analitik, basis data, atau dasbor kustom.
  </Card>
</CardGroup>

<Warning>
  Ekspor tunduk pada pembatasan laju. Ekspor besar (lebih dari 10.000 catatan) mungkin memerlukan beberapa detik untuk dihasilkan. Permintaan akan diblokir sampai ekspor selesai -- rencanakan dengan tepat untuk dataset yang sangat besar.
</Warning>
