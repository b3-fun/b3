...
| `message` | `string` | Deskripsi yang dapat dibaca manusia tentang apa yang salah. Aman untuk ditampilkan kepada pengembang (tetapi tidak untuk pengguna akhir). |
...
| `rate_limit_error` | `429` | Anda telah melebihi batas tarif untuk tingkat otentikasi Anda. |
| `idempotency_error` | `409` | Sebuah kunci idempotensi digunakan kembali dengan badan permintaan yang berbeda. |
...
|------|------|------|-------------|
| `missing_required_field` | `invalid_request_error` | `400` | Sebuah field yang diperlukan tidak disertakan dalam badan permintaan. Field `param` menunjukkan field mana yang hilang. |
| `invalid_field_value` | `invalid_request_error` | `400` | Sebuah field disertakan tetapi nilainya tidak valid (tipe, format, atau di luar jangkauan yang salah). |
| `invalid_address` | `invalid_request_error` | `400` | Sebuah field alamat Ethereum tidak merupakan alamat `0x` + 40 karakter heksadesimal yang valid. |
| `no_updates_provided` | `invalid_request_error` | `400` | Sebuah permintaan `PATCH` dikirim tanpa field valid untuk diperbarui. |
...
| Code | Type | HTTP | Description |
|------|------|------|-------------|
| `resource_not_found` | `not_found_error` | `404` | Sumber daya yang diminta tidak ada, atau milik organisasi yang berbeda. |

### Kesalahan pembatasan tarif dan idempotensi
...
| `idempotency_conflict` | `idempotency_error` | `409` | Sebuah header `Idempotency-Key` digunakan kembali dengan badan permintaan yang berbeda dari permintaan asli. |

### Kesalahan server

| Code | Type | HTTP | Description |
|------|------|------|-------------|
| `internal_error` | `api_error` | `500` | Terjadi kesalahan server yang tidak terduga. Ini secara otomatis dilacak. Jika ini berlanjut, hubungi dukungan. |
...
| `401` | Tidak Sah -- otentikasi gagal |
...
| `500` | Kesalahan Server Internal -- ada yang salah di pihak kami |
...
```bash
# Sebuah kunci hanya-baca mencoba membuat sumber daya
curl -X POST https://platform-api.anyspend.com/api/v1/payment-links \
  ...
  if (!response.ok) {
    ...
    switch (error.type) {
      ...
      case "authentication_error":
        // Periksa kunci API Anda -- mungkin telah kedaluwarsa atau dicabut
      ...
      case "rate_limit_error":
        ...
        console.warn(`Dibatasi tarif: ${error.message}`);
        await sleep(5000);
...
Gunakan `error.type` untuk alur kontrol luas (mis., coba lagi pada `rate_limit_error`, gagal cepat pada `authentication_error`). Gunakan `error.code` untuk penanganan spesifik dalam sebuah tipe.
</Accordion>

<Accordion title="Gunakan field param untuk membangun validasi formulir">
Ketika `error.param` hadir, Anda dapat memetakan langsung ke field formulir untuk menampilkan kesalahan validasi inline di UI Anda.
</Accordion>
...
Selalu log `error.type`, `error.code`, `error.message`, dan `error.param` bersama-sama. Ini memberi Anda gambaran lengkap saat melakukan debug masalah produksi.
</Accordion>
</AccordionGroup>
