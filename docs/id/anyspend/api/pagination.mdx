...
| `data` | `array` | Array dari objek sumber daya untuk halaman saat ini. Setiap item mencakup field tipe `object` sendiri. |
...
| `url` | `string` | Jalur endpoint API untuk daftar ini. |
...
|-----------|------|---------|-----|-------------|
| `page` | `integer` | `1` | -- | Nomor halaman yang akan diambil (diindeks dari 1). |
| `limit` | `integer` | `20` | `100` | Jumlah hasil per halaman. |

### Contoh paginasi dasar
...
Gunakan field `has_more` untuk menentukan kapan harus berhenti memaginasi:
...
### Bidang pengurutan Payment Links
...
| `current_uses` | Jumlah checkout yang selesai. |
...
## Contoh auto-paginasi

Untuk kenyamanan, Anda dapat membangun iterator asinkron yang menangani paginasi secara otomatis:
...
Mengatur `limit=1` meminimalkan beban respons saat Anda hanya membutuhkan jumlahnya.

## Praktik terbaik

<AccordionGroup>
<Accordion title="Gunakan limit=100 untuk operasi massal">
Ketika Anda perlu mengambil semua catatan (mis., untuk ekspor data atau sinkronisasi), gunakan `limit=100` maksimum untuk meminimalkan jumlah panggilan API dan tetap dalam batas laju.
</Accordion>

<Accordion title="Gunakan has_more daripada total_count untuk loop paginasi">
`has_more` adalah sinyal otoritatif untuk apakah harus mengambil halaman berikutnya. Hindari menghitung jumlah halaman dari `total_count` karena total dapat berubah antar permintaan.
</Accordion>

<Accordion title="Cache total_count untuk kontrol paginasi UI">
Ketika membangun UI paginasi (mis., "Halaman 3 dari 8"), ambil `total_count` sekali dan cache untuk sesi. Ambil ulang hanya ketika pengguna menyegarkan atau menerapkan filter baru.
</Accordion>

<Accordion title="Gabungkan filter dengan paginasi">
Terapkan filter sebelum memaginasi untuk mengurangi set hasil. Ini lebih efisien daripada mengambil semua catatan dan memfilter di sisi klien:

```bash
# Daripada mengambil semua link dan memfilter secara lokal...
# Filter di sisi server dan paginasikan hasil yang difilter
curl "https://platform-api.anyspend.com/api/v1/payment-links?active=true&search=premium&limit=20" \
  -H "Authorization: Bearer asp_live_abc123..."
```
</Accordion>

<Accordion title="Patuhi batas laju selama pengambilan massal">
Jika Anda memaginasi melalui dataset yang besar, perhatikan batas laju 120 permintaan/menit. Untuk dataset yang sangat besar, tambahkan jeda singkat antara permintaan halaman:
