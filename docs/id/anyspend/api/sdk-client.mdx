...
---
Klien SDK Platform AnySpend adalah klien TypeScript **headless** (tanpa dependensi React) untuk seluruh API Platform AnySpend. Berfungsi di mana saja JavaScript modern berjalan -- Node.js, browser, Cloudflare Workers, Deno, dan Bun.
...
const platform = new AnySpendPlatformClient(process.env.ANYSPEND_API_KEY!, {
  ...
  timeout: 30_000,           // timeout permintaan dalam ms (default: 30s)
...
<Info>
  Anda dapat memperoleh kunci API dari [Dasbor AnySpend](https://app.anyspend.com) di bawah **Settings > API Keys**. Kunci dikaitkan dengan organisasi Anda dan dapat memiliki izin baca-saja atau baca-tulis.
</Info>

## Sumber Daya Tersedia

Klien diorganisir ke dalam namespace sumber daya, masing-masing dengan metode yang langsung dipetakan ke titik akhir REST.
...
Untuk kenyamanan, setiap metode `list` memiliki pembantu `listAutoPaginate` yang mengembalikan iterator asinkron. Hal ini mengambil halaman di belakang layar saat Anda mengonsumsi item.
...
<Tip>
  Auto-pagination secara otomatis menghormati batasan laju. Jika API mengembalikan `429`, iterator menunggu durasi `Retry-After` sebelum mengambil halaman berikutnya.
</Tip>
...
SDK melempar kelas kesalahan bertipe sehingga Anda dapat menangani mode kegagalan spesifik:
...
try {
  ...
  if (err instanceof NotFoundError) {
    ...
    // Tangkapan untuk kesalahan API apa pun
...
- **Respons batasan laju 429** -- menunggu durasi `Retry-After`

Percobaan kembali menggunakan **exponential backoff** dengan jitter. Defaultnya adalah 3 percobaan kembali, dapat dikonfigurasi melalui opsi `maxRetries`.

```typescript
const platform = new AnySpendPlatformClient("asp_key", {
  maxRetries: 5, // hingga 5 percobaan kembali
});
```
...
<Tip>
  Menggunakan kunci idempotensi deterministik (seperti ID pesanan) disarankan untuk operasi kritis. Jika Anda secara tidak sengaja mengirim permintaan yang sama dua kali, panggilan kedua mengembalikan respons asli alih-alih membuat duplikat.
</Tip>
