...
이 가이드는 Bondkit 토큰의 초기 생성부터 완전한 분산화까지의 전체 라이프사이클을 안내합니다. 이 흐름을 이해하는 것은 성공적인 토큰 출시와 관리에 필수적입니다.
...
<AccordionGroup>
  <Accordion title="파라미터 선택">
    **중요 결정사항:**
    ...
    | **aggressivenessFactor** | 가격 곡선의 가파름 | 균형 잡힌 출시를 위해 30-60 |
    | **targetAmount** | 이주 임계값 | 5-50 거래 토큰 |
    | **tradingToken** | 사용자가 지불하는 것 | Base 메인넷에는 B3 사용 |
...
<Tabs>
  ...
  <Tab title="커뮤니티 참여">
    **실시간 업데이트:**
    ...
    token.onBuy((event) => {
      const announcement = `
        🟢 새로운 구매: ${formatEther(event.tokensOut)} ${symbol}
        💰 사용된 금액: ${formatEther(event.ethIn)} B3
        👤 구매자: ${event.buyer}
      `;
    ...
    token.onSell((event) => {
      const announcement = `
        ...
        💰 받은 금액: ${formatEther(event.ethOut)} B3  
        👤 판매자: ${event.seller}
      `;
  ...
  <Accordion title="최적 타이밍">
    ...
    **이주 전 발표:**
    ...
    const announcement = `
      ...
      📊 최종 Bonding 통계:
      • 조달된 금액: ${formatEther(progress.raised)} B3
    ...
    **교육 콘텐츠:**
    ...
    const migrationGuideForUsers = {
      trading: {
        ...
        after: "DEX 인터페이스나 swapService.executeSwap() 사용",
      ...
      risks: {
        ...
        liquidityRisk: "큰 거래는 가격 영향 있을 수 있음",
        smartContractRisk: "표준 De
