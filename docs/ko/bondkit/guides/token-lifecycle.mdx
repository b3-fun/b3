...
이 가이드는 Bondkit 토큰의 초기 생성부터 완전한 분산화에 이르기까지의 전체 수명주기를 안내합니다. 성공적인 토큰 출시와 관리를 위해 이 흐름을 이해하는 것이 필수적입니다.
...
<AccordionGroup>
  <Accordion title="파라미터 선택">
    **중요 결정사항:**
    ...
    | **aggressivenessFactor** | 가격 곡선의 가파름 | 균형잡힌 출시를 위해 30-60 |
    | **targetAmount** | 이주 임계값 | 5-50 거래 토큰 |
    | **tradingToken** | 사용자가 지불할 토큰 | Base 메인넷에는 B3 사용 |
  ...
  <Accordion title="수수료 구조 설정">
    ...
    **수수료 흐름:**
    ...
    - **Post-Migration**: 0.3% 거래 수수료 → LP 제공자
  </Accordion>
</AccordionGroup>
