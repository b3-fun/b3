---
title: 전체 토큰 라이프사이클
description: 'End-to-end 가이드: 토큰 생성부터 본딩 단계를 거쳐 전체 DEX 거래 및 분산화에 이르기까지.'
lang: ko
originalPath: bondkit/guides/token-lifecycle.mdx
---
## 개요

이 가이드는 Bondkit 토큰의 초기 생성부터 완전한 분산화에 이르기까지의 전체 생명주기를 안내합니다. 이 흐름을 이해하는 것은 성공적인 토큰 출시와 관리에 필수적입니다.
    ...
    **핵심 지표 대시보드:**
    ...
    async function getDashboardData(token) {
      ...
      return {
        ...
        // 상태
        phase: status === 1 ? '결합' : status === 2 ? 'DEX' : '기타'
      };
    }
  ...
  <Accordion title="최적의 타이밍">
    ...
    **이주 전 발표:**
    ...
    const announcement = `
      ...
      📊 최종 결합 통계:
      • 조성된 자금: ${formatEther(progress.raised)} B3
  ...
  <Tab title="유동성 관리">
    ...
    **유동성 인센티브 프로그램:**
    ...
    const lpIncentives = {
      ...
      bonusMultipliers: {
        week1: 2.0,  // 첫 주에 2배 보상
        month1: 1.5, // 첫 달에 1.5배 보상
    ...
    **교육 콘텐츠:**
    ...
    const migrationGuideForUsers = {
      ...
      benefits: {
        ...
        decentralization: "남아 있는 관리자 제어 없음"
      },
      
      risks: {
        priceVolatility: "시장 주도 가격, 변동성 있을 수 있음",
        liquidityRisk: "큰 거래는 가격에
