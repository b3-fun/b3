---
title: 문제 해결 가이드
description: 'Bondkit SDK의 일반적인 문제, 지갑 문제 및 통합 문제에 대한 해결책.'
lang: ko
...
async function bondkitHealthCheck() {
  try {
    // 팩토리 연결 테스트
    const factory = new BondkitTokenFactory(8453); // 베이스 메인넷
...
<AccordionGroup>
  <Accordion title="Error: 'Invalid contract address is required for BondkitToken'">
    **원인:** 토큰 주소가 누락되었거나 유효하지 않거나 체크섬이 적용되지 않았습니다.
  ...
  <Accordion title="Error: 'Unsupported chainId: X. This SDK is configured for Base only'">
    ...
    **해결책:**
    ...
    const factory = new BondkitTokenFactory(8453); // 명시적 베이스 ID
    ```
  </Accordion>
  
  <Accordion title="Error: 'DeployBondkitToken: execution reverted'">
    **원인:** 유효하지 않은 배포 매개변수 또는 권한 부족입니다.
    ...
    **해결책:**
    ...
    const config = {
      ...
      lpSplitRatioFeeRecipientBps: 1000n, // 0-10000(기준점)이어야 함
...
<AccordionGroup>
  ...
  <Accordion title="Error: 'BondingPhaseEnded' or DEX trading issues">
    **원인:** 토큰이 DEX 단계로 이전되었지만 여전히 본딩 방법을 사용합니다.
...
<AccordionGroup>
  <Accordion title="Error: 'Account for transaction could not be determined'">
    **원인:** 지갑이 제대로 연결되지 않았거나 구성되지 않았습니다.
  ...
  <Accordion title="Error: 'Transaction confirmation timeout'">
    ...
    **해결책:**
    ...
    const options = {
      ...
      maxFeePerGas: parseGwei("20"), // 더 빠른 포함을 위한 높은 수수료
    ...
    async function ensureBaseNetwork() {
      ...
      try {
        await window.ethereum.request({
          ...
          params: [{ chainId: '0x2105' }], // 베이스 메인넷
        });
        return true;
      } catch (switchError) {
        // 네트워크가 추가되지 않았다면, 추가하기
...
<AccordionGroup>
  <Accordion title="RPC Connection Problems">
    **증상:** 응답이 느리거나 타임아웃 또는 연결 오류가 발생합니다.
    ...
    **해결책:**
    ...
    async function testRPCHealth() {
      ...
      for (const rpcUrl of rpcUrls) {
        try {
          ...
          return rpcUrl; // 첫 번째로 작동하는 RPC 반환
        } catch (error) {
    ...
    // 필요한 경우 SDK RPC 재정의
    const factory = new BondkitTokenFactory(845
