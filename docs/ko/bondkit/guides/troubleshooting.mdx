...
<AccordionGroup>
  ...
  <Accordion title="오류: '지원되지 않는 chainId: X. 이 SDK는 Base만을 위해 설정되었습니다'">
  ...
  <Accordion title="오류: 'DeployBondkitToken: 실행이 되돌아왔습니다'">
    ...
    **해결책:**
    ...
    const config = {
      ...
      lpSplitRatioFeeRecipientBps: 1000n, // 0-10000 (기준점)을 초과할 수 없음
...
<AccordionGroup>
  <Accordion title="오류: '구매: 실행이 되돌아왔습니다' 또는 '판매: 실행이 되돌아왔습니다'">
    ...
    **해결책:**
    ...
    if (status !== 1) {
      throw new Error("토큰이 본딩 단계에 있지 않습니다 - 대신 DEX 메소드를 사용하세요");
    }
    ...
    async function diagnoseSellError(token, tokenAmount) {
      ...
      if (progress.raised === 0n) {
        return "사용 가능한 유동성 없음 - 모금액이 없을 때 판매할 수 없음";
      }
  ...
  <Accordion title="오류: 'BondingPhaseEnded' 또는 DEX 거래 문제">
    **원인:** 토큰이 DEX 단계로 이주되었지만 여전히 본딩 메소드를 사용하고 있습니다.
...
<AccordionGroup>
  <Accordion title="오류: '거래를 위한 계정을 결정할 수 없습니다'">
    ...
    async function waitForTransactionWithTimeout(token, hash, timeoutMs = 300000) {
      ...
      while (Date.now() - start < timeoutMs) {
        try {
          ...
          throw error; // 다른 오류, 재발생
        }
      }
      
      throw new Error(`거래 ${hash}가 ${timeoutMs}ms 후에 시간 초과됨`);
    }
    ```
  </Accordion>
    ...
    **일반적인 지갑 문제:**
      ...
      try {
        ...
        if (switchError.code === 4902) {
          try {
            ...
            console.error('Base 네트워크를 추가하지 못했습니다', addError);
            return false;
          }
        }
        return false;
    ...
    async function addTokenToWallet(tokenAddress, symbol, decimals = 18) {
      try {
        ...
        console.log(`${symbol}이 지갑에 추가되었습니다`);
      } catch (error) {
        console.error('지갑에
