  ...
  <Accordion title="오류: 'DeployBondkitToken: 실행이 되돌아왔습니다'">
    ...
    **해결책:**
    ...
    const config = {
      ...
      lpSplitRatioFeeRecipientBps: 1000n, // 0-10000(기준 포인트)이어야 함
...
<AccordionGroup>
  <Accordion title="오류: '구매: 실행이 되돌아왔습니다' 또는 '판매: 실행이 되돌아왔습니다'">
    ...
    **해결책:**
    ...
    if (status !== 1) {
      throw new Error("토큰이 결합 단계에 있지 않습니다 - 대신 DEX 방법을 사용하세요");
    }
    ...
    async function diagnoseSellError(token, tokenAmount) {
      ...
      if (progress.raised === 0n) {
        return "사용 가능한 유동성 없음 - 모금이 없을 때 판매할 수 없음";
      }
  ...
  <Accordion title="오류: 'BondingPhaseEnded' 또는 DEX 거래 문제">
    **원인:** 토큰이 DEX 단계로 이주했지만 여전히 결합 방법을 사용하고 있습니다.
