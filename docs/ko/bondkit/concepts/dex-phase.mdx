...
DEX 단계는 Bondkit 토큰의 수명 주기의 마지막 단계로, 거래가 자동 결합 곡선에서 완전히 분산된 Uniswap v4 풀로 전환되는 시점입니다. 이 단계는 이전 직후에 시작되며 완전한 분산화를 나타냅니다.

```mermaid
stateDiagram-v2
    [*] --> BondingPhase: 토큰 생성됨
    BondingPhase --> MigrationReady: 목표 도달
...
1. **계약 상태**: 토큰 상태가 `TokenStatus.Dex`로 변경됨
2. **Uniswap v4 풀**: 축적된 유동성으로 새 풀 생성
3. **가격 초기화**: 결합 곡선 탈출 가격에서 초기 풀 가격 계산
4. **소유권 포기**: 계약 소유권을 제로 주소로 이전
5. **거래 활성화**: 전체 DEX 거래가 즉시 시작됨

### 풀 구성

**자동 풀 설정:**
```typescript
// 이전에서 생성된 풀 매개변수 (from migration)
const poolConfig = {
...
**메소드 사용 가능:**
- ✅ `token.buy()` / `token.sell()` - **비활성화됨** (거부될 것임)
...
Uniswap v4에서 Bondkit 토큰은 향상된 기능을 위한 커스텀 훅을 포함합니다:

```typescript
// 훅 기능 (자동 적용됨)
const hookFeatures = {
  beforeSwap: "커스텀 수수료 계산",
  afterSwap: "분석 추적", 
...
await swapService.executeSwap(...); // ✅ 대신 이것 사용
```
...
<AccordionGroup>
  <Accordion title="토큰이 결합 단계로 돌아갈 수 있나요?">
    **아니요** - 이전은 영구적이며 되돌릴 수 없습니다. DEX 단계에 있는 토큰은 항상 Uniswap v4에서 거래됩니다.
  </Accordion>
  
  <Accordion title="사용되지 않은 결합 곡선 유동성은 어떻게 되나요?">
    결합 단계에서 축적된 모든 거래 토큰은 탈출 가격에서 계산된 bondkit 토큰과 함께 Uniswap v4 풀의 초기 유동성이 됩니다.
  </Accordion>
  
  <Accordion title="DEX 단계에서 제한 사항이 있나요?">
    제한 없음 - 토큰은 표준 ERC20이 되어 Uniswap v4에서 거래됩니다. 전체 DeFi 합성성이 활성화됩니다.
  </Accordion>
  
  <Accordion title="거래 수수료는 어떻게 다른가요?">
    DEX 단계는 수수료 수령자에게 5%의 결합 단계 수수료 대신 유동성 제공자에게 가는 표준 Uniswap v4 수수료(0.3%)를 사용합니다.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  <Card title="DEX 거래 시작하기" icon="arrow-right" href="/bondkit/sdk/reference#bondkitswapservice">
    DEX 거래를 위해 BondkitSwapService 사용 방법 알아보기
  </Card>
  <Card title="거래 차트 추가하기" icon="chart-line" href="/bondkit/sdk/reference#tradingview-component">  
