...
DEX 단계는 Bondkit 토큰의 생명주기의 마지막 단계로, 거래가 자동화된 본딩 곡선에서 완전히 분산된 Uniswap v4 풀로 전환되는 단계입니다. 이 단계는 마이그레이션 직후에 시작되며 완전한 분산화를 나타냅니다.
...
2. **Uniswap v4 풀**: 누적된 유동성으로 새 풀이 생성됩니다.
3. **가격 초기화**: 본딩 곡선 출구 가격에서 초기 풀 가격이 계산됩니다.
4. **소유권 포기**: 계약 소유권이 제로 주소로 이전됩니다.
...
const poolConfig = {
  ...
  fee: 3000,        // 0.3% 표준 수수료
  tickSpacing: 60,  // 표준 틱 간격
  hooks: customHook // Bondkit 특정 훅
};
```

**초기 유동성:**
- **거래 토큰**: 본딩 단계에서 누적된 모든 자금
- **Bondkit 토큰**: 출구 가격과 일치하도록 계산된 양
- **LP 토큰**: 수수료 수령자와 소각/분배 사이에 분할됨
...
// 마이그레이션에서 초기 유동성
const migrationData = {
  tradingTokenLiquidity: "10.0",    // 본딩에서의 ETH/B3
  bondkitTokenLiquidity: "500000",  // 계산된 토큰 양
  initialPrice: "0.00002",          // 토큰 당 가격
...
Uniswap v4에서 Bondkit 토큰은 향상된 기능을 위한 사용자 정의 훅을 포함합니다:
...
```typescript
// 마이그레이션 전: 직접 제어
await token.buy(amount, minOut);    // ✅ 작동함
await token.migrateToDex();         // 일회성 작업

// 마이그레이션 후: 시장 기반
await token.buy(amount, minOut);    // ❌ 되돌림
...
<AccordionGroup>
  <Accordion title="최적의 거래 전략">
    **슬리피지 관리:**
    - 소규모 거래에 대해 0.5-2% 슬리피지 사용
    - 더 큰 금액에 대해 더 높은 슬리피지
    - 실행 전 가격 영향 확인
...
<AccordionGroup>
  ...
  <Accordion title="사용되지 않은 본딩 곡선 유동성은 어떻게 되나요?">
    본딩 단계에서 누적된 모든 거래 토큰은 출구 가격에서 계산된 bondkit 토큰과 짝을 이루어 Uniswap v4 풀의 초기 유동성이 됩니다.
  </Accordion>
  
  <Accordion title="DEX 단계에서 제한 사항이 있나요?">
    제한 사항 없음 - 토큰은 Uniswap v4에서 거래되는 표준 ERC20이 됩니다. 전체 DeFi 조합성이 활성화됩니다.
  </Accordion>
  
  <Accordion title="거래 수수료는 어떻게 다른가요?">
    DEX 단계는 수수료 수령자에게 가는 5% 본딩 단계 수수료 대신 표준 Uniswap v4 수수료(0.3%)를 사용하며, 이는 유동성 제공자에게 갑니다.
  </Accordion>
</AccordionGroup>
