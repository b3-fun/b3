...
DEX 단계는 Bondkit 토큰의 수명 주기의 마지막 단계로, 거래가 자동 결합 곡선에서 완전히 분산된 Uniswap v4 풀로 전환됩니다. 이 단계는 마이그레이션 직후에 시작되며 완전한 분산화를 나타냅니다.
...
2. **Uniswap v4 풀**: 누적된 유동성으로 새 풀 생성
...
**초기 유동성:**
- **거래 토큰**: 결합 단계에서 누적된 모든 자금
- **Bondkit 토큰**: 출구 가격에 맞춰 계산된 양
...
// 마이그레이션에서 초기 유동성
const migrationData = {
  tradingTokenLiquidity: "10.0",    // 결합에서의 ETH/B3
...
**메소드 사용 가능성:**
- ✅ `token.buy()` / `token.sell()` - **비활성화됨** (거부됨)
...
Uniswap v4에 있는 Bondkit 토큰은 향상된 기능을 위한 사용자 정의 후크를 포함합니다:
...
<AccordionGroup>
  ...
  <Accordion title="사용되지 않은 결합 곡선 유동성은 어떻게 되나요?">
    결합 단계에서 누적된 모든 거래 토큰은 출구 가격에서 계산된 bondkit 토큰과 짝을 이루어 Uniswap v4 풀의 초기 유동성이 됩니다.
  </Accordion>
  
  <Accordion title="DEX 단계에서 제한 사항이 있나요?">
    제한 사항 없음 - 토큰은 Uniswap v4에서 거래되는 표준 ERC20이 됩니다. 전체 DeFi 결합성이 활성화됩니다.
  </Accordion>
  
  <Accordion title="거래 수수료는 어떻게 다른가요?">
    DEX 단계는 수수료 수령자에게 가는 5% 결합 단계 수수료 대신 표준 Uniswap v4 수수료(0.3%)를 사용하며, 이는 유동성 제공자에게 갑니다.
  </Accordion>
</AccordionGroup>
