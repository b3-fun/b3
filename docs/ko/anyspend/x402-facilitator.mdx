...
AnySpend x402 Facilitator는 다중 토큰, 크로스 체인 결제를 지원하는 생산급 결제 인프라입니다. 토큰 스왑, 크로스 체인 브리징, 결제 처리의 모든 복잡성을 처리하여 개발자가 원하는 토큰으로 결제를 받을 수 있게 하면서 선호하는 화폐로 수령할 수 있습니다.
...
- **커스텀 토큰**: permit 지원을 포함한 모든 토큰, B3과 같은 프로젝트 토큰
- **19+ 네트워크**: EVM 체인(Base, Ethereum, Arbitrum 등)과 SVM(Solana) 전반에 걸친 지원
...
- **크로스 VM 결제**: Solana ↔ EVM 전송에 대한 실험적 지원
- **최적 라우팅**: 최상의 실행을 위한 스마트 루트 선택

### 자동 토큰 변환

지원되는 모든 토큰 간에 원활하게 변환합니다:

- **구매자 유연성**: 사용자는 소유한 토큰으로 결제
- **판매자 선호**: 상인은 선택한 결제 화폐를 수령
- **최상의 가격**: 최적의 DEX 유동성 소스를 통한 라우팅
- **슬리피지 보호**: 가격 영향에 대한 자동 보호
...
- ✅ **정확한 결제 스킴** 수령인 및 금액 검증 포함
- ✅ **서명 기반 인증** (EIP-712, EIP-2612, EIP-3009)
- ✅ **신뢰할 수 있는 제3자 없는 암호학적 검증**
...
**API 레이어**
- 결제 검증 및 결제에 대한 RESTful HTTP API
...
const facilitator = new X402Facilitator({
  ...
  apiKey: process.env.ANYSPEND_API_KEY  // 선택 사항
});
...
const client = new X402Client({
  walletClient,
  facilitatorUrl: 'https://mainnet.anyspend.com/x402'  // 선택 사항, 기본적으로 이를 사용
});
...
전체 세부 정보는 [네트워크 지원](/anyspend/x402-network-support)을 참조하세요.
...
<Card title="프로덕션 준비" icon="rocket">
  엔터프라이즈급 신뢰성 및 성능으로 검증된 인프라
</Card>
