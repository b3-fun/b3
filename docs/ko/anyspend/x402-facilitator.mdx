...
AnySpend x402 Facilitator는 x402 프로토콜을 위한 다중 토큰, 크로스 체인 결제를 처리합니다. 토큰 교환, 크로스 체인 브리징, 결제를 처리하여 개발자가 원하는 토큰으로 결제를 받을 수 있게 하면서 선호하는 화폐로 수령할 수 있습니다.
...
- **커스텀 토큰**: permit 지원 토큰, B3 같은 프로젝트 토큰 포함
...
- **크로스 VM 결제**: Solana ↔ EVM 전송 실험적 지원
...
AnySpend Facilitator는 x402 사양을 완전히 구현합니다:

- `402 Payment Required` 상태를 통한 HTTP 네이티브 결제
- 표준 헤더(`X-PAYMENT`, `X-PAYMENT-RESPONSE`, `X-PREFERRED-TOKEN`)
- 수령인 및 금액 검증을 포함한 정확한 결제 스킴
- 서명 기반 인증(EIP-712, EIP-2612, EIP-3009)
- 신뢰할 수 있는 제3자 없이 암호화 검증
...
- **벤더 종속 없음**: 표준 HTTP API, 언제든지 facilitator 전환 가능
...
const client = new X402Client({
  walletClient,
  facilitatorUrl: 'https://mainnet.anyspend.com/x402'  // 선택사항, 기본적으로 이것을 사용
});
...
facilitator는 사용자 개인 키에 **절대** 접근하지 않습니다:
