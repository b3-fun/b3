...
AnySpend x402 Facilitator는 x402 프로토콜을 위한 멀티 토큰, 크로스 체인 결제를 지원하는 생산 등급 결제 인프라입니다. 토큰 스왑, 크로스 체인 브리징, 정산의 모든 복잡성을 처리하여 개발자가 원하는 토큰으로 결제를 받을 수 있게 하면서 선호하는 화폐로 수령할 수 있습니다.
...
- **커스텀 토큰**: permit 지원을 포함한 모든 토큰, 예를 들어 B3 프로젝트 토큰 등
...
- **동일 체인 결제**: 최소한의 오버헤드를 가진 직접 토큰 전송
- **크로스 체인 결제**: 네트워크 간 자동 브리징 (예: ETH → Base)
- **크로스 VM 결제**: Solana ↔ EVM 전송의 실험적 지원
...
- **판매자 선호**: 판매자는 선택한 정산 화폐를 수령
- **최상의 가격**: 최적의 DEX 유동성 소스를 통한 라우팅
- **슬리피지 보호**: 가격 영향에 대한 자동 보호 장치
...
- ✅ **정확한 결제 체계** 수령인 및 금액 검증 포함
- ✅ **서명 기반 인증** (EIP-712, EIP-2612, EIP-3009)
- ✅ **신뢰할 수 있는 제3자 없는 암호화 검증**

### 드롭인 교체

모든 x402 구현과 완전히 호환됩니다:
...
**API 레이어**
- 결제 검증 및 정산을 위한 RESTful HTTP API
...
app.use(paymentMiddleware(
  '0xYourAddress...',
  endpointConfig,
  facilitator  // mainnet.anyspend.com/x402에 사전 구성됨
));
```
...
const client = new X402Client({
  walletClient,
  facilitatorUrl: 'https://mainnet.anyspend.com/x402'  // 선택사항, 기본적으로 이를 사용
});
...
전체 세부 정보는 [네트워크 지원](/anyspend/x402-network-support)에서 확인하세요.

## AnySpend Facilitator을 선택하는 이유?

<CardGroup cols={2}>
<Card title="표준 준수" icon="check-circle">
  완전한 x402 프로토콜 준수 - 어떤 호환 클라이언트나 서버와도 작동
</Card>

<Card title="생산 준비 완료" icon="rocket">
  엔터프라이즈 등급의 신뢰성 및 성능을 갖춘 전투 검증된 인프라
</Card>
...
<Card title="판매자를 위한 빠른 시작" icon="money-bill" href="/anyspend/x402-quickstart-sellers">
  API에서 멀티 토큰 결제를 시작하세요
</Card>

<Card title="구매자를 위한 빠른 시작" icon="wallet" href="/anyspend/x402-quickstart-buy
