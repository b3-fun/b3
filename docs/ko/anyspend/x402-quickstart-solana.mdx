...
---
<Note>
**실시간으로 체험해 보세요!** Solana에서 X402를 [x402-demo.anyspend.com](https://x402-demo.anyspend.com/)에서 체험하거나 [GitHub에서 소스 코드를 탐색하세요](https://github.com/b3-fun/anyspend-x402-demo).
</Note>
...
이 가이드는 서버(결제 수락) 및 클라이언트(결제 수행) 애플리케이션 모두에서 **Solana**에서 **AnySpend X402**를 사용하는 방법을 보여줍니다. Solana에서의 AnySpend X402는 거래 서명을 사용하여 가스 없는 SPL 토큰 결제를 가능하게 하여, 유료 API, AI 에이전트 및 프리미엄 서비스에 완벽합니다.

**주요 기능:**
- **가스 없는 거래** - 중개자가 모든 거래 수수료를 지원
...
3. 클라이언트가 USDC 전송을 포함한 Solana 거래에 서명합니다
4. 중개자가 검증하고 거래를 제출합니다(가스 없음)
...
async function main() {
  ...
  if (paymentHeader) {
    ...
    // 거래는 다음에서 확인 가능: https://solscan.io/tx/${payment.transaction}
  }
}
...
**핵심 포인트:**
- Base58로 인코딩된 개인 키 사용(Solana 키 페어에서)
- 자동으로 402 응답 및 결제 흐름 처리
- `x-payment-response` 헤더에서 거래 해시 반환
...
- 사용자에게 거래 서명 승인 요청
- 완전히 가스 없음 - 사용자는 서명만 하고, 중개자가 수수료 지불
...
<AccordionGroup>
<Accordion title="거래 서명 검증 실패">
확인하세요:
- 지갑에 결제에 충분한 USDC가 있는지
...
- **라이브 데모**: 직접 체험해 보세요 [x402-demo.anyspend.com](https://x402-demo.anyspend.com/)
- **데모 소스 코드**: [github.com/b3-fun/anyspend-x402-demo](https://github.com/b3-fun/anyspend-x402-demo)
- **Discord**: 우리의 [Discord 커뮤니티에 참여하세요](https://discord.gg/b3dotfun)
