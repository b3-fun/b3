...
---
<Note>
**실시간 체험해 보세요!** Solana에서 X402를 [x402-demo.anyspend.com](https://x402-demo.anyspend.com/)에서 체험하거나 [GitHub에서 소스 코드를 탐색하세요](https://github.com/b3-fun/anyspend-x402-demo).
</Note>
...
이 가이드는 서버(결제 수락) 및 클라이언트(결제 수행) 애플리케이션에서 **Solana**에서 **AnySpend X402**를 사용하는 방법을 보여줍니다. Solana에서의 AnySpend X402는 트랜잭션 서명을 사용하여 가스 없는 SPL 토큰 결제를 가능하게 하여, 유료 API, AI 에이전트 및 프리미엄 서비스에 완벽합니다.
    ...
    SOLANA_PAYTO_ADDRESS,
    {
      "POST /api/solana/premium": {
        price: {
          ...
          asset: {
            address: 'EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v', // Solana의 USDC
            decimals: 6,
          },
        },
        network: 'solana',
...
app.listen(3001, () => {
  console.log('Solana X402 서버가 3001 포트에서 실행 중');
});
```
...
4. 퍼실리테이터가 트랜잭션을 검증하고 제출함(가스 없음)
...
**핵심 포인트:**
- base58로 인코딩된 개인 키 사용(Solana 키페어에서)
...
- 올바른 네트워크(mainnet 대비 devnet)에 있는지
- 퍼실리테이터 URL이 올바른지
</Accordion>
...
- 지갑 확장 프로그램(Phantom, Solflare 등)이 설치되어 있는지
...
- **Discord**: [Discord 커뮤니티](https://discord.gg/b3dotfun)에 참여하세요
