...
이 가이드는 서버(결제 수락) 및 클라이언트(결제 수행) 애플리케이션 모두에서 **Solana**에서 **AnySpend X402**를 사용하는 방법을 보여줍니다. Solana에서의 AnySpend X402는 거래 서명을 사용하여 가스 없는 SPL 토큰 결제를 가능하게 하여, 유료 API, AI 에이전트 및 프리미엄 서비스에 이상적입니다.
...
- **Solana 또는 Express.js의 기본 지식** (사용 사례에 따라 다름)
...
async function main() {
  ...
  if (paymentHeader) {
    ...
    // 거래 주소: https://solscan.io/tx/${payment.transaction}
  }
}
...
다른 SPL 토큰의 경우, 민트 주소와 소수점을 지정하세요.
...
<Accordion title="결제가 성공했지만 데이터가 반환되지 않음">
확인하세요:
...
- **Discord**: [Discord 커뮤니티](https://discord.gg/b3dotfun)에 가입하세요
