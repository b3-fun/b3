...
description: Solana에서 AnySpend X402를 사용하여 USDC 및 SPL 토큰으로 결제를 수락하고 지불하세요
lang: ko
originalPath: anyspend/x402-quickstart-solana.mdx
---
<Note>
**실시간으로 체험해보세요!** Solana에서 X402를 [x402-demo.anyspend.com](https://x402-demo.anyspend.com/)에서 체험하거나 [GitHub에서 소스 코드를 탐색하세요](https://github.com/b3-fun/anyspend-x402-demo).
</Note>
...
이 가이드는 서버(결제 수락) 및 클라이언트(결제 수행) 양쪽 애플리케이션에서 **Solana**에서 **AnySpend X402**를 사용하는 방법을 보여줍니다. Solana에서의 AnySpend X402는 트랜잭션 서명을 사용하여 가스 없는 SPL 토큰 결제를 가능하게 하여, 유료 API, AI 에이전트 및 프리미엄 서비스에 완벽합니다.

**주요 기능:**
- **가스 없는 트랜잭션** - 중개자가 모든 트랜잭션 수수료를 후원
- **SPL 토큰 지원** - Solana에서 USDC 및 기타 SPL 토큰으로 결제
...
- **Node.js 18+** 설치됨
- **Solana 지갑**에 일부 USDC가 있음 (테스트용)
- **Solana 또는 Express.js에 대한 기본 지식** (사용 사례에 따라 다름)

## 서버 설정

몇 줄의 코드로 Solana USDC 결제를 수락합니다.
    ...
    SOLANA_PAYTO_ADDRESS,
    {
      "POST /api/solana/premium": {
        price: {
          ...
          asset: {
            address: 'EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v', // Solana에서의 USDC
            decimals: 6,
          },
        },
        network: 'solana',
...
**이 작업의 의미:**
1. 클라이언트가 엔드포인트를 요청함
2. 미들웨어가 Solana 결제 세부 정보와 함께 `402 Payment Required`를 반환함
3. 클라이언트가 USDC 전송을 포함한 Solana 트랜잭션에 서명함
4. 중개자가 트랜잭션을 검증하고 제출함 (가스 없음)
5. 결제 확인 후 라우트 핸들러가 실행됨
...
Node.js 스크립트, AI 에이전트 및 서버 측 애플리케이션에 적합합니다.
...
const privateKey = process.env.SOLANA_PRIVATE_KEY; // Base58 개인 키
...
async function main() {
  ...
  if (paymentHeader) {
    ...
    // 트랜잭션은 다음에서 확인 가능: https://solscan.io/tx/${payment.transaction}
  }
}
...
**핵심 포인트:**
- Solana 키쌍에서 base58로 인코딩된 개인 키 사용
...
Phantom, Solflare, Ledger 또는 기타 Solana 지갑을 사용하는 웹 애플리케이션용입니다.
  ...
  return (
    <div>
      ...
      <button onClick={fetchPremiumData} disabled={!connected || loading}>
        {loading ? "처리 중..." : "프리미엄 데이터 가져오기 (0.01 USDC)"}
      </button>
...
- 완전히 가스 없음 - 사용자는 서명만 하고 중개자가 수수료를 지불
...
- **라이브 데모**: [x402-demo.anyspend.com](https://x402-demo.anyspend.com/)에서 직접 체험해보세요
- **데모 소스 코드**: [github.com/b3-fun/anyspend-x402-demo](https://github.com/b3-fun/anyspend-x402-demo)
- **Discord**: 우리의 [Discord 커뮤니티](https://discord.gg/b3dotfun)에 가입하세요
- **GitHub**: [github.com/b3-fun/anyspend-x402](https://github.com/b3-fun/anyspend-x402)
- **예제**: [저장소](https://github.com/b3-fun/anyspend-x402/tree/main/examples/typescript)에서 전체 예제 보기
