...
이 가이드는 **AnySpend x402** 결제 지원을 Express API에 추가하는 방법을 보여줍니다. 이를 통해 모든 토큰으로 결제를 받고 원하는 토큰으로 수령할 수 있습니다. 몇 줄의 코드만으로 API가 엔드포인트를 수익화하고 자동 결제 검증 및 정산을 처리할 수 있습니다.

**주요 이점:**
- **구매자**는 보유한 모든 토큰(ETH, DAI, USDC 등)으로 결제할 수 있습니다.
- **판매자**는 선호하는 모든 토큰(USDC, B3, DAI, 사용자 정의 토큰 등)을 받을 수 있습니다.
...
API를 구성하여 USDC뿐만 아니라 원하는 모든 토큰을 받을 수 있습니다:
...
- 구매자는 여전히 **모든 토큰**으로 결제할 수 있습니다 (ETH, USDC, DAI 등).
- AnySpend가 그들의 결제를 **B3 토큰**으로 변환합니다.
- 지갑에 정확히 **100 B3 토큰**을 받습니다.
- 구매자와 판매자 모두 토큰 선호도를 유지합니다.
...
사용자 정의 토큰을 구성하려면 EIP-712 매개변수를 알아야 합니다. 다음은 그 방법입니다:
...
그런 다음 클라이언트는 토큰 선호도를 지정할 수 있습니다:
...
const customFacilitator = new Facilitator({
  ...
  apiKey: process.env.CDP_API_KEY_ID,        // 선택사항: Coinbase C
