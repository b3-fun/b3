...
이 가이드는 Express API에 **AnySpend x402** 결제 지원을 추가하는 방법을 보여줍니다. 이를 통해 어떤 토큰으로든 결제를 받고 원하는 토큰으로 수령할 수 있습니다. 몇 줄의 코드만으로 API가 엔드포인트를 수익화하고 자동 결제 검증 및 정산을 처리할 수 있습니다.

**주요 이점:**
- **구매자**는 보유한 어떤 토큰(ETH, DAI, USDC 등)으로도 결제할 수 있습니다.
- **판매자**는 원하는 어떤 토큰(USDC, B3, DAI, 사용자 정의 토큰 등)을 받을 수 있습니다.
...
const endpointConfig = {
  '/api/premium-data': {
    amount: '1.00',        // USDC로 $1.00
    asset: 'USD'           // USDC로 변환됩니다
  },
  '/api/ai-inference': {
    amount: '0.50',        // USDC로 $0.50
    asset: 'USD'
  }
};
...
2. 미들웨어는 결제 세부 정보와 함께 `402 Payment Required`를 반환합니다.
3. 클라이언트는 선호하는 토큰(ETH, DAI 등)으로 결제합니다.
...
API를 구성하여 USDC뿐만 아니라 원하는 토큰을 받을 수 있습니다:
...
- 구매자는 여전히 **어떤 토큰** (ETH, USDC, DAI 등)으로도 결제할 수 있습니다.
- AnySpend는 그들의 결제를 **B3 토큰**으로 변환합니다.
- 지갑에 정확히 **100 B3 토큰**을 받습니다.
- 구매자와 판매자 모두에게 완전한 유연성을 제공합니다!
...
**이점:**
- 클라이언트는 지원되는 어떤 토큰으로도 결제할 수 있습니다.
- AnySpend가 자동으로 USDC로 변환합니다.
- 항상 USDC를 받습니다.

#### 방법 2: 특정 토큰 요구 사항

특정 네트워크에서 특정 토큰을 요구합니다. USDC만 받는 것이 아니라 **어떤 토큰**이든 받을 수 있습니다!
...
const endpointConfig = {
  '/api/data': {
    ...
    asset: '0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913', // Base의 USDC
    network: 'base-mainnet'
  }
};
```

**예제 2: B3 토큰 (사용자 정의 토큰) 받기**
...
const endpointConfig = {
  'POST /api/premium': {
    ...
    network: NETWORK,
    config: {
      description: 'CoinGecko에서 프리미엄 ETH 가격 이력 데이터 접근',
      mimeType: 'application/json',
    },
  }
};
...
const endpointConfig = {
  '/api/ai-inference': {
    ...
    asset: '0x50c5725949A6F0c72E6C4a641F24049A917DB0Cb', // Base의 DAI
    network: 'base-mainnet'
  }
};
```
...
**예제 5: 사용자 정의 ERC-20 토큰 받기**

허가 지원이 있는 모든 ERC-20 토큰의 경우:
...
<Note>
**핵심 포인트:**
- 구매자는 여전히 **보유한 어떤 토큰**으로도 결제할 수 있습니다.
- AnySpend는 그들의 토큰을 당신이 받고 싶은 토큰으로 자동 변환합니다.
- 구성에서 지정한 토큰을 정확히 받습니다.
- 구매자와 판매자 모두의 유연성이 유지됩니다.
</Note>

### 토큰 세부 정보 찾기

사용자 정의 토큰을 구성하려면 EIP-712 매개변수를 알아야 합니다. 이를 찾는 방법은 다음과 같습니다:
