...
- **지갑 주소** USDC 결제를 받기 위함
- Express 미들웨어에 대한 **기본 지식**

## 설치

AnySpend x402 패키지 설치:
...
특정 네트워크에서 특정 토큰을 요구합니다. USDC만 받는 것이 아니라 **어떤 토큰**도 받을 수 있습니다!
...
const endpointConfig = {
  'POST /api/premium': {
    ...
    network: NETWORK,
    config: {
      description: 'CoinGecko에서 프리미엄 ETH 가격 이력 데이터에 대한 액세스',
      mimeType: 'application/json',
    },
  }
};
...
- AnySpend는 자동으로 그들의 토큰을 원하는 토큰으로 변환합니다.
...
클라이언트가 선호하는 결제 토큰을 지정할 수 있도록 합니다:
...
요청 매개변수를 기반으로 가격을 계산합니다:

```typescript
import { paymentMiddleware, facilitator } from '@b3dotfun/anyspend-x402';

app.use('/api/compute', (req, res, next) => {
  // 요청된 계산 단위를 기반으로 가격 계산
