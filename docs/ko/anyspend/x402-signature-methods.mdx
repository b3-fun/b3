...
| **Nonce 유형** | 순차적 (자동 증가) | 무작위 bytes32 |
| **의존성** | 이전 nonce를 기다려야 함 | 순서 필요 없음 |
| **적합함** | 일반 ERC-20 토큰 | 스테이블코인 결제 |
| **재생 방지** | 순차적 nonce | 무작위 nonce 추적 |
| **가스 효율성** | 2번의 거래 (permit + transferFrom) | 1번의 거래 (직접 전송) |

## EIP-3009: transferWithAuthorization (USDC)

### 개요

직접 전송 승인 - 서명이 별도의 승인 단계 없이 발신자로부터 수신자에게 즉각적인 전송을 승인합니다.
...
- ✅ **무작위 nonce** - 순차적 의존성 없음, 병렬 거래 가능
- ✅ **즉각적인 결제** - 단일 거래에서 실행
- ✅ **지불자에게 가스 없음** - 중개자가 가스 지불
- ✅ **프런트 러닝 없음** - 무작위 nonce가 MEV 공격을 방지
...
const signature = await signTransferWithAuthorization({
  ...
  nonce: randomBytes32()         // 무작위 nonce 생성
});
```
...
사용자가 무작위 nonce를 사용하여 EIP-712 메시지에 전송을 승인하는 서명
</Step>
...
<Step title="USDC 전송됨">
USDC가 사용자로부터 중개자에게 한 번의 원자적 거래로 직접 전송됨
</Step>

<Step title="Nonce 무효화">
무작위 nonce가 사용된 것으로 표시되어 재생 공격을 방지
</Step>
</Steps>
...
승인을 설정하는 서명 기반 승인, 이어서 별도의 `transferFrom()` 호출이 이루어집니다. 이는 대부분의 현대 ERC-20 토큰에 대한 표준 방법입니다.
...
- ✅ **널리 채택됨** - 많은 토큰에서 표준
- ✅ **시간 제한 승인** - 마감 기한 기반 만료
- ✅ **ERC-20 호환** - 기존 인프라와 작동
- ✅ **지불자에게 가스 없음** - 중개자가 가스 지불
- ✅ **생태계 지원** - 주요 지갑과 dapps에서 지원
...
사용자가 현재 순차적 nonce를 사용하여 EIP-712 허가 메시지에 서명
</Step>

<Step title="중개자 Permit 호출">
중개자가 `permit()`을 호출하여 체인상에 승인을 설정
</Step>
...
**EIP-3009:**
- 무작위 nonce가 체인 및 계약 간 재생을 방지
- 각 nonce는 주소당 한 번만 사용 가능
- Nonce 상태가 체인상의 매핑에 저장됨
...
// 2. 현재 nonce(permit용)를 가져오거나 무작위 nonce 생성
...
<CardGroup cols={2}>
<Card title="EIP-2612 명세서" icon="file-lines" href="https://eips.ethereum.org/EIPS/eip-2612">
  공식 EIP-2612 permit 명세서
</Card>

<Card title="EIP-3009 명세서" icon="file-lines" href="https://eips.ethereum.org/EIPS/eip-3009">
  공식 EIP-3009 transferWithAuthorization 명세서
</Card>
