---
title: Permit 대 TransferWithAuthorization
...
직접 전송 인증 - 서명이 별도의 승인 단계 없이 발신자로부터 수신자에게 즉시 전송을 인증합니다.
...
사용자가 랜덤 nonce를 사용하여 전송을 인증하는 EIP-712 메시지에 서명
</Step>
...
허용량을 설정하는 서명 기반 승인으로, 별도의 `transferFrom()` 호출을 따릅니다. 이는 대부분의 현대 ERC-20 토큰에 대한 표준 방법입니다.
...
중개자가 체인 상에서 허용량을 설정하기 위해 `permit()`을 호출
</Step>
...
**EIP-3009:**
- 랜덤 nonce가 체인 및 계약 간 재생을 방지
- 각 nonce는 주소당 한 번만 사용 가능
- nonce 상태는 체인 상의 매핑에 저장됨
