...
title: >-
  ...
  **Permit**은 사용자가 단일 트랜잭션으로 토큰 승인과 트랜잭션을 동시에 수행할 수 있게 해주는 기능입니다. 이는 사용자가 토큰을 다른
  주소로 이동하기 전에 별도의 승인 트랜잭션을 보낼 필요가 없다는 것을 의미합니다. Permit을 사용하면 사용자 경험이 향상되고, 트랜잭션
  수수료가 절약됩니다.


  **TransferWithAuthorization**은 비슷한 목적으로 사용되지만, 이는 토큰 소유자가 자신의 토큰을 다른 주소로 직접
  이동시킬 수 있는 사전 서명된 허가를 생성할 수 있게 해줍니다. 이 방법은 토큰 거래를 더 유연하게 만들고, 다양한 유형의 거래와 상호작용을
  가능하게 합니다.


  두 기능 모두 블록체인에서의 토큰 관리와 거래를 더욱 효율적이고 사용자 친화적으로 만들기 위해 설계되었습니다. 그러나 각각의 메커니즘이
  제공하는 보안 수준과 사용 사례는 다를 수 있으므로, 개발자와 사용자는 자신의 요구에 가장 잘 맞는 기능을 선택해야 합니다.
...
AnySpend x402는 전통적인 토큰 승인 대신 암호학적 서명을 사용하여 사용자에게 **가스 없는 결제**를 가능하게 합니다. 사용자는 승인 메시지를 서명하기만 하면 되고, 모든 가스 비용은 중개자가 지불합니다.
...
|---------|-------------------|--------------------------------------|
| **사용됨** | 대부분의 현대 ERC-20 | USDC (모든 네트워크) |
...
- ✅ **프론트 러닝 없음** - 랜덤 nonce가 MEV 공격을 방지
...
사용자가 랜덤 nonce를 사용하여 EIP-712 메시지에 전송 승인을 서명
</Step>
...
<Step title="USDC 전송됨">
USDC가 한 번의 원자 거래에서 사용자로부터 중개자에게 직접 전송됨
</Step>

<Step title="Nonce 무효화">
랜덤 nonce가 사용된 것으로 표시되어 재생 공격을 방지
</Step>
</Steps>
...
서명 기반 승인으로, 별도의 `transferFrom()` 호출을 통해 허용량을 설정합니다. 이는 대부분의 현대 ERC-20 토큰에 대한 표준 방법입니다.

**사용됨:** permit 지원이 있는 대부분의 현대 ERC-20 토큰

### 주요 장점

- ✅ **광범위하게 채택됨** - 많은 토큰에서 표준
- ✅ **시간 제한 승인** - 마감 기반 만료
...
사용자가 현재 순차적 nonce를 사용하여 EIP-712 permit 메시지에 서명
</Step>

<Step title="중개자가 Permit 호출">
중개자가 체인상에 허용량을 설정하기 위해 `permit()`을 호출
</Step>

<Step title="Nonce 자동 증가">
토큰 계약이 사용자의 nonce를 자동으로 증가
</Step>

<Step title="중개자가 토큰 전송">
중개자가 승인을 사용하여 `transferFrom()`을 호출하여 토큰을 전송
</Step>
</Steps>
...
두 방법 모두 마감 시간을 집행하여 오래된 서명을 방지합니다:
...
<Note>
모든 가스 비용은 중개자가 지불하며 AnySpend 수수료 0.25%에 포함됩니다. 사용자는 직접 가스를 지불하지 않습니다.
</Note>
