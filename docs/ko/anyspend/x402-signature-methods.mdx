...
title: >-
  ...
  Permit과 TransferWithAuthorization의 차이점을 이해하는 것은 블록체인 기술에서 중요한 부분입니다. 이 두 용어는
  토큰 거래에 있어서 사용자의 경험과 보안을 개선하기 위해 사용됩니다.


  <Permit>은 사용자가 단일 트랜잭션으로 토큰을 승인하고 전송할 수 있게 해주는 기능입니다. 이는 사용자가 토큰을 전송하기 전에 별도의
  승인 트랜잭션을 보낼 필요가 없다는 것을 의미합니다. 결과적으로, 거래 과정이 더욱 간소화되고 가스 비용이 절약됩니다.


  <TransferWithAuthorization>은 또 다른 방법으로, 이는 사전에 서명된 허가를 통해 토큰을 전송할 수 있게 해줍니다. 이
  방법은 사용자가 트랜잭션을 생성할 때 자신의 개인 키를 사용하여 서명하고, 이 서명된 메시지를 토큰의 수신자 또는 다른 당사자에게 전달할 수
  있습니다. 수신자는 이 메시지를 사용하여 토큰을 자신의 주소로 전송할 수 있습니다.


  두 방법 모두 사용자가 토큰을 보다 효율적으로 관리할 수 있게 해주며, 특히 다중 서명 지갑이나 스마트 계약과 같은 고급 기능을 사용할 때
  유용합니다. 그러나 각각의 방법은 구현과 사용 사례에 따라 장단점이 있으므로, 개발자와 사용자는 자신의 요구에 가장 잘 맞는 방법을 선택해야
  합니다.
...
지원되는 두 가지 주요 서명 표준이 있으며, 각각 다른 토큰 유형에 최적화되어 있습니다:

- **EIP-3009 (transferWithAuthorization)** - USDC에 사용, 한 단계 직접 전송
- **EIP-2612 (permit)** - 대부분의 현대 ERC-20 토큰에 사용, 승인 + 전송의 두 단계
...
| **이상적인 경우** | 일반 ERC-20 토큰 | 스테이블코인 결제 |
...
직접 전송 승인 - 서명은 별도의 승인 단계 없이 발신자로부터 수신자에게 즉각적인 전송을 승인합니다.
...
<Steps>
<Step title="사용자 서명 승인">
사용자는 랜덤 nonce를 사용하여 EIP-712 메시지에 전송을 승인하는 서명을 합니다
</Step>

<Step title="중개자 전송 실행">
중개자는 서명과 함께 `receiveWithAuthorization()`을 호출합니다
</Step>
...
랜덤 nonce가 사용된 것으로 표시되어 재생 공격을 방지합니다
</Step>
</Steps>
...
승인을 설정한 후 별도의 `transferFrom()` 호출을 따르는 서명 기반 승인입니다. 이는 대부분의 현대 ERC-20 토큰에 대한 표준 방법입니다.
...
사용자는 현재 순차적 nonce로 EIP-712 허가 메시지에 서명합니다
</Step>

<Step title="중개자 Permit 호출">
중개자가 `permit()`을 호출하여 체인에 승인을 설정합니다
</Step>

<Step title="Nonce 자동 증가">
토큰 계약은 사용자의 nonce를 자동으로 증가시킵니다
</Step>
...
**장점:**
- 순서 의존성 없음 - 여러 서명을 어떤 순서로든 사용 가능
- 병렬 거래 가능
...
<CardGroup cols={2}>
<Card title="EIP-2612 사양" icon="file-lines" href="https://eips.ethereum.org/EIPS/eip-2612">
  공식 EIP-2612 permit 사양
</Card>

<Card title="EIP-3009 사양" icon="file-lines" href="https://eips.ethereum.org/EIPS/eip-3009">
  공식 EIP-3009 transferWithAuthorization 사양
</Card>
