...
이 가이드는 **AnySpend x402 클라이언트 SDK**를 사용하여 EIP-2612 또는 EIP-3009 지원이 있는 호환 토큰으로 유료 리소스를 결제하는 방법을 보여줍니다. SDK는 서명 생성과 재시도 로직을 포함한 전체 결제 흐름을 자동으로 처리합니다.

x402의 아름다움은 AnySpend을 사용하는 것뿐만 아니라 **모든 x402 지원 서비스**에 대해 이와 같은 클라이언트를 사용할 수 있다는 것입니다.
...
- 호환 토큰(USDC, DAI 또는 기타 EIP-2612/EIP-3009 토큰)이 있는 **암호화폐 지갑**
- [viem](https://viem.sh) 또는 [ethers.js](https://docs.ethers.org)와 같은 **지갑 라이브러리**
- **x402 지원 서비스에 대한 접근** (유료 API와 같은)

<Note>
**중요**: EIP-2612(Permit) 또는 EIP-3009(TransferWithAuthorization) 지원이 있는 토큰만 AnySpend x402에서 가스 없는 결제와 호환됩니다. [토큰 호환성 검사기](https://anyspend.com/x402-tokens)를 사용하여 토큰이 지원되는지 확인하세요.
</Note>
...
<Note>
**X402의 새로운 기능**: 이제 직접 HTTP 요청을 할 때 또는 기존 HTTP 클라이언트와 통합할 때 유용한 HTTP 헤더를 사용하여 결제 선호도를 지정할 수 있습니다.
</Note>

X402 지원 서비스에 요청을 할 때 이 HTTP 헤더를 사용하여 선호하는 결제 토큰과 네트워크를 지정할 수 있습니다:
...
이 헤더는 모든 HTTP 클라이언트와 함께 작동하며 AnySpend 미들웨어를 사용하는 X402 지원 리소스 서버에서 자동으로 적용됩니다. 헤더가 제공되지 않으면 서버는 기본적으로 USDC를 사용합니다.
...
기본 `fetch` API 또는 기타 HTTP 클라이언트를 사용할 때 이 헤더를 수동으로 추가할 수도 있습니다:
...
interface X402ClientOptions {
  ...
  timeout?: number;                   // 요청 시간 초과(ms 단위) (기본값: 30000)
}
```
...
interface RequestOptions {
  ...
  preferredToken?: string;            // 기본 선호 토큰을 덮어쓰기
}
```
...
**EIP-2612 또는 EIP-3009 지원이 있는 토큰만** AnySpend x402에서 가스 없는 결제와 호환됩니다.
...
<Card title="토큰 호환성 검사기" icon="check-circle" href="https://anyspend.com/x402-tokens">
  19개 이상의 네트워크에서 EIP-2612 또는 EIP-3009 지원이 있는 모든 호환 토큰을 찾아보세요
</Card>

체인별 자세한 토큰 주소는 [네트워크 지원](/anyspend/x402-network-support)을 참조하세요.
