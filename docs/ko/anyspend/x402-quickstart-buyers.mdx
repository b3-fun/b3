...
이 가이드는 **AnySpend x402 Client SDK**를 사용하여 EIP-2612 또는 EIP-3009 지원이 가능한 토큰으로 유료 콘텐츠에 대한 결제를 하는 방법을 보여줍니다. SDK는 서명 생성 및 재시도 로직을 포함한 전체 결제 흐름을 자동으로 처리합니다.

x402의 아름다움은 AnySpend을 사용하는 것뿐만 아니라 **모든 x402 지원 서비스**에 대해 동일한 클라이언트를 사용할 수 있다는 것입니다.

<Card title="토큰 호환성 검사기" icon="check-circle" href="https://anyspend.com/x402-tokens">
  19개 이상의 네트워크에서 EIP-2612 또는 EIP-3009 지원이 가능한 호환 토큰을 탐색하세요
</Card>
...
- **암호화폐 지갑** (USDC, DAI 또는 기타 EIP-2612/EIP-3009 토큰)
- **지갑 라이브러리** [viem](https://viem.sh) 또는 [ethers.js](https://docs.ethers.org) 같은
- **x402 지원 서비스에 대한 접근** (유료 API 같은)

<Note>
**중요**: 오직 EIP-2612 (Permit) 또는 EIP-3009 (TransferWithAuthorization) 지원 토큰만이 AnySpend x402를 통한 가스 없는 결제와 호환됩니다. [토큰 호환성 검사기](https://anyspend.com/x402-tokens)를 사용하여 토큰이 지원되는지 확인하세요.
</Note>
...
const x402Client = new X402Client({
  ...
  preferredToken: '0x...', // 선택사항: 기본 결제 토큰
...
1. 클라이언트가 초기 요청을 함
2. API가 결제 세부 정보와 함께 `402 Payment Required` 반환
3. X402Client가 자동으로 결제 승인 서명
4. 클라이언트가 결제와 함께 요청을 재시도
5. API가 결제를 검증하고 정산
6. 클라이언트가 요청한 데이터를 받음

## 고급 사용법

### 결제 토큰 지정

결제에 사용할 특정 토큰을 지정하는 두 가지 방법이 있습니다:
...
- 기존 `fetch` 기반 코드와 함께 작동
- 결제 동작에 대한 더 세분화된 제어
- 최대 결제 금액 지정 가능
- 사용자 지정 결제 요구 사항 선택기
- 다중 네트워크 서명자 지원

리소스 서버가 AnySpend 미들웨어를 지원하는 경우, USDC 대신 선호하는 토큰으로 동일한 금액을 결제합니다.

### 결제 선호도를 위한 HTTP 헤더 사용

<Note>
**X402의 새로운 기능**: 이제 직접 HTTP 요청을 할 때 또는 기존 HTTP 클라이언트와 통합할 때 결제 선호도를 HTTP 헤더를 사용하여 지정할 수 있습니다.
</Note>

X402 지원 서비스에 요청을 할 때, 이러한 HTTP 헤더를 사용하여 선호하는 결제 토큰 및 네트워크를 지정할 수 있습니다:
...
이 헤더들은 모든 HTTP 클라이언트와 함께 작동하며, AnySpend 미들웨어를 사용하는 X402 지원 리소스 서버에서 자동으로 적용됩니다. 헤더가 제공되지 않는 경우, 서버는 기본적으로 USDC를 사용합니다.
...
기본 `fetch` API나 다른 HTTP 클라이언트를 사용할 때 이러한 헤더를 수동으로 추가할 수도 있습니다:
...
interface X402ClientOptions {
  ...
  timeout?: number;                   // 요청 시간 제한(ms) (기본값: 30000)
}
```
...
function wrapFetchWithPayment(
  ...
  selectPaymentRequirements?: Function, // 사용자 지정 요구 사항 선택기
  config?: {
    timeout?: number;                  // 요청 시간 제한(ms)
...
**오직 EIP-2612 또는 EIP-3009 지원 토큰만이** AnySpend x402와 가스 없는 결제를 위해 호환됩니다.

### EIP-2612와 EIP-3009는 무엇인가요?

- **EIP-2612 (Permit)**: 오프체인 서명을 통한 가스 없는 토큰 승인을 허용
- **EIP-3009 (TransferWithAuthorization)**: 오프체인 서명을 통한 가스 없는 전송을 가능하게 함 (USDC에서 사용됨)

### 일반적으로 호환되는 토큰

- **USDC** - 모든 체인에서 EIP-3009 지원
- **B3** - EIP-2612 허가 지원
- **기타 많은 토큰** - 아래 도구를 사용하여 호환성 확인

<Card title="토큰 호환성 검사기" icon="check-circle" href="https://anyspend.com/x402-tokens">
  19개 이상의 네트워크에서 EIP-2612 또는 EIP-3009 지원이 가능한 모든 호환 토큰을 탐색하세요
</Card>

체인별 상세한 토큰 주소는 [네트워크 지원](/anyspend/x402-network-support)을 참조하세요.
