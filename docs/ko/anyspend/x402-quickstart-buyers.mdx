...
x402의 아름다움은 AnySpend을 사용하는 것뿐만 아니라 **모든 x402 지원 서비스**에 대해 이 동일한 클라이언트를 사용할 수 있다는 것입니다.

<Card title="토큰 호환성 검사기" icon="check-circle" href="https://anyspend.com/x402-tokens">
  19개 이상의 네트워크에서 EIP-2612 또는 EIP-3009 지원을 가진 호환 가능한 토큰을 찾아보세요
</Card>
...
try {
  ...
  console.log('결제 TX:', response.paymentResponse?.txHash);
} catch (error) {
...
const fetchWithPayment = wrapFetchWithPayment(
  fetch,
  walletClient,
  '1000000', // 최대 결제 금액 (6자리 소수점에서 1 USDC)
...
리소스 서버가 AnySpend 미들웨어를 지원하는 경우 USDC 대신 선호하는 토큰으로 동등한 금액을 지불합니다.
...
X402 지원 서비스에 요청을 할 때 이러한 HTTP 헤더를 사용하여 선호하는 결제 토큰과 네트워크를 지정할 수 있습니다:
...
이 헤더들은 모든 HTTP 클라이언트와 함께 작동하며, AnySpend 미들웨어를 사용하는 X402 지원 리소스 서버에서 자동으로 적용됩니다. 헤더가 제공되지 않으면 서버는 기본적으로 USDC를 사용합니다.
...
기본 `fetch` API 또는 기타 HTTP 클라이언트를 사용할 때 이러한 헤더를 수동으로 추가할 수도 있습니다:
...
if (initialResponse.status === 402) {
  ...
  console.log('결제 성공!', JSON.parse(atob(paymentResponse)));
}
```
...
interface X402ClientOptions {
  ...
  timeout?: number;                   // 요청 타임아웃(ms) (기본값: 30000)
}
```
...
function wrapFetchWithPayment(
  ...
  maxPaymentValue: string,             // 최소 단위로 최대 결제 (예: 1 USDC의 경우 '1000000')
...
<Card title="토큰 호환성 검사기" icon="check-circle" href="https://anyspend.com/x402-tokens">
  19개 이상의 네트워크에서 EIP-2612 또는 EIP-3009 지원을 가진 모든 호환 가능한 토큰을 찾아보세요
</Card>
