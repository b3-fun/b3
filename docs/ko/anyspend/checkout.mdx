---
title: 결제하기
description: '장바구니, 주문 요약, 양식, 배송, 할인, 그리고 암호화폐/법정 화폐 결제를 포함한 결제 컴포넌트'
lang: ko
originalPath: anyspend/checkout.mdx
---

체크아웃 컴포넌트는 주문 요약을 한쪽에, 결제를 다른 한쪽에 렌더링하는 두 패널 레이아웃을 제공합니다. 이들은 암호화폐와 법정화폐, 사용자 정의 폼 필드, 배송 옵션, 할인 코드 및 선택적 B3 워크플로 트리거를 지원합니다. [실시간으로 보기](https://sdk-demo.anyspend.com/).

<Info>
  백엔드 주도, 세션 기반 플로우를 위한 **체크아웃 세션 REST API**에 대해서는 [체크아웃 세션](/anyspend/checkout-sessions)을 참조하세요. 이 페이지는 **React 체크아웃 컴포넌트**를 다룹니다.
</Info>
...
주요 체크아웃 컴포넌트는 주문 요약/장바구니 패널과 암호화폐 및 법정화폐 옵션을 지원하는 결제 패널로 구성된 두 패널 레이아웃을 렌더링합니다. 고객 정보 수집, 배송 선택 및 할인 코드를 위한 폼 패널을 선택적으로 포함합니다.
...
<ParamField path="destinationTokenChainId" type="number" required>
  결제에 사용될 체인 ID (예: `8453`은 Base를 위함)
</ParamField>
...
<ParamField path="totalAmount" type="string">
  항목 금액의 합과 다를 경우 총액을 재정의합니다. 할인이나 수수료 후의 총액을 사용할 때 사용합니다.
</ParamField>

<ParamField path="shipping" type="string | { amount: string; label?: string }">
  배송비. wei 단위의 금액을 문자열로 전달하거나 사용자 정의 레이블이 있는 객체를 전달합니다.
</ParamField>

<ParamField path="tax" type="string | { amount: string; label?: string; rate?: string }">
  세금 금액. wei 단위의 금액을 문자열로 전달하거나 레이블과 선택적 비율 표시(예: `"8.5%"`)가 있는 객체를 전달합니다.
</ParamField>

<ParamField path="discount" type="string | { amount: string; label?: string; code?: string }">
  할인 금액(공제로 표시됨). wei 단위의 금액을 문자열로 전달하거나 레이블과 선택적 코드가 있는 객체를 전달합니다.
</ParamField>
...
<ParamField path="mode" type="'page' | 'embedded'" default="'page'">
  독립 실행형의 경우 `page`, 레이아웃 내 인라인의 경우 `embedded`
</ParamField>

<ParamField path="showPoints" type="boolean" default="false">
  주문 상태 요약에 획득한 포인트를 표시합니다
</ParamField>

<ParamField path="showOrderId" type="boolean" default="false">
  주문 상태 요약에 주문 ID를 표시합니다
</ParamField>

<ParamField path="footer" type="ReactNode | null">
  주문 요약에 대한 사용자 정의 푸터입니다. 기본 "Powered by" 푸터를 숨기려면 `null`을 전달합니다.
</ParamField>

#### 사용자 정의

<ParamField path="slots" type="AnySpendSlots">
  UI 섹션을 교체합니다. [사용자 정의](/anyspend/customization#slots)를 참조하세요.
</ParamField>

<ParamField path="content" type="AnySpendContent">
  텍스트/메시지를 재정의합니다. [사용자 정의](/anyspend/customization#content)를 참조하세요.
</ParamField>

<ParamField path="theme" type="AnySpendTheme">
  색상을 구성합니다. [사용자 정의](/anyspend/customization#theme)를 참조하세요.
</ParamField>

<ParamField path="classes" type="AnySpendCheckoutClasses">
  CSS 클래스 재정의입니다. [사용자 정의](/anyspend/customization#css-class-overrides)를 참조하세요.
</ParamField>

#### 사용자 정의 폼

체크아웃 중에 고객 정보를 수집하기 위해 JSON 스키마 또는 사용자 정의 React 컴포넌트를 사용합니다.

<ParamField path="formSchema" type="CheckoutFormSchema">
  고객으로부터 수집할 필드를 정의하는 JSON 스키마 (이메일, 이름, 주소 등). [CheckoutFormSchema](#checkoutformschema) 아래를 참조하세요.
</ParamField>

<ParamField path="formComponent" type="React.ComponentType<CheckoutFormComponentProps>">
  체크아웃 폼으로 렌더링할 사용자 정의 React 컴포넌트입니다. `formSchema`가 충분히 유연하지 않을 때 사용합니다.
</ParamField>
...
<ParamField path="shippingOptions" type="ShippingOption[]">
  라디오 버튼 선택기로 표시될 배송 옵션 배열입니다. 선택된 옵션의 금액은 주문 총액에 자동으로 추가됩니다.
</ParamField>

<ParamField path="collectShippingAddress" type="boolean">
  `true`인 경우, 배송 주소 폼(거리, 도시, 주, 우편번호, 국가)을 렌더링합니다. 주소는 주문의 `callbackMetadata`에 포함됩니다.
</ParamField>
...
<ParamField path="validateDiscount" type="(code: string) => Promise<DiscountResult>">
  할인 코드를 백엔드에 대해 유효성을 검사하는 비동기 함수입니다. 할인 금액이 포함된 `DiscountResult`를 반환합니다. 유효한 할인은 주문 총액에 자동으로 적용됩니다.
</ParamField>
...
`AnySpendCheckout`을 확장하여 사용자가 결제를 완료하면 결제 데이터와 모든 사용자 정의 메타데이터와 함께 B3 워크플로가 자동으로 트리거됩니다.
...
<ParamField path="callbackMetadata" type="object">
  주문에 병합될 메타데이터입니다. `inputs` 필드는 워크플로에서 `{{root.result.inputs.*}}`를 통해 접근할 수 있습니다.
</ParamField>
...
interface CheckoutItem {
  ...
  /** 항목 이미지 URL */
  imageUrl?: string;
  /** wei 단위의 가격 (목적지 토큰의 최소 단위) */
  amount: string;
  /** 수량 */
  quantity: number;
  /** 레이블: 값 쌍으로 표시되는 사용자 정의 메타데이터 (예: { "Size": "Large" }) */
