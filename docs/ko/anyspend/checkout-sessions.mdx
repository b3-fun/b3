...
---
체크아웃 세션을 사용하면 사용자가 결제 방법을 선택하기 전에 서버에서 추적 객체를 생성할 수 있습니다. 세션은 경량 데이터베이스 레코드입니다 -- 외부 API 호출 없음, 주문 생성 없음. 주문은 사용자가 결제를 확정할 때 나중에 이루어집니다.

<Info>
  React 체크아웃 컴포넌트를 찾고 계신가요? [Checkout](/anyspend/checkout)을 참조하세요. 이 페이지는 서버 주도 흐름에 대한 **REST API**를 다룹니다.
</Info>

## 기본 URL

모든 체크아웃 세션 엔드포인트는 AnySpend 서비스에 있습니다:
...
<Steps>
  ...
  <Step title="사용자 결제">
    **암호화폐:** `globalAddress`로 보내기 | **피아트 온램프:** `oneClickBuyUrl`로 리디렉션
  </Step>
  <Step title="완료 여부 폴링">
...
- 호스팅된 체크아웃 페이지에서 사용자가 결제 방법을 선택할 수 있습니다.
- 세션 생성은 로컬 DB 쓰기이므로, 타사 API 오류로 인해 실패하지 않습니다.
...
모든 필드는 선택 사항입니다. 결제 구성(금액, 토큰, 체인)은 주문에 있지 세션에 없습니다.
...
- 세션이 `open` 상태여야 함 (`400`이면 만료/처리 중/완료됨)
...
`success_url` 및 `cancel_url`에 템플릿 변수를 사용합니다:
...
템플릿 변수가 없으면 자동으로 `?sessionId=<uuid>`가 추가됩니다.
...
자동 폴링이 있는 쿼리 훅. 상태가 `complete` 또는 `expired`에 도달하면 폴링을 중지합니다.
...
`checkoutSession`이 설정되면 컴포넌트는 주문 전에 세션을 생성하고, 리디렉션을 위해 세션의 `success_url`을 사용합니다. prop이 없으면 기존 흐름은 이전과 동일하게 작동합니다.
...
const poll = setInterval(async () => {
  ...
  if (s.data.status === "complete") {
    clearInterval(poll);
    // success_url로 리디렉션하거나 확인을 표시
  }
}, 3000);
```

### 피아트 온램프 (Coinbase/Stripe)

```typescript
const BASE_URL = "https://api.anyspend.com";

// 위와 동일한 1-2단계, 하지만 주문 생성 시 온램프 구성 포함:
const order = await fetch(`${BASE_URL}/orders`, {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({
...
// 사용자를 벤더 체크아웃 페이지로 리디렉션
window.location.href = order.data.oneClickBuyUrl;

// 벤더가 리디렉션한 후, 완료될 때까지 GET /checkout-sessions/:id를 폴링
