...
<ParamField path="GET /events" type="read">
  선택적 필터링을 사용하여 API 이벤트를 나열합니다. 결과는 역시간 순서(최신 순)로 반환됩니다.
</ParamField>
...
<ParamField path="from" type="string" query>
  ISO 8601 타임스탬프. 이 시간 이후에 생성된 이벤트만 반환.
</ParamField>

<ParamField path="to" type="string" query>
  ISO 8601 타임스탬프. 이 시간 이전에 생성된 이벤트만 반환.
</ParamField>
...
| `resource_type` | `string` | 영향을 받은 리소스 유형 |
...
<Note>
  이벤트는 90일 동안 보관됩니다. 더 긴 보존을 위해 날짜 범위 필터를 사용하여 주기적으로 이벤트를 내보내십시오.
</Note>
