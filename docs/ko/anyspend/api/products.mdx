...
---
제품은 카탈로그의 항목이나 서비스를 나타냅니다. 각 제품은 가격, 토큰, 체인 구성을 정의하여 여러 결제 링크 및 체크아웃 세션에서 재사용할 수 있습니다.
...
<ResponseField name="amount" type="string | null">
  토큰의 최소 단위로 가격. `variable` 제품 유형의 경우 Null.
</ResponseField>
...
<ResponseField name="form_schema" type="object | null">
  체크아웃 중 수집하는 사용자 정의 양식 필드. [Payment Links form_schema](/anyspend/api/payment-links#the-payment-link-object)와 동일한 구조.
</ResponseField>

<ResponseField name="shipping_options" type="array | null">
  실물 상품의 배송 옵션. [Payment Links shipping_options](/anyspend/api/payment-links#the-payment-link-object)와 동일한 구조.
</ResponseField>
      ...
      "data": [
        {
          ...
          "chain_id": 1,
          "description": "무제한 액세스를 제공하는 프로페셔널 등급",
...
<ParamField body="amount" type="string">
  토큰의 최소 단위로 가격. `one_time` 및 `subscription` 유형에 필요.
</ParamField>
...
<ParamField body="metadata" type="object">
  임의의 키-값 쌍 (예: `{"sku": "PRO-001"}`). 최대 50개 키, 500자 값.
</ParamField>

<ParamField body="form_schema" type="object">
  체크아웃 중 수집하는 사용자 정의 양식 필드.
</ParamField>
...
[제품 생성 엔드포인트](#create-a-product)에서 받는 모든 본문 매개변수가 허용됩니다. 추가로:

<ParamField body="active" type="boolean">
  제품을 비활성화하려면 `false`로 설정합니다.
</ParamField>

<Warning>
  제품을 업데이트해도 이로부터 생성된 결제 링크는 소급적으로 업데이트되지 않습니다. 각 결제 링크는 자체 구성을 유지합니다.
</Warning>
...
curl -X PATCH "https://platform-api.anyspend.com/api/v1/products/prod_abc123" \
  ...
  -d '{
    ...
    "amount": "500000000",
    "description": "연간 청구를 위한 프로페셔널 등급"
  }'
```
...
제품을 소프트 삭제합니다. 제품은 비활성으로 표시되며 기본적으로 목록 결과에 더 이상 나타나지 않습니다. 이 제품을 사용하는 기존 결제 링크는 계속 작동합니다.
...
제품의 설정으로 사전 구성된 결제 링크를 자동으로 생성합니다. 이는 제품의 `name`, `amount`, `token_address`, `chain_id`, `recipient_address`, `description`, `image_url`, `form_schema`, 및 `shipping_options`을 사용하여 새 결제 링크를 생성하는 편리한 방법입니다.
...
추가 [결제 링크 필드](/anyspend/api/payment-links#create-a-payment-link)는 재정의로 전달할 수 있습니다.
...
<Tabs>
  <Tab title="Response">
    출처 제품으로 `product_id`가 설정된 새 [결제 링크 객체](/anyspend/api/payment-links#the-payment-link-object)를 반환합니다.
