...
---
거래는 결제 링크와 체크아웃 세션을 통한 완료되었거나 진행 중인 결제를 나타냅니다. 거래 API는 읽기 전용입니다 -- 고객이 체크아웃 흐름을 통해 결제할 때 자동으로 거래가 생성됩니다.

<Info>
  거래 금액은 토큰의 가장 작은 단위(예: USDC의 경우 6자리 소수)로 저장됩니다. `amount_usd` 필드는 거래 시점의 USD 환산액을 제공합니다.
</Info>
...
<ResponseField name="source_amount" type="string | null">
  지불자가 보낸 금액(스왑/브리지 전), 원본 토큰의 가장 작은 단위로.
</ResponseField>

<ResponseField name="fee_amount" type="string | null">
  받은 토큰의 가장 작은 단위로 표시된 플랫폼 수수료 금액.
</ResponseField>
...
<ResponseField name="shipping_address" type="object | null">
  ...
  <Expandable title="배송 주소 필드">
    ...
    <ResponseField name="line1" type="string">주소 줄 1.</ResponseField>
    <ResponseField name="line2" type="string | null">주소 줄 2.</ResponseField>
...
선택적 필터를 사용하여 거래의 페이지별 목록을 검색합니다.

<ParamField query="status" type="string">
  상태별 필터: `pending`, `confirming`, `completed`, `failed`.
</ParamField>
...
<ParamField query="from" type="string">
  이 ISO 8601 날짜(예: `2026-01-01`) 이후에 생성된 거래 필터.
</ParamField>

<ParamField query="to" type="string">
  이 ISO 8601 날짜(예: `2026-02-28`) 이전에 생성된 거래 필터.
</ParamField>
...
<ParamField query="from" type="string">
  이 ISO 8601 날짜 이후에 생성된 거래 필터.
</ParamField>

<ParamField query="to" type="string">
  이 ISO 8601 날짜 이전에 생성된 거래 필터.
</ParamField>

<Info>
  내보내기는 50,000개의 기록으로 제한됩니다. 더 큰 데이터 세트의 경우, 날짜 범위 필터를 사용하여 내보내기를 더 작은 청크로 나눕니다.
</Info>
