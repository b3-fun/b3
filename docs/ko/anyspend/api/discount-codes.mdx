...
---
할인 코드를 사용하면 결제 링크에 대해 백분율 기반 또는 고정 금액 할인을 제공할 수 있습니다. 개별 코드를 생성하고, 대량으로 배치를 생성하고, 결제 시 코드를 검증할 수 있습니다.

<Info>
  할인 코드 관리는 **admin** 권한이 필요합니다. `/validate` 엔드포인트는 **read** 권한이 필요하며, 이는 클라이언트 측 체크아웃 흐름에서 안전하게 호출할 수 있음을 의미합니다.
</Info>
...
<ParamField path="value" type="number" required>
  할인 값. `percentage`의 경우 1-100 사이의 숫자. `fixed`의 경우 토큰의 최소 단위로 금액 (예: 5 USDC의 경우 `5000000`).
</ParamField>
...
<ParamField path="DELETE /discount-codes/:id" type="admin">
  할인 코드를 영구적으로 삭제합니다. 이 코드를 사용하는 활성 세션은 영향을 받지 않습니다.
</ParamField>
...
<ParamField path="POST /discount-codes/validate" type="read">
  결제 링크와 주문 금액에 대해 할인 코드를 검증합니다. 유효한 경우 할인 세부 정보를 반환하거나, 코드가 유효하지 않은 이유를 설명하는 오류 메시지를 반환합니다.
</ParamField>
...
<ParamField path="amount" type="string" required>
  `min_order_amount`을 확인하고 할인된 총액을 계산하는 데 사용되는 토큰의 최소 단위로 된 주문 금액.
</ParamField>
...
<Note>
  validate 엔드포인트는 사용을 소비하지 **않습니다**. 할인이 적용된 결제가 완료될 때만 사용이 계산됩니다.
</Note>
...
<ParamField path="POST /discount-codes/batch" type="admin">
  공유 구성으로 한 번에 여러 할인 코드를 대량 생성합니다. 프로모션 캠페인이나 고유한 일회용 코드를 생성하는 데 유용합니다.
</ParamField>
...
<ParamField path="max_uses" type="number">
  코드 당 최대 사용 횟수. 일회용 코드의 경우 `1`로 설정합니다.
</ParamField>
...
| `code` | `string` | 할인 코드 문자열 (대문자) |
| `type` | `string` | `percentage` 또는 `fixed` |
| `value` | `number` | 할인 값 (백분율 1-100, 또는 최소 단위로 고정 금액) |
| `payment_link_id` | `string \| null` | 제한된 결제 링크, 또는 `null`은 모든 링크 |
| `max_uses` | `number \| null` | 최대 사용 횟수, 또는 `null`은 무제한 |
| `current_uses` | `number` | 지금까지 사용된 횟수 |
| `min_order_amount` | `string \| null` | 필요한 최소 주문 금액 |
...
다음 중 하나라도 참이면 할인 코드는 **유효하지 않은 것**으로 간주됩니다:
