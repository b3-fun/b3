...
상태, 날짜 범위 및 형식에 대한 선택적 필터를 사용하여 거래 기록 내보내기.
...
| `status` | `string` | 아니요 | all | 상태별 필터: `completed`, `pending`, `failed` |
| `from` | `string` | 아니요 | -- | 시작 날짜 (ISO 8601, 예: `2025-01-01T00:00:00Z`) |
| `to` | `string` | 아니요 | -- | 종료 날짜 (ISO 8601) |

### CSV 출력

CSV 내보내기는 헤더 행 다음에 거래당 하나의 행을 포함합니다:
...
<CardGroup cols={2}>
  <Card title="정기적인 내보내기 일정 설정" icon="calendar">
    데이터를 백업하고 조정하기 위해 매일 또는 매주 데이터를 내보내도록 cron 작업이나 예약된 함수를 설정하세요.
  </Card>
  <Card title="대규모 데이터셋에 대한 날짜 필터 사용" icon="filter">
    수천 건의 거래가 있는 경우, 타임아웃을 피하기 위해 `from` 및 `to` 매개변수를 사용하여 더 작은 날짜 범위로 내보내세요.
  </Card>
  <Card title="스프레드시트용 CSV" icon="table">
    CSV 내보내기는 Excel, Google Sheets 및 기타 스프레드시트 도구에서 바로 열어 빠르게 분석할 수 있습니다.
  </Card>
  <Card title="프로그래밍 용도의 JSON" icon="code">
    JSON 내보내기는 데이터를 분석 파이프라인, 데이터베이스 또는 사용자 정의 대시보드로 전송하는 데 이상적입니다.
  </Card>
</CardGroup>

<Warning>
  내보내기는 속도 제한이 적용됩니다. 대규모 내보내기(10,000개 이상의 기록)는 생성하는 데 몇 초가 걸릴 수 있습니다. 요청은 내보내기가 완료될 때까지 차단되므로 매우 큰 데이터셋에 대해 적절히 계획하세요.
</Warning>
