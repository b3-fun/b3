...
---
AnySpend 플랫폼 SDK 클라이언트는 전체 AnySpend 플랫폼 API를 위한 **헤드리스**(React 의존성 없음) TypeScript 클라이언트입니다. 이는 Node.js, 브라우저, Cloudflare Workers, Deno, Bun에서 실행되는 현대적인 JavaScript 어디에서나 작동합니다.
...
const platform = new AnySpendPlatformClient(process.env.ANYSPEND_API_KEY!, {
  ...
  timeout: 30_000,           // 요청 타임아웃(ms) (기본값: 30초)
...
<Info>
  API 키는 [AnySpend 대시보드](https://app.anyspend.com)의 **설정 > API 키**에서 얻을 수 있습니다. 키는 조직에 귀속되며 읽기 전용 또는 읽기-쓰기 권한을 가질 수 있습니다.
</Info>

## 사용 가능한 리소스

클라이언트는 REST 엔드포인트에 직접 매핑되는 메소드를 가진 리소스 네임스페이스로 구성됩니다.
...
모든 `list` 메소드는 선택적 페이지네이션 매개변수를 받아들이며 페이지네이션된 응답을 반환합니다:
...
편의를 위해 모든 `list` 메소드에는 항목을 소비함에 따라 내부적으로 페이지를 가져오는 비동기 반복자를 반환하는 `listAutoPaginate` 도우미가 있습니다.
...
<Tip>
  자동 페이지네이션은 자동으로 속도 제한을 존중합니다. API가 `429`를 반환하면 반복자는 `Retry-After` 기간을 기다린 후 다음 페이지를 가져옵니다.
</Tip>
...
try {
  ...
  if (err instanceof NotFoundError) {
    ...
    console.log(`속도 제한됨. ${err.retryAfter}초 후에 다시 시도하세요`);
  } else if (err instanceof IdempotencyError) {
    console.log("Idempotency 충돌 -- 요청이 이미 다른 매개변수로 처리되었습니다");
  } else if (err instanceof ApiError) {
    // 모든 API 오류에 대한 포괄적인 캐치
...
| `ApiError` | 어떤 | 모든 API 오류에 대한 기본 클래스 |
...
- **429 속도 제한 응답** -- `Retry-After` 기간을 기다립니다

재시도는 **지수 백오프**와 지터를 사용합니다. 기본값은 3번의 재시도이며, `maxRetries` 옵션을 통해 설정할 수 있습니다.
...
<Warning>
  `429`를 제외한 `4xx` 오류에 대해서는 재시도가 **실행되지 않습니다**. 이는 클라이언트 측 문제를 나타내며 재시도해도 해결되지 않기 때문입니다.
</Warning>

## 멱등성

모든 `POST` 및 `PATCH` 요청은 자동으로 `Idempotency-Key` 헤더를 포함합니다. 이는 요청이 재시도되는 경우(예: 네트워크 오류로 인해) 서버가 이를 두 번 처리하지 않도록 보장합니다.

기본 생성기는 `crypto.randomUUID()`를 사용합니다. 자신만의 것을 제공할 수 있습니다:
...
<Tip>
  중요한 작업에 대해서는 주문 ID와 같은 결정적인 idempotency 키를 사용하는 것이 좋습니다. 같은 요청을 실수로 두 번 보내면 두 번째 호출이 원본 응답을 반환하고 중복을 생성하지 않습니다.
</Tip>
