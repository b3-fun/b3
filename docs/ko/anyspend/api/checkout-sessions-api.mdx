...
---
체크아웃 세션은 개별 결제 시도를 나타냅니다. 고객별로 고유한 체크아웃 URL을 생성하고, 결제 정보를 미리 채우며, 자체 참조 ID로 전환을 추적하기 위해 서버 측에서 세션을 생성할 수 있습니다.
...
<ResponseField name="payment_link_id" type="string | null">
  세션이 결제 링크에서 생성된 경우 연관된 결제 링크 ID.
</ResponseField>
...
<ResponseField name="customer_email" type="string | null">
  고객의 이메일 주소. 휴식 상태에서 암호화되며, 웹훅 페이로드에 포함됩니다.
</ResponseField>

<ResponseField name="customer_name" type="string | null">
  고객의 이름. 휴식 상태에서 암호화되며, 웹훅 페이로드에 포함됩니다.
</ResponseField>
...
| `failed` | 체인 오류나 타임아웃으로 인해 결제가 실패했습니다. |
...
고유한 결제 URL이 있는 서버 측 체크아웃 세션을 생성합니다. 백엔드에서 체크아웃 흐름을 제어하는 프로그래매틱 통합에 이를 사용하세요.
...
<ParamField body="chain_id" type="number">
  결제를 받고자 하는 체인 ID. `payment_link_id`가 제공되지 않은 경우 필요합니다.
</ParamField>
...
<ParamField body="customer_email" type="string">
  고객 이메일 주소. 휴식 상태에서 암호화되며 주문 이행을 위한 웹훅 페이로드에 포함됩니다.
</ParamField>

<ParamField body="customer_name" type="string">
  고객 이름. 휴식 상태에서 암호화되며 주문 이행을 위한 웹훅 페이로드에 포함됩니다.
</ParamField>

<ParamField body="expires_in" type="number" default={1800}>
  세션 만료 시간(초 단위). 최소 300(5분), 최대 86400(24시간). 기본값: 1800(30분).
</ParamField>
