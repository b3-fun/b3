...
---
네트워크 실패, 타임아웃, 그리고 재시도는 모든 API를 사용할 때 발생하는 현실입니다. 멱등성 키는 요청을 다시 시도해도 중복 리소스를 생성하거나 동일한 작업을 두 번 수행하지 않도록 보장합니다.
...
<Steps>
<Step title="첫 번째 요청">
API는 요청을 정상적으로 처리하고, 리소스를 생성한 다음 응답을 캐시합니다. 응답은 멱등성 키와 요청 본문의 SHA-256 해시와 연결됩니다.
</Step>
...
API는 중복 키를 감지하지만 본문 해시가 일치하지 않습니다. `409 Conflict` 오류와 함께 `idempotency_conflict` 코드를 반환합니다. 이는 키의 잘못된 사용을 방지합니다.
</Step>
</Steps>
...
<Info>
  성공적인 응답(HTTP 2xx)만 캐시됩니다. 원래 요청이 4xx 또는 5xx 오류로 실패한 경우, 키는 사용되지 않으며 동일한 키로 다시 시도할 수 있습니다.
</Info>
...
정확히 같은 요청을 다시 시도하면, 재생 헤더와 함께 캐시된 응답을 받습니다:
...
이는 안전 장치입니다. 동일한 키가 실수로 두 가지 다른 작업과 연결되는 버그를 방지합니다.
...
| `DELETE` | 필요 없음 | DELETE 요청은 본질적으로 멱등성이 있습니다(존재하지 않는 리소스를 삭제하려고 하면 404가 반환됩니다). |

## 멱등성 키 생성하기

멱등성 키는 최대 256자의 문자열이 될 수 있습니다. 여기 몇 가지 권장 방법이 있습니다:
...
요청을 해싱하면 동일한 요청이 항상 동일한 키를 생성하도록 합니다. 동일한 메시지가 여러 번 처리될 수 있는 큐 기반 시스템에 유용합니다.
</Tab>
</Tabs>

## 멱등성 키 범위

멱등성 키는 **조직**에 범위가 지정됩니다. 두 개의 다른 조직은 충돌 없이 동일한 키 문자열을 사용할 수 있습니다. 조직 내에서는 각 키를 24시간 창에서 한 번만 사용할 수 있습니다.

## 모범 사례

<AccordionGroup>
<Accordion title="결제 관련 작업에는 항상 멱등성 키를 사용하세요">
결제 링크를 생성하거나, 체크아웃 세션을 시작하거나, 재정적 작업을 트리거하는 모든 요청에는 멱등성 키를 포함해야 합니다. 이것이 중복을 방지하는 가장 중요한 장소입니다.
</Accordion>

<Accordion title="첫 번째 시도 전에 키를 생성하세요">
멱등성 키를 한 번 생성하고 동일한 논리적 작업에 대한 모든 재시도에서 재사용하세요:
...
<Accordion title="다른 작업에 키를 재사용하지 마세요">
각 논리적 작업은 자체 멱등성 키를 가져야 합니다. 이전 작업(다른 작업)에서 사용된 키를 재사용하면 이전 작업의 캐시된 응답이나 `409` 충돌이 발생할 수 있습니다.
</Accordion>
...
`Idempotent-Replayed: true` 헤더가 있으면 응답이 캐시에서 재생된 것입니다. 이는 로깅과 디버깅에 유용할 수 있어, 새로운 응답과 재생된 응답을 구분할 수 있습니다.
</Accordion>

<Accordion title="정확히 한 번의 의미를 위해 결정적 키를 사용하세요">
시스템이 큐에서 이벤트를 처리하는 경우(예: Kafka, SQS, BullMQ), 이벤트 ID 또는 메시지 ID에서 멱등성 키를 파생시킵니다. 이는 동일한 이벤트를 두 번 처리해도 중복 리소스가 생성되지 않도록 보장합니다:
