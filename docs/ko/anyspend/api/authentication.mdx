...
---
AnySpend 플랫폼 API는 인증을 위해 API 키를 사용합니다. 각 키는 단일 조직에 할당되며 특정 권한 세트를 가지고 있습니다.
...
<Step title="새 키 생성">
  **API 키 생성**을 클릭하고, 설명이 포함된 이름(예: "프로덕션 백엔드" 또는 "CI/CD 파이프라인")을 지정하고 권한 수준을 선택하세요.
</Step>
...
<Warning>
  API 키는 생성될 때 단 한 번만 표시됩니다. 분실하면 이전 키를 취소하고 새로운 키를 생성해야 합니다.
</Warning>
...
접두사는 코드베이스에서 AnySpend 키를 쉽게 식별하게 하며 다른 서비스의 키를 실수로 사용하는 것을 방지합니다.
...
`Authorization: Bearer` 방식이 OAuth 2.0 규약을 따르며 대부분의 HTTP 클라이언트 및 API 테스팅 도구에서 지원되므로 선호됩니다.

## 권한 수준

각 API 키는 하나 이상의 권한이 할당됩니다. 권한은 상위 수준이 모든 하위 수준 기능을 포함하는 계층 구조를 따릅니다.
...
| **admin** | `write`에 있는 모든 것 + API 키, 조직 설정 및 결제 관리 | 인프라 관리, CI/CD, 관리 도구 |
...
`admin` 권한이 있는 키를 가지고 있다면, 프로그래밍 방식으로 새 키를 생성할 수 있습니다:
...
## Quick Pay -- 오픈 티어(인증 없음)

Quick Pay 엔드포인트는 인증을 요구하지 않습니다. AnySpend 계정이 필요 없는 일회성 결제에 적합합니다:
...
Quick Pay는 **IP 주소당 분당 5회 요청**으로 제한됩니다. API 키가 필요하지 않습니다.

## 키 회전 모범 사례

주기적으로 API 키를 회전시키면 자격 증명이 손상될 위험을 줄일 수 있습니다. 여기에 권장되는 접근 방식이 있습니다:
...
API 키는 서버 측 코드(백엔드 서비스, 서버리스 함수, CI/CD 파이프라인)에서만 사용해야 합니다. 다음에는 포함하지 마세요:
...
실수로 커밋되지 않도록 `.env`를 `.gitignore`에 추가하세요.
</Accordion>
...
정기적으로 키의 `last_used_at`을 확인하여 취소해야 하는 사용하지 않는 키를 식별하세요. 사용하지 않는 활성 키는 보안 위험입니다.
</Accordion>

<Accordion title="만료 날짜 설정">
임시 통합, CI/CD 토큰 또는 계약자 접근에 항상 `expires_at` 타임스탬프를 설정하여 키가 자동으로 비활성화되도록 하세요.
</Accordion>
</AccordionGroup>
