...
---
AnySpend 플랫폼 API는 **AI-first 통합**을 염두에 두고 설계되었습니다. 모든 엔드포인트는 고유한 `operationId`, 상세한 JSON 스키마, 그리고 LLM 함수 호출, 자율 에이전트 및 AI 오케스트레이션 프레임워크와 함께 사용하기 쉽게 해주는 일관된 규칙을 가지고 있습니다.
...
<Tip>
  OpenAPI 사양을 사용하여 LLM을 위한 도구 정의를 자동 생성하세요. 대부분의 AI 프레임워크는 OpenAPI 작업을 프로그래밍 방식으로 함수/도구 정의로 변환할 수 있습니다.
</Tip>
...
```json
{
  ...
  "description": "고객과 공유할 수 있는 새로운 결제 링크를 생성합니다. 공유 가능한 URL을 포함한 결제 링크 객체를 반환합니다.",
  "input_schema": {
    "type": "object",
    "properties": {
      "name": {
      ...
      "amount": {
        "type": "string",
        "description": "토큰의 가장 작은 단위로 지불 금액. USDC (6자리 소수점)의 경우, '1000000' = 1 USDC."
      },
      "recipient_address": {
        "type": "string",
        "description": "지불을 받을 이더리움 주소 (0x 접두사, 42자)"
      },
      "token_address": {
        "type": "string",
        "description": "ERC-20 토큰 계약 주소. 생략하면 기본값으로 Base의 USDC."
      },
      "chain_id": {
        "type": "number",
      ...
      "description": {
        "type": "string",
        "description": "결제 페이지에서 지불자에게 보여줄 선택적 설명"
      }
    },
  ...
  "function": {
    ...
    "parameters": {
      ...
      "properties": {
        ...
        "amount": {
          "type": "string",
          "description": "가장 작은 단위로 지불 금액 (예: '1000000'은 1 USDC)"
        },
        "recipient_address": {
          "type": "string",
          "description": "지불을 받을 이더리움 주소"
        },
        "token_address": {
          "type": "string",
...
AI 어시스턴트가 AnySpend 도구를 대화에서 어떻게 사용할 수 있는지에 대한 예시입니다:

<Steps>
  <Step title="사용자가 결제 링크 생성을 요청">
    **사용자:** "Pro 계획 구독에 대해 50 USDC 결제 링크를 생성해주세요"
  ...
  <Step title="AI가 사용자에게 응답">
    "Pro 계획 구독에 대해 50 USDC 결제 링크를 생성했습니다. 여기 링크입니다: https://anyspend.com/pay/pl_abc123"
  </Step>
</Steps>
...
const tools: Anthropic.Tool[] = [
  {
    ...
    input_schema: {
      ...
      properties: {
        name: { type: "string", description: "링크의 이름" },
        amount: { type: "string", description: "가장 작은 단위의 금액" },
  ...
  },
  {
    ...
    description: "최근 거래 목록, 선택적 상태 필터 포함",
    input_schema: {
      type: "object" as const,
      properties: {
...
const SYSTEM_PROMPT =
  "당신은 결제 관리 어시스턴트입니다. 제공된 도구를 사용하여 사용자가 결제 링크를 생성하고, 거래를 보고, AnySpend 계정을 관리하도록 도와주세요. 금액은 토큰의 가장 작은 단위입니다 -- USDC의 경우 6자리 소수점으로, 1 USDC = 1000000.";
...
<AccordionGroup>
  <Accordion title="list_payment_links">
    ```json
    {
      ...
      "description": "모든 결제 링크를 나열하며, 선택적으로 상태별로 필터링",
      "input_schema": {
        "type": "object",
        "properties": {
          "status": {
  ...
  <Accordion title="get_analytics_overview">
    ```json
    {
      ...
      "description": "총 거래량, 거래 수, 상위 결제 링크를 포함한 고급 분석 개요를 가져옵니다",
      "input_schema": {
        "type": "object",
        "properties": {
          "from": {
    ...
    ```json
    {
      ...
      "input_schema": {
        ...
        "properties": {
          ...
          "value": {
            "type": "string",
            "description": "할인 값: 백분율 (예: '20'은 20%) 또는 가장 작은 단위의 고정 금액"
          },
          "max_uses": {
            "type": "number",
  ...
  <Accordion title="export_transactions">
    ```json
    {
      ...
      "description": "보고 및 분석을 위해 거래를 CSV 또는 JSON으로 내보냅니다",
      "input_schema": {
        "type": "object",
        "properties": {
          "format": {
...
<CardGroup cols={2}>
  ...
  <Card title="작업 에이전트에 대해 쓰기 키 사용" icon="pen">
    결제 링크를 생성하거나 리소스를 관리해야 하는 에이전트는 **읽기-쓰기
