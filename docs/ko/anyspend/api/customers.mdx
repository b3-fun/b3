...
---
고객은 결제 링크를 통해 결제하는 사람들을 나타냅니다. 고객 기록은 지갑이 체크아웃 세션을 완료할 때 자동으로 생성되지만, API를 통해 수동으로도 생성하고 관리할 수 있습니다.

<Info>
  고객의 `name` 및 `email` 필드는 개인 정보 보호 준수를 위해 **휴지 상태에서 암호화**됩니다. API를 통해 검색할 때 자동으로 복호화됩니다.
</Info>
...
<ResponseField name="name" type="string | null">
  고객 이름. 휴지 상태에서 암호화됩니다.
</ResponseField>

<ResponseField name="email" type="string | null">
  고객 이메일. 휴지 상태에서 암호화됩니다.
</ResponseField>
...
수동으로 고객 기록을 생성합니다. 결제를 하기 전에 고객을 사전 등록하는 데 유용합니다.
...
<ParamField body="metadata" type="object">
  임의의 키-값 쌍 (예: `{"plan": "pro"}`). 최대 50개의 키, 값당 500자.
</ParamField>
...
<Tabs>
  <Tab title="Response">
    전체 [고객 객체](#the-customer-object)를 반환합니다.
  </Tab>
</Tabs>
...
<ParamField body="metadata" type="object">
  업데이트된 메타데이터. 이는 전체 메타데이터 객체를 **대체**합니다. 단일 키를 추가하려면 기존 메타데이터 전체와 새 키를 포함하세요.
</ParamField>

<Tip>
  `wallet_address`는 생성 후 변경할 수 없습니다. 다른 지갑과 고객을 연결하려면 새 고객 기록을 생성하세요.
</Tip>
...
고객 기록을 영구적으로 삭제합니다. 이 고객과 관련된 거래 기록은 보존되지만, 고객 참조는 삭제된 것으로 표시됩니다.
...
<Info>
  내보내기는 10,000개의 기록으로 제한됩니다. 더 큰 데이터셋의 경우, 목록 엔드포인트와 페이지네이션을 사용하세요.
</Info>
