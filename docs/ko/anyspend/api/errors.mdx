...
---
AnySpend 플랫폼 API는 모든 엔드포인트에서 일관된 구조화된 오류 형식을 사용합니다. 오류는 Stripe 규칙을 따릅니다 -- `error` 키가 포함된 JSON 객체로, 타입, 코드, 사람이 읽을 수 있는 메시지 및 (해당되는 경우) 오류를 일으킨 매개변수를 포함합니다.
...
| 필드 | 타입 | 설명 |
|-------|------|-------------|
| `type` | `string` | 오류의 카테고리입니다. 광범위한 오류 처리 로직에 사용하세요. |
...
| `param` | `string` 또는 없음 | 해당되는 경우 오류를 일으킨 요청 매개변수입니다. |

## 오류 유형

오류 유형은 오류를 광범위한 카테고리로 그룹화합니다. 최상위 오류 처리에 이를 사용하세요.

| 유형 | HTTP 상태 | 설명 |
|------|-------------|-------------|
| `invalid_request_error` | `400` | 요청이 잘못되었거나 유효하지 않은 매개변수를 포함하고 있습니다. 문제를 일으킨 필드는 `param` 필드를 확인하세요. |
| `authentication_error` | `401` | API 키가 누락되었거나 유효하지 않거나 만료되었거나 취소되었습니다. |
| `permission_error` | `403` | API 키는 유효하지만 요청된 작업에 필요한 권한 수준이 부족합니다. |
| `not_found_error` | `404` | 요청한 리소스가 존재하지 않거나 귀하의 조직에 속하지 않습니다. |
| `rate_limit_error` | `429` | 인증 계층에 대한 비율 제한을 초과했습니다. |
| `idempotency_error` | `409` | 다른 요청 본문으로 재사용된 멱등성 키입니다. |
| `api_error` | `500` | 예기치 않은 내부 오류가 발생했습니다. 이러한 오류는 드물며 자동으로 보고됩니다. |
...
| `no_updates_provided` | `invalid_request_error` | `400` | 업데이트할 유효한 필드가 없는 `PATCH` 요청이 전송되었습니다. |
| `invalid_status_transition` | `invalid_request_error` | `400` | 요청된 상태 변경이 허용되지 않습니다(예: 이미 만료된 세션을 완료하려고 함). |
...
| 코드 | 유형 | HTTP | 설명 |
|------|------|------|-------------|
| `resource_not_found` | `not_found_error` | `404` | 요청한 리소스가 존재하지 않거나 다른 조직에 속합니다. |

### 비율 제한 및 멱등성 오류

| 코드 | 유형 | HTTP | 설명 |
|------|------|------|-------------|
| `rate_limit_exceeded` | `rate_limit_error` | `429` | 현재 시간 창에서 너무 많은 요청을 했습니다. 오류 메시지에는 재시도 지연이 포함됩니다. |
...
| 코드 | 유형 | HTTP | 설명 |
|------|------|------|-------------|
| `internal_error` | `api_error` | `500` | 예기치 않은 서버 오류입니다. 이러한 오류는 자동으로 추적됩니다. 지속되면 지원팀에 문의하세요. |
...
```json
{
  "error": {
    ...
    "message": "'token_address' 필드는 유효한 이더리움 주소여야 합니다 (0x + 40 16진수 문자).",
    "param": "token_address"
  }
}
```
  ...
  if (!response.ok) {
    ...
    switch (error.type) {
      case "invalid_request_error":
        // 요청 수정 -- 나쁜 필드에 대해 error.param 확인
        console.error(`잘못된 요청: ${error.message} (field: ${error.param})`);
        break;

      case "authentication_error":
        // API 키 확인 -- 만료되었거나 취소될 수 있음
...
def create_payment_link(data: dict) -> dict:
    ...
    if not response.ok:
        ...
        if error_type == "invalid_request_error":
            raise ValueError(
                f"필드 '{error.get('param', 'unknown')}'에 대한 잘못된 요청: "
                f"{error['message']}"
            )
...
`error.type`을 사용하여 광범위한 제어 흐름을 관리하세요(예: `rate_limit_error`에 대해 재시도, `authentication_error`에 대해 빠르게 실패). `error.code`을 사용하여 유형 내에서 구체적인 처리를 수행하세요.
</Accordion>

<Accordion title="폼 유효성 검사를 구축하기 위해 param 필드 사용">
`error.param`이 존재할 때, 이를 직접 폼 필드에 매핑하여 UI에서 인라인 유효성 검사 오류를 표시할 수 있습니다.
</Accordion>

<Accordion title="일시적인 오류에 대한 재시도 로직 구현">
...
<Accordion title="4xx 오류는 재시도하지 마세요(429 제외)">
`400`, `401`, `403`, `404`와 같은 클라이언트 오류는 재시도해도 해결되지 않습니다. 재시도하기 전에 요청이나 자격 증명을 수정하세요.
</Accordion>

<Accordion title="디버깅을 위해 전체
