...
---
Webhooks를 사용하면 AnySpend 조직에서 이벤트가 발생할 때 실시간 HTTP 콜백을 서버에서 받을 수 있습니다 -- 결제 완료, 체크아웃 만료 등. API를 폴링하는 대신 URL을 등록하면 AnySpend가 이벤트를 푸시합니다.
...
<Steps>
  <Step title="웹훅 엔드포인트 등록">
    대시보드 또는 API를 통해 웹훅을 생성하고, URL과 구독할 이벤트를 지정합니다.
  </Step>
  <Step title="AnySpend가 이벤트 전송">
    구독한 이벤트가 발생하면 AnySpend가 JSON 페이로드와 서명 헤더가 포함된 `POST` 요청을 URL로 전송합니다.
  </Step>
  <Step title="검증 및 처리">
    서버가 HMAC-SHA256 서명을 검증하고, 이벤트를 처리한 후 30초 이내에 `200` 상태로 응답합니다.
  </Step>
  <Step title="자동 재시도">
    서버가 `2xx` 상태로 응답하지 않으면 AnySpend가 지수 백오프로 최대 3회까지 재시도합니다.
  </Step>
</Steps>
...
| `checkout.expired` | 지불자가 완료하기 전에 체크아웃 세션 만료 |
...
<Tabs>
  <Tab title="SDK">
    ...
    // 중요: 이 비밀을 안전하게 저장하세요 -- 한 번만 표시됩니다
  ...
  <Tab title="대시보드">
    [AnySpend 대시보드](https://app.anyspend.com)에서 **설정 > 웹훅 > 엔드포인트 추가**로 이동합니다. URL을 입력하고, 이벤트를 선택한 다음 **생성**을 클릭합니다. 서명 비밀이 한 번 표시되므로 즉시 복사합니다.
  </Tab>
</Tabs>

## 웹훅 페이로드

모든 웹훅 전송은 다음 JSON 본문을 포함한 `POST` 요청을 전송합니다:
...
| `X-AnySpend-Signature` | 원시 요청 본문의 HMAC-SHA256 16진수 다이제스트 |
| `X-AnySpend-Webhook-Id` | 웹훅 엔드포인트 ID |
| `X-AnySpend-Delivery-Id` | 고유 배송 시도 ID |
| `X-AnySpend-Event` | 이벤트 유형 (예: `payment.completed`) |
| `X-AnySpend-Timestamp` | ISO 8601 타임스탬프 |

## 시스템과의 결제 조정

`payment.completed` 웹훅은 항상 `checkoutSession` 블록을 포함하며, `clientReferenceId`, `metadata`, `customerEmail`, `customerName`을 포함합니다 -- 내부 기록과 결제를 일치시키는 데 필요한 모든 것입니다.

### client_reference_id 사용

체크아웃을 생성할 때 주문 또는 사용자 ID를 URL 매개변수 또는 API를 통해 전달합니다:
...
더 풍부한 데이터를 위해 메타데이터 키-값 쌍을 사용합니다:
...
<Tip>
  `client_reference_id`와 `metadata`는 항상 웹훅 페이로드에 포함됩니다. 간단한 통합을 위해 [URL 매개변수](/anyspend/api/payment-links#url-parameters)를 통해 설정하거나 서버 측 제어를 위해 [Checkout Sessions API](/anyspend/api/checkout-sessions-api)를 사용할 수 있습니다.
</Tip>
...
<Warning>
  웹훅을 처리하기 전에 항상 `X-AnySpend-Signature` 헤더를 검증하세요. 검증 없이는 공격자가 위조된 이벤트를 엔드포인트로 보낼 수 있습니다.
</Warning>
...
app.post(
  ...
  (req, res) => {
    ...
    if (
      ...
      console.error("잘못된 웹훅 서명");
      return res.status(401).send("잘못된 서명");
    }

    // 서명이 유효함 -- 이벤트를 파싱하고 처리
...
function handlePaymentCompleted(data) {
  ...
  // 주문 이행, 확인 이메일 전송, 데이터베이스 업데이트 등
}

function handlePaymentFailed(data) {
  console.log(`결제 ${data.id} 실패`);
  // 고객에게 알림, 주문 상태 업데이트 등
}
...
3회 실패한 재시도 후에는 배송이 영구적으로 `실패`로 표시됩니다. 대시보드 또는 API에서 수동으로 재시도할 수 있습니다.

<Info>
  엔드포인트가 지속적으로 실패하는 경우(10회 이상 연속 실패), 웹훅은 자동으로 **비활성화**되며 이메일 알림을 받게 됩니다. 문제를 해결한 후 대시보드에서 다시 활성화하세요.
</Info>
...
웹훅 URL로 합성 이벤트를 보내는 테스트 엔드포인트를 사용합니다. 이를 통해 엔드포인트가 도달 가능하고 서명 검증 로직이 정확한지 확인할 수 있습니다.
...
테스트 이벤트는 `payment.completed` 이벤트 유형과 모의 데이터를 사용합니다. 다른 이벤트처럼 처리해야 하지만, 테스트 중 부작용을 건너뛰고 싶다면 `X-AnySpend-Test: true` 헤더를 확인할 수 있습니다.

## 모범 사례

<CardGroup cols={2}>
  <Card title="빠르게 응답" icon="bolt">
    `200`을 즉시 반환하고 이벤트를 비동기적으로 처리하세요 (예: 백그라운드 작업 큐에서). 웹훅 배송은 30초 후에 타임아웃됩니다.
  </Card>
  <Card title="중복 처리" icon="clone">
    `X-AnySpend-Delivery-Id` 헤더 또는 `data.id` 필드를 사용하여 이벤트를 중복 제거하세요. 재시도는 동일한 이벤트를 여러 번 전달할 수 있습니다.
  </Card>
  <Card title="서명 검증" icon="shield-halved">
    항상 `X-AnySpend-Signature` 헤더를 검증하세요. 검증 없이는 페이로드를 신뢰하지 마세요.
  </Card>
  <Card title="HTTPS 사용" icon="lock">
    웹훅 URL은 프로덕션에서 HTTPS를 사용해야 합니다. 개발
