...
---
모든 목록 엔드포인트에서 AnySpend 플랫폼 API는 페이지네이션된 응답을 반환합니다. 이 페이지에서는 대규모 결과 세트를 탐색하고, 데이터를 필터링하며, 결과를 정렬하는 방법을 다룹니다.
...
| `limit` | `integer` | `20` | `100` | 페이지당 결과 수입니다. |
...
`has_more` 필드를 사용하여 페이지네이션을 멈출 시점을 결정합니다:
...
| 파라미터 | 타입 | 설명 |
|-----------|------|-------------|
| `status` | `"active"`, `"completed"`, `"expired"` | 상태별로 세션 필터링합니다. |
...
정렬을 지원하는 목록 엔드포인트는 `sort` 및 `order` 파라미터를 받습니다.
...
편의를 위해 페이지네이션을 자동으로 처리하는 비동기 반복자를 만들 수 있습니다:
...
모든 페이지를 가져오지 않고 일치하는 리소스의 총 수를 얻으려면 어떤 페이지에서든 `total_count`를 사용합니다:
...
`limit=1`을 설정하면 카운트만 필요할 때 응답 페이로드를 최소화합니다.

## 모범 사례

<AccordionGroup>
<Accordion title="대량 작업에는 limit=100 사용">
데이터 내보내기나 동기화 등 모든 레코드를 가져와야 할 때는 API 호출 수를 최소화하고 속도 제한 내에 있도록 최대 `limit=100`을 사용합니다.
</Accordion>

<Accordion title="페이지네이션 루프에는 has_more 대신 total_count 사용">
`has_more`는 다음 페이지를 가져올지 여부를 결정하는 공식 신호입니다. 요청 사이에 총계가 변경될 수 있으므로 `total_count`에서 페이지 수를 계산하는 것을 피하세요.
</Accordion>

<Accordion title="UI 페이지네이션 컨트롤에 total_count 캐싱">
예를 들어 "8페이지 중 3페이지"와 같은 페이지네이션 UI를 구축할 때 `total_count`를 한 번 가져와 세션에 캐시합니다. 사용자가 새로 고침하거나 새 필터를 적용할 때만 다시 가져옵니다.
</Accordion>

<Accordion title="페이지네이션과 필터 결합">
모든 레코드를 가져와 클라이언트 측에서 필터링하는 대신, 필터를 적용한 후 페이지네이션하여 결과 세트를 줄입니다. 이 방법이 더 효율적입니다:

```bash
# 모든 링크를 가져와 로컬에서 필터링하는 대신...
# 서버 측에서 필터링하고 필터링된 결과를 페이지네이션합니다
curl "https://platform-api.anyspend.com/api/v1/payment-links?active=true&search=premium&limit=20" \
  -H "Authorization: Bearer asp_live_abc123..."
```
</Accordion>

<Accordion title="대량 가져오기 중 속도 제한 존중">
큰 데이터 세트를 페이지네이션하는 경우, 분당 120회 요청의 속도 제한을 유의하세요. 매우 큰 데이터 세트의 경우, 페이지 요청 사이에 짧은 지연을 추가하세요:
...
async function fetchWithDelay() {
  ...
  // 10,000개 레코드의 경우, 이는 100회 요청입니다 -- 분당 120회 제한 내에 있습니다.
}
```
</Accordion>
</AccordionGroup>
