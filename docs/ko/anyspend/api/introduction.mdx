...
---
AnySpend 플랫폼 API는 결제 인프라에 대한 프로그래밍 방식의 전체 제어를 제공합니다. 통합을 구축하고, 워크플로우를 자동화하며, AI 에이전트가 커머스 스택을 관리하게 하세요 -- 모두 깔끔한 Stripe 스타일의 REST API를 통해 가능합니다.

## 이것은 누구를 위한 것인가?

- AnySpend과 사용자 정의 통합을 구축하는 **개발자**
- 결제 링크를 생성하고, 제품을 관리하거나 분석을 끌어오는 **AI 에이전트**
- 커머스 워크플로우를 조율하는 **자동화 플랫폼** (Zapier, n8n, 사용자 정의 스크립트)
...
AnySpend 결제를 사용자에게 다시 연결하는 두 가지 패턴이 있습니다:
...
전체 고객 컨텍스트와 함께 백엔드에서 체크아웃 세션을 생성하세요:
...
<Tip>
  웹훅 핸들러에서 결제를 시스템과 연결하는 전체 예제를 보려면 [웹훅 가이드](/anyspend/api/webhooks-guide#reconciling-payments-with-your-system)를 참조하세요.
</Tip>
...
| 등급 | 필요한 인증 | 제한 속도 | 사용 사례 |
|------|--------------|------------|----------|
| **Open** | 없음 (IP 기반 제한 속도) | IP당 5요청/분 | Quick Pay -- 계정이 필요 없는 일회성 결제 |
| **API Key** | 헤더를 통한 `asp_xxx` 키 | 키당 120요청/분 | 모든 표준 API 작업 -- 결제 링크, 제품, 거래 등의 CRUD |
| **JWT** | 대시보드 세션 토큰 | 표준 세션 제한 | 대시보드 전용 경로 (웹 앱에 의해 자동으로 관리됨) |
...
목록 응답을 탐색하는 방법에 대한 자세한 내용은 [페이지네이션 & 필터링](/anyspend/api/pagination) 페이지를 참조하세요.
...
제한 속도는 인증 등급에 따라 다릅니다. 제한을 초과하면 API는 `429` 상태와 `rate_limit_error`를 반환합니다:
...
`POST` 및 `PATCH` 요청에 대한 안전한 재시도를 위해 `Idempotency-Key` 헤더를 포함하세요. 동일한 키와 본문으로 중복 요청은 캐시된 응답을 반환합니다. 자세한 내용은 [멱등성](/anyspend/api/idempotency) 페이지를 참조하세요.
...
<Card title="제품" icon="box" href="/anyspend/api/products">
  제품 카탈로그를 관리하세요 -- 결제 링크에 첨부할 수 있는 제품을 생성하세요.
</Card>
...
<Card title="거래" icon="arrow-right-arrow-left" href="/anyspend/api/transactions">
  거래 기록을 조회하고, 상태별로 필터링하며, 정산 세부 정보를 끌어오세요.
</Card>

<Card title="고객" icon="users" href="/anyspend/api/customers">
  고객 지갑을 추적하고, 구매 기록을 보고, 고객 기록을 관리하세요.
</Card>
...
<Step title="첫 번째 요청하기">
  위의 curl 예제 또는 선호하는 HTTP 클라이언트를 사용하여 결제 링크를 생성하세요.
</Step>
