...
HypeDuel operates on a sophisticated three-token economic system designed to optimize user experience, create sustainable value flows, and enable community ownership. Each token serves a specific purpose while working together to power the entire ecosystem.
...
graph TD
    subgraph "Token Hierarchy"
        A[B3 Token<br/>Foundation Currency] --> B[HYPES Credits<br/>Gaming Currency<br/>1:1 peg]
        A --> C[ArenaCoins<br/>Arena Ownership<br/>Bonding Curves]
    end

    subgraph "Core Functions"
        B --> D[Instant Betting<br/>Zero Gas Fees]
        B --> E[Boost Purchases<br/>Real-time Action]
        C --> F[Fee Sharing<br/>Revenue Rights]
        C --> G[Community<br/>Governance]
        A --> H[Cross-Platform<br/>Utility]
        A --> I[Treasury<br/>Operations]
    end

    subgraph "Value Creation"
        D --> J[Platform Growth]
        E --> J
        F --> K[Token Appreciation]
        G --> L[Community Building]
        H --> M[Ecosystem Expansion]
        I --> N[Sustainability]
    end
