...
export const formatDisplayNumber = (
  ...
  // Ensure significantDigits is within valid range (1-21) for Intl.NumberFormat
  let significantDigits = style === "decimal" ? options?.significantDigits || 6 : options?.significantDigits;
  if (significantDigits && (significantDigits < 1 || significantDigits > 21)) {
    significantDigits = Math.min(Math.max(significantDigits, 1), 21);
  }
  ...
  if (v < 1) {
    ...
    if (showSubscripts && numberOfLeadingZeros > 3) {
