...
export class BondkitTokenFactory {
  ...
  public async deployBondkitToken(
    ...
    if (!this.walletClientInstance.account && !this.walletKey) {
      throw new Error(
        "Wallet key not set or client not connected with an account."
      );
    }
    ...
    try {
      ...
      for (const log of receipt.logs) {
        try {
          if (
            log.topics[0] !==
            "0x75b2d0aabe689b83d7eb7920447b5ae7bef7f28da01d0beb3e197899392eb0d6"
          ) {
            continue;
          }
          ...
          return (decodedEvent.args as BondkitTokenCreatedEventArgs)
            .tokenAddress;
        } catch (e) {}
      }
      throw new Error(
        "BondkitTokenCreated event not found in transaction logs."
      );
    } catch (error) {
   ...
   */
  public async getBondkitTokenConfig(
    tokenAddress: Address
  ): Promise<BondkitTokenConfig | undefined> {
    try {
      const result = await this.contract.read.getBondkitTokenConfig([
        tokenAddress,
      ]);
  ...
  public async getBondkitTokenImplementation(): Promise<Address | undefined> {
    try {
      ...
      console.error(
        "Error getting Bondkit token implementation address:",
        error
      );
      return undefined;
    }
  ...
  public async transferOwnership(newOwner: Address): Promise<Hex | undefined> {
    if (!this.walletClientInstance.account && !this.walletKey) {
      throw new Error(
        "Wallet key not set or client not connected with an account for write operation."
      );
    }
    try {
      const accountToUse = this.walletKey
