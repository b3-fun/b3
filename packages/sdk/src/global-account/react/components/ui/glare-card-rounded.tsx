...
export const GlareCardRounded = ({ children, className }: { children: React.ReactNode; className?: string }) => {
  ...
  useEffect(() => {
    // Simulate hover effect on mount
    isPointerInside.current = true;
    if (refElement.current) {
      // Apply some initial animation
      ...
      state.current.background.y = 50 + initialPercentage.y / 3 - 16.67;
      state.current.rotate.x = -(initialDelta.x / 3.5) * rotateFactor;
      state.current.rotate.y = (initialDelta.y / 2) * rotateFactor;
      state.current.glare.x = initialPercentage.x;
      state.current.glare.y = initialPercentage.y;
      updateStyles();

      refElement.current.style.setProperty("--opacity", "0.1"); // Show the glare effect

      // Simulate pointer leaving after 500ms
      const timeoutId = setTimeout(() => {
        ...
        if (refElement.current) {
          ...
          // Reset background and glare to default if needed
          state.current.glare.x = 50;
          state.current.glare.y = 50;
          state.current.background.x = 50;
          state.current.background.y = 50;
          updateStyles();
        }
      }, 500);
  ...
  return (
    ...
    >
      <div className="grid h-full w-full origin-center overflow-hidden rounded-full border-none will-change-transform [transform:rotateY(var(--r-x))_rotateX(var(--r-y))] hover:filter-none hover:[--duration:200ms] hover:[--easing:linear] hover:[--opacity:0.1]">
        <div className="grid h-full w-full mix-blend-soft-light [clip-path:inset(0_0_0_0_round_var(--radius))] [grid-area:1/1]">
          <div ref={contentRef} className={cn("h-full w-full object-cover", className)}>
            {children}
          </div>
        </div>
        <div
