...
export const GlareCard = ({ children, className }: { children: React.ReactNode; className?: string }) => {
  ...
  return (
    ...
    >
      <div className="grid h-full w-full origin-center overflow-hidden rounded-[var(--radius)] border border-black/50 will-change-transform [transform:rotateY(var(--r-x))_rotateX(var(--r-y))] hover:filter-none hover:[--duration:200ms] hover:[--easing:linear] hover:[--opacity:0.1]">
        <div className="grid h-full w-full mix-blend-soft-light [clip-path:inset(0_0_0_0_round_var(--radius))] [grid-area:1/1]">
          <div ref={contentRef} className={cn("h-full w-full bg-slate-950 object-cover", className)}>
            {children}
          </div>
        </div>
        <div
